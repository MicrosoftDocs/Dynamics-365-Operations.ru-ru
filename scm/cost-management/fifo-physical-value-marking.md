...
-   6. Выполняется закрытие запасов. В соответствии с методом ФИФО первый финансово обновленный расход будет сопоставлен с первым финансово обновленным приходом. Для проводки расхода будет выполнена корректировка в размере USD 10,00.

Новое скользящее среднее значение себестоимости отражает среднее значение по финансово обновленным проводкам. Следующая иллюстрация показывает влияния складской модели ФИФО для этой серии проводок, когда параметр **Включать физическую стоимость** не используется. ![ФИФО без параметра "Включать физическую стоимость"](./media/fifowithoutincludephysicalvalue.gif) **Ключ к схеме**
...
-   Each vertical arrow is labeled with a sequential identifier, such as *1a*. Идентификаторы определяют последовательность разноски проводок по запасам по оси времени.
...
Если **Включать физическую стоимость** установлен флажок для номенклатуры на **номенклатурной группы моделей** страницы, система использует как проводки физического и финансового прихода для расчета скользящей средней себестоимости. Когда это возможно, система будет также выполнять корректировку физически обновленных проводок расхода. Когда флажок **Включать физическую стоимость** снят, при закрытии запасов с использованием складской модели ФИФО сопоставления будут производиться только с финансово обновленными проводками. Следующая иллюстрация показывает эти проводки:
...
Проводка 5b будет сопоставлена с проводкой прихода 1b. Для этой проводки расхода будет выполнена отрицательная корректировка на USD 11,25. Новое скользящее среднее значение себестоимости отражает среднее значение по проводкам, обновленным финансово и физически, равное 27,50 долларов США. Следующая иллюстрация показывает влияния складской модели ФИФО для этой серии проводок, когда параметр **Включать физическую стоимость** используется. ![ФИФО с параметром "Включать физическую стоимость"](./media/fifowithincludephysicalvalue.gif) **Ключ к схеме**
...
Новое скользящее среднее значение себестоимости отражает среднее значение по проводкам, обновленным финансово и физически, равное 27,50 долларов США. На следующей иллюстрации показаны результаты выбора складской модели ФИФО для этой серии проводок, когда используется маркировка расходов и приходов. ![ФИФО с пометкой](./media/fifowithmarking.gif) **Ключ к схеме**
