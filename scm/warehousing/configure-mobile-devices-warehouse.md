...
Чтобы создать пункт меню действия или запроса, установите **режим** на **Косвенная**. Список **код действия** параметров то освобождается, и можно выбрать тип запроса или мероприятия, для данного пункта меню. Чтобы создать пункт меню, чтобы создать работы склада, установите **режим** на **работа**. Список **работы процесса создания** параметров то становится доступной. Чтобы создать пункт меню для обработки существующей работы склада, установите в поле **Режим** значение **Работа**, затем задайте для параметра **Использовать существующую работу** значение **Да**. **Примечание:** дополнительных полей, которые могут быть доступны для пунктов меню, в зависимости от режима, выбранного для меню, номенклатуры и ли пункт меню используются для выполнения существующей работы. Дополнительные сведения о выбранных дополнительное поле см. раздел «Параметры элемента меню дополнительных» далее в этой статье.
...
<td>Перемещение номенклатур из одного местонахождения в другое в полуавтоматическом режиме. Работник выбирает местонахождение, из которого перемещаются номенклатуры, и Microsoft Dynamics 365 for Operations использует директиву местонахождения для определения местонахождения, в которое требуется переместить номенклатуры.</td>
</tr>
<tr>
...
<td>Microsoft Dynamics 365 для операций определяет, какой тип работы, который назначен для работника и порядок, в котором сотрудник выполняет работу в. При выборе этого параметра можно нажать кнопку <strong>управляемое системой рабочих</strong> в области действий, чтобы открыть <strong>системы управляемое порядок сортировки</strong> страницы, где можно настроить сортировку для работы. Критерии сортировки контролировать порядок, в котором сотрудник выполняет работу в. Можно добавить столько критериев, сколько требуется.</td>
</tr>
<tr class="odd">
...
<td>Работник выбирает работу для выполнения, когда работа связана с более крупный объектом, таким как загрузка или отгрузка. Работник определяет порядок, в котором будет проводиться отбор номенклатур в. Если выбрать этот параметр, следующие поля станут обязательными:
<ul>
...
<td>Работник выбирает зону, пул работ или местонахождение, а Microsoft Dynamics 365 for Operations назначает работу на основании этого выбора. При выборе этого параметра можно щелкнуть <strong>Подсчет циклов</strong> на панели действий, чтобы указать дополнительную информацию для отображения и можно также определить, сколько раз работник должен повторить подсчет для определения разницы.</td>
</tr>
</tbody>
</table>
...
<td>Выберите этот параметр, чтобы сгруппировать работу размещения. Этот параметр доступен, если работа была сгруппирована работником или Microsoft Dynamics 365 for Operations. Когда работник завершает все работы комплектации в группе, создается работа размещения для этой же группы.</td>
</tr>
<tr class="even">
...
<td>Выберите шаблон работы, который создаст работу для процесса. Например при получении номенклатуры для заказа на покупку, размещение рабочих будет создаваться на основе шаблона рабочего. Если не выбрать шаблон рабочего, Microsoft Dynamics 365 для операций назначает шаблон, на основе критериев запроса. Дополнительные сведения о шаблонах работы см. в разделе <a href="control-warehouse-location-directives.md">Контроль работы склада с шаблонами работы и директивами для мест хранения</a>.</td>
</tr>
</tbody>
</table>
...
[Настройка элемента меню мобильных устройств для работы типа заказа на покупку (руководство по задачам)](https://ax.help.dynamics.com/en/wiki/set-up-a-mobile-device-menu-item-for-completing-work-of-type-purchase-order/)

[Настройка элемента меню мобильные устройства для регистрации полученных товаров (руководство по задачам)](https://ax.help.dynamics.com/en/wiki/set-up-a-mobile-device-menu-item-to-register-received-items/)
