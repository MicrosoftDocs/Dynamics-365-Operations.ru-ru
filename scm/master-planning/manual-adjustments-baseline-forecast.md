---
title: "Вручную внести корректировки базового прогноза"
...
# <a name="make-manual-adjustments-to-the-baseline-forecast"></a>Вручную внести корректировки базового прогноза
...
-   В первом столбце показаны номенклатуры, ключи распределения номенклатуры, компании и так далее, для которых был создан прогноз. Подзаголовок страницы содержит описание текущих аналитик прогноза, которые отображаются в сетке. К примеру, если подзаголовок страницы является **компании или узла / товар ключ распределения**, и одно из заголовков строк в сетке **USMF / 1 / D\_выделения**, эта строка показывает прогноз для компании USMF, для узла 1 и **D\_выделения** ключа распределения номенклатуры.
...
## <a name="forecast-aggregation-and-deaggregation"></a>Прогноз агрегирование и deaggregation
...
При выполнении корректировки прогнозируемое количество в общего уровня, система использует Взвешенное распространение изменения между строками, которые создать агрегат распространения. 
...
-   Можно указать процент уровня надежности, которой должно быть прогноза, который создается при использовании Dynamics 365 для операций спроса прогнозирование службы Microsoft Azure машинного обучения. Интервал доверия состоит из диапазона значений, которые функционируют в качестве достоверных оценок прогноза спроса. Уровень доверия 95 процентов показывает, что имеется 5-процентный риск того, что прогноз спроса окажется за пределами диапазона интервала доверия.
