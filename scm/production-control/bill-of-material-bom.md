...
description: "В этой статье приводятся сведения о спецификации (BOM) и формулы, которые являются центральной частью определения продуктов и вариантов продукта. Спецификаций и формулами указание требуемых материалов или ингредиентов для конкретного продукта. Формулы также указать побочных и попутных продуктов, полученных в контексте конкретного производства."
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
В этой статье приводятся сведения о спецификации (BOM) и формулы, которые являются центральной частью определения продуктов и вариантов продукта. Спецификаций и формулами указание требуемых материалов или ингредиентов для конкретного продукта. Формулы также указать побочных и попутных продуктов, полученных в контексте конкретного производства. 
...
Спецификация является отдельной сущности, описываемый следующую информацию:
...
Формула — это подтип спецификации, который обычно используется в процессе производства. В дополнение к компонентам и ингредиентам формула описывает сопутствующие и побочные продукты. В текущую версию, сопутствующие продукты и побочных продуктов для формулы определение требует версию формулы. Формулы обычно определяется для одного конкретного готового продукта (формулы или номенклатуры планирования), определенный в версии формулы.
...
Чтобы задать определенную спецификацию или формулу как версию спецификации или формулы по умолчанию, которая будет использоваться при сводном планировании или при создании производственных заказов, необходимо активировать версию. При активации версии производится проверка уникальности версии для существующих ограничений (например, период, узла или количество). Вы получаете сообщение об ошибке, если версия, что вы пытаетесь активировать конфликтует с версией, уже активно. Тогда следует либо деактивировать конфликтующую версию, либо изменить ограничения версии (обычно период), чтобы предотвратить неоднозначную активацию.
