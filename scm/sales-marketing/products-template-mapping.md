...
-   Убедитесь, что **Число знаков после запятой** для единицы измерения соответствует требованиям в **CDS -\> Назначение**. Если требуются другие значения на единицу измерения, это можно сделать с помощью **ValueMap** из единицы измерения, например [Each : 0] & [Pound : 2].

    -   Значение шаблона по умолчанию — 0.
