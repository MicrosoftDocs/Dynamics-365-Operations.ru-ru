...
Этот пример показывает, как вы можете ограничить конфигурацию динамика определенными видами отделки корпуса и передней панели. В первой таблице показаны отделки корпуса и передние решетки, которые обычно доступны для настройки. Значения определяются для ** Готово ящик ** и **вперед решетки** текстовых типов атрибутов.
...
## <a name="what-syntax-should-be-used-to-write-constraints"></a>Чтобы написать ограничения того, какой синтаксис следует использовать?
При написании ограничений используется синтаксис языка OML. Система использует поиск решения для ограничения поиска решения Microsoft Foundation для устранения ограничений.
...
<td><ul>
<li><strong>Оператор:</strong> подразумевает [x! = 0, y &gt;= 0]</li>
<li><strong>Нотации infix:</strong> x! = 0-: y &gt;= 0</li>
</ul></td>
</tr>
<tr class="even">
<td>И</td>
<td>Выражение истинно, только если условия истинны. Если число условий равно 0, результатом будет <strong>True</strong>.</td>
<td>И [аргументы], инфиксные: &amp;b &amp; ... &amp;z</td>
<td><ul>
<li><strong>Оператор:</strong> и [x == 2, y &lt;= 2]</li>
<li><strong>Нотации infix:</strong> x == 2 &amp;y &lt;= 2</li>
</ul></td>
</tr>
<tr class="odd">
<td>Или</td>
<td>Выражение истинно, если любое из условий истинно. Если число условий равно 0, результатом будет <strong>False</strong>.</td>
<td>Или [аргументы], инфиксные: | b | ... | z</td>
<td><ul>
<li><strong>Оператор:</strong> или [x == 2, y &lt;= 2]</li>
<li><strong>Нотации infix:</strong> x == 2 | y &lt;= 2</li>
</ul></td>
</tr>
<tr class="even">
...
<td>Плюс [аргументы], инфиксные: + b +... + z</td>
<td><ul>
...
<td>Время [аргументы], инфиксные: * b *... * z</td>
<td><ul>
...
<tr class="even">
<td>Степень</td>
<td>Выражение возводит аргумент в степень. Возведение в степень применяется справа налево. (Другими словами, это правоассоциативным.) Таким образом <strong>питания [а, б,]</strong> эквивалентно <strong>питания [, степень [б,]]</strong>. <strong>Power</strong> можно использовать только в том случае, если экспонента является положительной константой.</td>
<td>Питания [аргументы], инфиксные: ^ b ^... ^ z</td>
<td><ul>
...
<td><ul>
<li><strong>Оператор:</strong> не [x] &amp;не [y == 3]</li>
...
Нельзя использовать зарезервированные ключевые слова, такие как имена решателя, для атрибутов, компонентов или субкомпонентов в модели конфигурации продукта. Ниже приведен список зарезервированных ключевых слов, которые не могут использовать:
...
[Создание ограничения выражения (руководство по задачам)](http://ax.help.dynamics.com/en/wiki/create-an-expression-constraint/)

[Добавить вычисления в модели конфигурации продукта (руководство по задачам)](http://ax.help.dynamics.com/en/wiki/add-a-calculation-to-a-product-configuration-model/)
