...
description: "В этой статье описываются два типа ограничения таблицы для компонентов в модели конфигурации продукта — пользовательские и системные. Ограничения таблицы представляют собой матрицы разрешенных комбинаций атрибутов, где каждая строка определяет один набор возможных значений атрибутов."
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
В этой статье описываются два типа ограничения таблицы для компонентов в модели конфигурации продукта — пользовательские и системные. Ограничения таблицы представляют собой матрицы разрешенных комбинаций атрибутов, где каждая строка определяет один набор возможных значений атрибутов.
...
Прежде чем ограничение таблицы вступит в силу в модели конфигурации продукта, ограничение таблицы должны включаться в ограничение одного из компонентов модели. Возможность создания нового ограничения типа ограничения таблицы и затем выберите определение ограничения таблицы для использования. В заключение все поля в ограничении таблицы необходимо составить атрибутам в модели конфигурации продукта.
