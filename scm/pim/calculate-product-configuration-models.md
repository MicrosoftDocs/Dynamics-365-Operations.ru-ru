---
title: "Расчеты для моделей конфигурации продуктов на часто задаваемые вопросы"
description: "В этой статье Описание вычисления для моделей конфигурации продуктов и объясняется, как расчеты используются вместе с ограничениями."
author: YuyuScheller
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="calculations-for-product-configuration-models-faq"></a>Расчеты для моделей конфигурации продуктов на часто задаваемые вопросы

В этой статье Описание вычисления для моделей конфигурации продуктов и объясняется, как расчеты используются вместе с ограничениями.
...
Следующее выражение цель для расчета является длина кабеля питания, а значение ввода является цвет:  

**Выражение:**\[Если цвет == «Зеленый», 1,5, 1.0\]  

При настройке номенклатуры, длина кабеля питания составляет **1,5** при указании **зеленый** в качестве значения атрибута цвета. Если указать любой другой цвет, длина будет равна **1,0**. Однако поскольку расчеты однонаправленны, расчет не устанавливает атрибут цвета равным **Green** при указании длины **1,5**.
...
## <a name="how-do-i-set-a-target-attribute-as-hidden-or-readonly"></a>Порядок настройки целевого атрибута как скрытые или только для чтения?
Чтобы настроить атрибут как скрытый или доступный только для чтения, выполните следующие действия.

1.  Щелкните **Управление сведениями о продукте**&gt;**обычные**&gt;**моделей конфигурации продуктов**.
...
№ п/п Значения, установленные при настройке продукта являются значения, которые используются. Расчет, которые происходит при изменении входные значения не может переопределить значения, введенные для определенного атрибута пользователем.
...
-   Значения, которые возвращаются расчетом находятся за пределами домена атрибута. Примером является целым числом от \[1..10\], которое рассчитывается значение 0.
