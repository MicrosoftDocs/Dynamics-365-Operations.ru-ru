...
# <a name="set-up-a-continuity-program-for-a-call-center"></a>Настройка программы непрерывности для центра обработки вызовов
...
3.  Создание родительского продукта, представляющий непрерывности программы, созданной в задаче 2. При добавлении этого продукта в заказ на продажу, **непрерывности** открывается страница. Затем эту страницу можно использовать для создания фактического непрерывного заказа. Родительский продукт не определяет отдельные продукты, которые клиент получает в каждой отгрузке.
