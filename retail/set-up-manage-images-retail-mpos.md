---
title: "Создание и настройка изображений для современного Retail POS-терминала"
description: "В этой статье описывается, участвующих в настройке и управлении изображений для различных объектов, которые отображаются в Retail современное POS (MPOS)."
author: MargoC
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
# <a name="set-up-and-manage-images-for-retail-modern-pos"></a>Создание и настройка изображений для современного Retail POS-терминала

В этой статье описывается, участвующих в настройке и управлении изображений для различных объектов, которые отображаются в Retail современное POS (MPOS).
...
9.  Чтобы обновить шаблон СМИ в каталоге изображений со стороны канала, не забудьте запустить **1150 задания каталога** из **Retail ИТ**&gt;**график распределения**. [![catalog1](./media/catalog1.png)](./media/catalog1.png)

## <a name="previewing-an-image-from-the-entity-level"></a>Предварительный просмотр изображения с уровня объекта
1.  Со страницы номенклатуры объекта в центральном офисе можно предварительно просмотреть изображение с URL-адресом изображения, образованным по шаблону мультимедиа. Например, перейдите в соответствующий каталог и нажмите в области действий **СМИ**&gt;**изображения**. С помощью раскрывающегося списка выберите различные магазины, у которых могут быть разные профили каналов.
...
Для всех пяти поддерживаемых объектов (работника, клиента, в каталоге, категория и продукты), при каждом обновлении **определение носителей шаблона** диалоговое окно для настройки изображения, убедитесь в том, что задание каталога (1150) из **Retail ИТ**&gt;**график распределения**. Это задание позволит синхронизировать обновленный шаблон мультимедиа с каналом, после чего MPOS сможет его использовать. Запускайте задание "Каталог" (1150) после внесения каких-либо из следующих изменений:

-   При обновлении шаблон каталога носителя изображения из **каталога изображения**&gt;**определение носителей шаблона**.
-   При обновлении шаблон СМИ изображение сотрудника из **изображений сотрудника**&gt;**определение носителей шаблона**.
-   При обновлении СМИ шаблона клиента изображения из **клиента изображения**&gt;**определение носителей шаблона**.
-   При обновлении шаблон СМИ изображения продуктов из **изображения продуктов**&gt;**определение носителей шаблона**.
-   При обновлении шаблон СМИ категории изображения из **категории изображения**&gt;**определение носителей шаблона**. Необходимо также опубликовать канал.
...
4.  На экспресс-вкладке **Сформировать URL-адреса изображений для Excel** щелкните **Сформировать**. **Важно:** при внесении изменений в шаблон мультимедиа необходимо щелкнуть **Сформировать**, прежде чем можно будет использовать функциональность "Изменить в Excel". [![excel1](./media/excel1.jpg)](./media/excel1.jpg) теперь отображается предварительный просмотр изображения URL-адреса, которые были созданы на основе шаблона последней сохраненной СМИ. [![с Excel2](./media/excel2.png)](./media/excel2.png)**Примечание:** URL-адресов, которые создаются для Excel используйте путь и соглашения о СМИ шаблона, который определяется. К этим соглашениям относятся соглашения в отношении имен файлов. Ожидается , что физические изображения находятся за пределами Dynamics AX, и эти изображения можно извлечь по URL-адресам, образованным по ранее определенному шаблону мультимедиа. Эти автоматически полученные URL-адреса можно с помощью функциональности "Изменить в Excel".
5.  Щелкните **Изменить в Excel**.
6.  После открытия листа Microsoft Excel щелкните **Разрешить изменение** в появившемся запросе.
7.  В появившемся запросе щелкните **Доверять этой надстройке** в правой части и дождитесь завершения установки надстройки. [![Доверять этой надстройки](./media/excel4.jpg)](./media/excel4.jpg)
8.  Если вам будет предложено выполнить вход, введите учетные данные, которые вы использовали для входа в центральный офис. [![Знак в строке](./media/excel5.png)](./media/excel5.png)
...
11. Для всех объектов, кроме "Продукты", вы можете перезаписать URL-адреса изображений. Измените существующий URL-адрес изображения, чтобы в нем использовался новый конечный URL-адрес изображения, и замените имя файла новым именем файла изображения. Имя файла должно быть уникальным, чтобы гарантировать уникальность записи. [![Перезапись URL-адресов изображений в Excel](./media/excel6.jpg)](./media/excel6.jpg)**Примечание:** при перезаписи URL-адресов изображений с сущностями, продукты с помощью редактирования в функции Excel или страницу элемент объекта, MPOS всегда отображает все носители шаблонов URL-адресов изображений вместе с URL-адреса изображения перезаписан.
...
1.  Щелкните **каталогов**&gt;**СМИ**&gt;**изображения**и выберите изображение из каталога для обновления.
...
4.  Нажмите кнопку **OК**. URL-адрес изображения для изображения каталога обновляется, и открывается предварительный просмотр. [![preview3](./media/preview3.png)](./media/preview3.png)
...
**[![cat3](./media/cat3.png)](./media/cat3.png)Примечание:** после сопоставления изображения категории, необходимо публиковать канал и запустить задание канала помогает гарантировать, что эти изменения будут опубликованы в базе канала.
...
7.  Переведите MPOS в автономный режим. Вы должны увидеть изображение, которое вы отправили для данного продукта в центральном офисе. [![offline1](./media/offline1.png)](./media/offline1.png)
...
1.  Перейдите в каталог и на панели операций нажмите кнопку **СМИ**&gt;**изображения**.
