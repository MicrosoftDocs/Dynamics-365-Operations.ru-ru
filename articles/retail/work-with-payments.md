...
| Функция            | описание |
|---------------------|-------------|
| Обычный              | Используйте функцию **Обычный** для способа платежа, если определены такие способы оплаты, как например оплата наличными или ваучерами. Когда эти типы платежей применяются к заказу на продажу в центре обработки вызовов, они немедленно разносятся как предоплаты на счете клиента. Ваучер предоплаты разносится в истории проводок клиента, где он будут систематически сопоставляться с накладной для заказа на продажу при создании накладных. |
| Проверка               | Используйте функцию **Чек**, если вы определили инструмент банковского чека как форму платежей. Когда этот типа оплаты применяется для заказа на продажу, пользователь должен ввести номер чека клиента в процессе обработки применения оплаты. Оплата чеками всегда обрабатываются как предоплата, когда она применяются. Как и для функции платежа **Обычный**, эти ваучеры предоплаты будут систематически сопоставляться с накладными, которые созданы для заказа. |
