...
Навигация: щелкните **Розничная торговля** &gt; **Каналы** &gt; **Магазины розничной торговли** &gt; **Все магазины розничной торговли**. 
...
Навигация: щелкните **Розничная торговля** &gt; **Настройка канала** &gt; **Настройка POS** &gt; **Профили POS** &gt; **Профили оборудования**. 

Профиль оборудования представляет собой список устройств, настроенных для регистра POS или станции оборудования. Профиль оборудования может быть сопоставлен непосредственно регистру POS или станции оборудования.
...
[![Объект управления и объект обслуживания](./media/retail_peripherals_overview01.png)](./media/retail_peripherals_overview01.png)
...
> [!NOTE]
> Поддержка OPOS не гарантирует поддержку всех устройств, для которых имеются драйверы OPOS. Retail сначала должен поддерживать этот тип устройства или класс с помощью OPOS. Кроме того, сервисные службы не всегда могут быть обновлены до последней версии CCO. Также следует иметь в виду, что, как правило, качество объектов обслуживания может варьироваться.
...
Если элементы управления OPOS доступны для используемого принтера Windows, принтер все равно должен работать правильно с Retail.
...
Для информации см. раздел [Настройка и установка Retail Hardware Station](retail-hardware-station-configuration-installation.md).

### <a name="modern-pos-for-windows-setup-and-configuration"></a>Установка и настройка Modern POS для Windows

Для информации см. раздел [Настройка, установка и активация Retail Modern POS (MPOS)](retail-modern-pos-device-activation.md).
...
<li>OPOS</li>
<li>Сеть
<p><strong>Примечание.</strong> Только один лоток можно настроить, если настроен параметр <strong>Использование совместных смен</strong> в лотке.</p>
</li>
</ul>
</td>
</tr>
...
> [!NOTE]
> Если станция оборудования IIS является "Выделенной", существует однозначное соответствие между клиентом POS и станцией оборудования.
...
<li>OPOS</li>
<li>Драйвер Windows
<p><strong>Примечание.</strong> Для принтеров Windows в сети пользователь станции оборудования должен иметь разрешение на доступ к принтеру.</p>
</li>
<li>Сеть</li>
</ul>
</td>
...
<li>OPOS</li>
<li>Сеть
<p><strong>Примечание.</strong> Можно настроить только один лоток на профиль оборудования , если настроен параметр <strong>Использование совместных смен</strong> в лотке.</p>
</li>
</ul>
</td>
</tr>
...
> [!NOTE]
> Для Retail версии 1611 профиль станции оборудования больше не используется. Атрибуты, которые ранее настраивались в профиле станции оборудования, теперь являются частью самой станции оборудования.
...
3. Создайте станцию оборудования типа **Выделенный** для розничного магазина, где будет использоваться регистр POS. Описание не обязательно.
...
8. Установите и активируйте Modern POS. Дополнительные сведения об установке Modern POS см. в разделе [Настройка, установка и активация Retail Modern POS (MPOS)](retail-modern-pos-device-activation.md).
...
9. Установите и активируйте Modern POS. Дополнительные сведения об установке Modern POS см. в разделе [Настройка, установка и активация Retail Modern POS (MPOS)](retail-modern-pos-device-activation.md).
...
- Должен быть включен и использоваться только протокол Transport Layer Security (TLS) версии 1.2 (или текущая более новой версии).
    ...
    > [!NOTE]
    > По умолчанию протокол SSL и все версии протокола TLS, за исключением TLS 1.2, отключены.

    Чтобы изменить или включить эти значения, выполните следующие шаги:
...
Сведения см. в разделе [Симулятор периферийных устройств для Retail](dev-itpro/retail-peripheral-simulator.md).
...
| Epson        | Tm-T88IV   | OPOS      |                         |
| Epson        | TM-T88V    | OPOS      |                         |
| Epson        | ePOS-Print | Произвольная    | Подключение по сети   |
| Star         | TSP650II   | OPOS      |                         |
...
| Verifone     | См. комментарии | Adyen     | Соединитель Adyen поддерживает все устройства, перечисленные [здесь](https://www.adyen.com/pos-payments/terminals) |
...
- Компьютер, на котором выполняется Modern POS, доверяет сертификату, который используется на компьютере, на котором выполняется станция оборудования.
    ...
    - Для проверки этой настройки в веб-браузере перейдите к следующему URL-адресу: `https://<Computer Name>:<Port Number>/HardwareStation/ping`.
