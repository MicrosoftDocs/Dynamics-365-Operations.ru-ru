...
ms.openlocfilehash: bf242fc0ee2c472720cb7a1910764bdb3cf0db75
ms.sourcegitcommit: 2b890cd7a801055ab0ca24398efc8e4e777d4d8c
ms.translationtype: HT
ms.contentlocale: ru-RU
ms.lasthandoff: 05/07/2019
ms.locfileid: "1538375"
---
...
1. Создайте новую конфигурацию модели данных электронной отчетности для финансовых отчетов (просмотрите эту страницу для получения дополнительных сведений о разработке моделей данных электронной отчетности).
2. В конструкторе модели электронной отчетности создайте корневой элемент и назовите его "Модель".
3. В корневом элементе создайте элемент с типом данных "Список записей" и назовите его **Элементы**.
4. Создайте следующие поля в **Элементы**:

   | Имя поля | Тип данных | описание |
   |-----------|-----------|-------------|
   | Код | Строка | Это поле получает сведения из кода ячейки отчета. |
   | Код отчета | Строка | Это поле получает код финансового отчета. |
   | Текст | Строка | Это поле получает значение ячейки отчета, если рассчитанное значение является значением типа **Строка**. |
   | Стоимость | Действующий | Это поле получает значение ячейки отчета, если рассчитанное значение является значением типа **Вещественное**. |

    ![Модель данных](media/model.jpg)

5. Создайте новую конфигурацию сопоставления модели электронной отчетности в добавленной конфигурации модели электронной отчетности. Дополнительные сведения см. в разделе [Определение сопоставлений модели электронной отчетности и выбор источников данных для их]( ../../dev-itpro/analytics/tasks/er-define-model-mapping-select-data-sources-2016-11.md).

6.  Создайте сопоставление модели. В конструкторе сопоставления модели выполните следующие действия:

    1. Создайте параметр пользовательского ввода для поля **Код отчета**:

        1. В левой области на вкладке **Сопоставление** в области **Типы источников данных** выберите **Общие**, затем выберите строку **Входной параметр пользователя**.
        2. В области **Источники данных** справа выберите **Добавить корень**. 
        3. В поле **Имя** введите **FinancialReport\_UIP** в качестве имени параметра пользовательского ввода. В поле **Имя типа данных Operations (EDT, enum)** выберите расширенный тип данных (EDT) **LedgerRRGRepCode\_RU**.

            ![Параметр пользовательского ввода для кода отчета](media/uip.jpg)

    2. Аналогичным образом создайте параметр пользовательского ввода для поля **Базовая дата**. Однако введите **BaseDate\_UIP** в поле **Имя** и выберите расширенный тип данных (EDT) **BaseDate** в поле **Имя типа данных Operations (EDT, enum)**.

        ![Параметр пользовательского ввода для базовой даты](media/base-date-uip.jpg)

    3. Создайте и настройте параметр пользовательского ввода для **Дата отчета** (например, используйте расширенный тип данных (EDT) **ReportingDate\_RU**):

    4. Добавьте класс **LedgerRRGCustomReportHelper\_RU** в качестве источника данных с именем **$RRGCustom**.

        ![Источник данных $RRGCustom](media/data-source-rrg-custom.jpg)

    5. Создайте вычисляемое поле с именем **$DataCustom**, которое имеет следующее выражение:

        '$RRGCustom'.getCustomReportData(FinancialReport\_UIP, BaseDate\_UIP, ReportingDate\_UIP)

    > [!NOTE]
    > Функция **GetCustomReportData** класса **LedgerRRGCustomReportHelper\_RU** имеет входные параметры **Имя финансового отчета**, **Базовая дата** и **Дата отчета**. Она возвращает список записей все рассчитанных значений для всех настроенных ячеек выбранного финансового отчета на основе базовой даты и даты отчета. 

    Список записей, который возвращается в виде выходных данных, содержит следующие поля в каждой строке записи: 

    - **ParmFieldId** — код ячейки отчета.
    - **ParmFieldAmount** — значение вычисляемой ячейки, если она имеет тип данных **Вещественный**.
    - **ParmFieldText** — значение вычисляемой ячейки, если она имеет тип данных **Строка**.

    6. Привяжите источник данных к элементам модели:

       1. Привяжите вычисляемое поле **$DataCustom** к элементу модели **Элементы**.
       2. Привяжите поля списка записей следующим образом:

           - Элементы \> Код \<-\> ParmFieldId
           - Элементы \> Текст \<-\> ParmFieldText
...
В конфигурации формата отфильтруйте список записей **Элементы** по постоянному значению **Items.Code**. Привяжите поля **Items.Text** или **Items.Value** отфильтрованной строки к соответствующим элементам формата.

8. Выполните настроенный формат из рабочей области **Электронная отчетность**, чтобы сформировать отчет. Задайте следующие поля:

    - **Дата расчета** — укажите базовую дату для определения периода для финансового отчета.
    - **Код отчета** — выберите код для финансового отчета.
    - **Дата отчета** — при необходимости укажите дату, когда создается отчет.

    ![Выполнение отчета](media/run-report.jpg)

    > [!NOTE]
    > - Если вы создаете корректирующий отчет для закрытых периодов, и настроили поле **Дата отчета**, расчет ячеек в финансовых отчетах учитывает проводки базового периода и более поздние проводки вплоть до даты отчета, которые исправляют базовый период. (Дата отчета в учтенной проводке входит в базовый период.)
...
## <a name="configure-electronic-messages-to-generate-the-financial-report-and-store-the-results"></a>Настройка электронных сообщений для создания финансового отчета и сохранения результатов
