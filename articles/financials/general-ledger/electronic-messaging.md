...
Параметр **Разрешено удаление** статуса элемента сообщения определяет, будет ли пользователю разрешено удалить элемент сообщения с этим статусом через форму **Электронные сообщения** или форму **Элементы электронного сообщения**. 
...
| Имя поля           | Описание |
|----------------------|-------------|
|Состояние сообщения        | Уникальное имя статуса электронного сообщения, который характеризует состояние сообщения в каждый момент времени. Это имя отображается в форме электронных сообщений и в журнале, связанном с электронным сообщением. |
|Описание           | Описание, связанное со статусом электронного сообщения      |
|Тип отклика         | Некоторые действия в обработке могут привести к более чем одному типу отклика. Например, действие типа **Веб-служба** может привести к типу отклика **Выполнено успешно** или **Техническая ошибка**, в зависимости от результатов его выполнения. В этом случае необходимо определить статус сообщения для обоих типов откликов. См. раздел [Типы действий обработки сообщений](#message-processing-action-types) для получения дополнительных сведений о типах действий и связанных с ними типах ответов. |
|Статус элемента сообщения   |Бывают случаи, когда статус электронного сообщения должен влиять соответственно на статусы связанных элементов сообщений. Свяжите такой статус элемента сообщения в этом поле, выбрав его из списка подстановки. |
|Разрешено удаление          | Параметр **Разрешено** удаление статуса электронного сообщения определяет, будет ли пользователю разрешено удалить электронное сообщение с этим статусом через форму **Электронные сообщения**.            |
...
| Изменение пользователя            | В случае, когда пользователь должен быть способен изменить значение дополнительного поля из интерфейса пользователя, задайте для этого флажка значение **Да**, в противном случае — **Нет**. |
| Счетчик              | Когда дополнительное поле должно содержать порядковый номер в электронном сообщении, пометьте этот флажок. Значения дополнительного поля будет заполнены автоматически во время выполнения действия типа "Экспорт электронной отчетности".  |
| Скрыто               | Если дополнительные поле в пользовательском интерфейсе должно быть скрыто, пометьте этот флажок.  |

Каждое дополнительное поле может иметь различные значения для обработки. Можно задать эти значения на экспресс-вкладке "Значения":

| Поле                | Описание |
|----------------------|-------------|
| Значение поля          | Введите значение поля для использования в связи с сообщением или элементом сообщения во время отчетности. |
| Описание поля    | Введите описание значения поля для использования в связи с сообщением или элементом сообщения во время отчетности. |
...
Комбинации критериев, определенных в полях **Номер счета/группы**, **Код счета**, **Действует**, **Истечение срока**, не влияют по умолчанию на выбор значения для дополнительных полей, но может быть использована в исполняемом классе для реализации некоторой конкретной логики расчета значения дополнительного поля.
...
Некоторые исполняемые классы могут иметь обязательные параметры, которые должны быть определены до первого запуска исполняемого класса. Чтобы определить такие параметры, нажмите кнопку **Параметры** на панели действий, настройте соответствующие значения и поля диалоговом окне и нажмите кнопку **OK**. Важно нажать кнопку **OK** здесь, так как в противном случае параметры не будут сохранены в базу и исполняемый класс не будет вызван должным образом.
...
| URL-путь авторизации        | Укажите путь для составления URL-адреса для авторизации.  |
| URL-путь маркера                | Укажите путь для составления URL-адреса для маркера.  |
...
| Сопоставление формата авторизации  | Выберите формат электронной отчетности (ER), используемый для создания запроса для авторизации.   |
| Сопоставление модели маркера импорта    | Выберите сопоставление модели импорта электронной отчетности для использования для хранения маркера доступа.  |
| Предоставленная область     Маркер доступа истекает через  | Это поле обновляется автоматически. Его значение указывает предоставленную область запросов к веб-приложению.  |
| Принять                        | Укажите свойство принятия веб-запросов. Например, "application/vnd.hmrc.1.0+json".  |
| Тип содержимого           | Укажите тип содержимого Например, "application/json".  |

Следующие функции доступны со страницы **Веб-приложения** для поддержки процесса авторизации:
-   **Получить код авторизации** — для инициализации авторизации веб-приложения.
-   **Получить маркер доступа** — для инициализации получения маркера доступа.
-   **Обновить маркер доступа** — чтобы обновить маркер доступа.

Если маркер доступа к веб-приложению хранится в базе данных системы в зашифрованном формате, он может использоваться для запросов к веб-службе. Для целей безопасности доступ к маркеру доступа должен быть ограничен только теми ролями безопасности, которым необходимо разрешить обрабатывать эти запросы. Когда пользователь, не входящий в эту группу безопасности, пытается адресовать запрос, исключение информирует пользователя, что ему не разрешено взаимодействовать через выбранное веб-приложение.
Используйте быструю таблицу **Роли безопасности** страницы "Налог > Настройка > Электронные сообщения > Веб-приложения" для настройки ролей, которые должны иметь доступ к маркеру доступа. Когда для веб-приложения не определены роли безопасности, системный администратор только сможет взаимодействовать через это веб-приложение.
...
| Веб-адрес        | Введите интернет-адрес веб-службы. Если указано веб-приложение для веб-службы и интернет-адрес должен быть такой же, как тот, который определен для выбранного веб-приложения, нажмите кнопку **Копировать базовый URL-адрес**, чтобы скопировать **Базовый URL-адрес** из веб-приложения в поле **Интернет-адрес** веб-службы.  |
...
| Принять                  | Укажите свойство принятия веб-запросов. |
...
| Код успешного ответа   | Укажите код статуса HTTP, указывающий, что запрос выполнен успешно. |
| Сопоставление формата заголовков запроса  | Выберите формат электронной отчетности для создания заголовков веб-запросов. |
...
| Сопоставление формата          | Выберите формат электронной отчетности, который должен вызываться для действия. Это поле доступно только для действий типов **Экспорт электронной отчетности**, **Импорт электронной отчетности**, **Сообщение экспорта электронной отчетности**. |
| Сопоставление формата для URL-пути | Выберите формат электронной отчетности, который должен вызываться для действия. Это поле доступно только для действий типов **Веб-служба** и используется для составления пути URL-адреса, который будет добавлен к базовому интернет-адресу, указанным для выбранного веб-сервера. |
| Тип элемента сообщения       | Выберите тип записей, для которых должно оцениваться это действие. Это поле доступно для действий типов **Уровень выполнения элемента сообщения**, **Экспорт электронной отчетности** и **Импорт электронной отчетности**, типа **Веб-служба**, а также для некоторых других типов. Если оставить это поле пустым, вычисляются все типы элементов сообщений, определенные для обработки сообщений. |
...
| Имя файла  | Укажите имя файла, которое приведет к действию, такому как ответ от веб-сервера или создание отчета. Это поле доступно только для действий типа **Веб-служба** и **Сообщение экспорта электронной отчетности**.   |
| Показать диалоговое окно  | Установите этот флажок, если необходимо показать диалоговое окно пользователю перед созданием отчета. Это поле доступно только для действий типа **Сообщение экспорта электронной отчетности**.   |
...
- **Веб-служба** — этот тип используется для действий, которые должны передавать формируемый отчет в веб-службу. Этот тип действия не используется для отчетности сообщения о счетах продажи и покупки для Италии. Для действий типа **Веб-служба** можно указать **Текст подтверждения** на экспресс-вкладке **Прочие подробные сведения** в пункте **Действия обработки сообщения**. Этот текст подтверждения будет показан пользователю перед тем, как запрос будет направлен в выбранную веб-службу.
...
> [!NOTE]
> Экспресс-вкладка **Начальные статусы** недоступна для действий, которые имеют начальный тип **Создать сообщение**.
...
В следующей таблице показано, какие статусы результатов должны быть настроены в отношении типов действий:
...
Экспресс-вкладка **Действие** позволяет добавлять предопределенные действия к обработке. Можно указать, должно ли действие выполняться отдельно или оно может быть инициировано обработкой. Чтобы определить, может ли действие быть инициализировано только пользователем, установите флажок **Выполнить отдельно** для действия в процессе обработки. Снимите флажок у параметра **Выполнить отдельно**, если требуется, чтобы действие запускалось путем обработки, когда для сообщений или элементов сообщений в статусе, который определен как начальный статус для этого действия. Действие типа **Действие пользователя** можно выполняться только отдельно. 

Иногда необходимо объединить несколько действий в последовательность даже в том случае, когда первое из них определено для выполнения отдельно. Например, когда требуется, чтобы создание отчета было инициализировано пользователем, но после создания отчет немедленно должен отправляться в веб-службу и ответ от веб-службы должен быть отражен в системе. Можно использовать в таких целях **Неделимую последовательность**. Чтобы сделать это, нажмите кнопку **Неделимая последовательность**на панели операций экспресс-вкладки **Действие** страницы **Обработка электронных сообщений**, создайте последовательность и выберите ее в столбце **Неделимая последовательность** для тех действий, которые должны выполняться всегда друг с другом. Первое действие в этом случае можно настроить как **Выполнить отдельно**, но все остальные нельзя.
...
- **Собрать данные** — эта кнопка связана с действием типа **Заполнить записи**.
...
На экспресс-вкладке **Журнала действий** отображаются сведения обо всех действиях, которые были выполнены для выбранного сообщения. Если действие завершилось ошибкой, сообщения об ошибке будут присоединены к связанной строке журнала действий. Выберите строку и щелкните на кнопку **скрепки** в верхнем правом углу страницы, чтобы просмотреть сведения об ошибке.
...
На экспресс-вкладке **Элементы сообщения** отображаются все элементы сообщения, связанные с выбранным сообщением. Для каждого элемента сообщения следующая функция может использоваться в зависимости от статуса этого элемента сообщения:
...
- **Исходный документ** — данная кнопка позволяет пользователю открыть страницу с исходным документом выбранного сообщения.
