...
> [!IMPORTANT]
> Инфраструктура для управления финансами и операциями не поддерживает функцию копирования экземпляра. Можно развернуть новые среды и использовать перемещения базы данных для создания копий. Дополнительные сведения о самостоятельном развертывании см. в разделе [Обзор самостоятельного развертывания](../fin-ops-core/dev-itpro/deployment/infrastructure-stack.md). Дополнительные сведения о перемещениях баз данных в инфраструктуре для управления финансами и операциями см. в разделе [Домашняя страница операций по перемещению баз данных](../fin-ops-core/dev-itpro/database/dbmovement-operations.md).
...
> [!Note]
> Начиная с августа 2022 года документы в хранилище BLOB-объектов Microsoft Azure включаются при копировании производственной среды в среду "песочницы". Любые документы и шаблоны, которые прикрепляются, будут скопированы из исходной среды в целевую среду.
...
- Все пользователи, кроме тех, у кого есть роль безопасности "системный администратор", и другие учетные записи пользователей внутренних служб, будут недоступны. Пользователь с правами администратора может удалять данные до того, как другие пользователи смогут вернуться в систему.
...
4. В области задач **Копирование экземпляра** выберите экземпляр, который требуется перезаписать, и нажмите кнопку **Копировать**. Дождитесь обновления поля **Статус копирования** до **Завершено**.
...
Дополнительные сведения о копировании сред Power Apps см. в разделе [Копирование среды](/power-platform/admin/copy-environment#copy-an-environment-1).
