...
Human Resources использует следующую формулу:
...
2. **Предварительный просмотр функции: используется для эквивалента полной занятости**. Если вы включите эту функцию предварительного просмотра, Human Resources использует эквивалент полной занятости (FTE), определенный для должности, для пропорционального начисления сотруднику. Например, если значение FTE равно 0,5, а сумма начисления — 10, сотруднику будет начислено 5. Эту функцию можно использовать, только если разрешено использование нескольких типов отпусков.
