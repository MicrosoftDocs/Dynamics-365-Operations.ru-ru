...
description: В этой статье описываются концептуальные элементы рабочей области "Управление персоналом".
author: twheeloc
ms.date: 11/03/2021
ms.topic: article
ms.prod: ''
...
> [!NOTE] 
> Рекомендуется назначить должность работнику перед применением контрольного списка. Иногда задачи по адаптации направляются руководителю нового сотрудника. Однако если должность не назначена, менеджера нового сотрудника определить невозможно. В этом случае задачи подготовки сотрудника, предназначенные для руководителя, будут назначены владельцу контрольного списка вместо этого.
...
> [!NOTE]
> Параметры на странице **Параметры Human Resources** относятся к организации. Таким образом, интервал времени, в котором отображается недавний наем, может различаться в зависимости от компании. Например, в компании USMF может возникнуть необходимость просмотреть весь новый наем за последние семь дней. Однако в компании USSI может потребоваться просмотреть весь новый наем за последние 14 дней. В этом случае откройте страницу **Параметры Human Resources** в каждой компании и настройте необходимые параметры.
