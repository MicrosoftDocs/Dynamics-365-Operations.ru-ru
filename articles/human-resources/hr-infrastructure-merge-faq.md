...
description: В этой теме содержатся ответы на часто задаваемые вопросы о слиянии инфраструктуры для приложений Microsoft Dynamics 365 Human Resources и Финансы и операции.
author: twheeloc
ms.date: 08/13/2021
ms.topic: article
ms.prod: ''
...
В этой теме содержатся ответы на часто задаваемые вопросы о слиянии инфраструктуры для приложений Microsoft Dynamics 365 Human Resources и Финансы и операции.
...
- Они обеспечивают согласованность между Dynamics 365 Human Resources и другими приложениями Финансы и операции в плане управления жизненным циклом приложений (ALM), Microsoft Dynamics Lifecycle Services (LCS), географической доступности, расширяемости и т. д.
...
### <a name="how-does-this-change-affect-me-if-i-am-an-existing-customer-and-i-use-both-the-hr-module-on-the-finance-and-operations-infrastructure-and-dynamics-365-human-resources-through-custom-integrations"></a>Каким образом это изменение повлияет на меня, если я существующий клиент, и я использую как модуль HR в инфраструктуре Финансы и операции, и Dynamics 365 Human Resources при помощи пользовательской интеграции?

Пользовательские интеграции между Dynamics 365 Human Resources и модулем HR в Dynamics 365 Finance больше не потребуются. Все данные о персонале будут находиться в той же базе данных, что и другие приложения Финансы и операции.

## <a name="migration-from-dynamics-365-human-resources-to-finance-and-operations-apps"></a>Перенос данных из Dynamics 365 Human Resources в приложения Финансы и операции
...
Если в организации используются как Dynamics 365 Human Resources, так и приложения Финансы и операции, следует спланировать дополнительное время для проверки правильности переноса данных в новую среду. Миграция в новую инфраструктуру приведет к объединению данных из вашей среды Human Resources с вашей средой Финансы и операции. Конфликтующие данные требуют ввода данных пользователем, чтобы определить, как конфликт следует разрешить. Пользователям и администраторам придется управлять сопоставлениями данных там, где имеются конфликты, и тестировать миграцию в изолированных средах перед миграцией рабочих сред.
...
Для организаций, использующих модуль HR в приложениях Финансы и операции, новые возможности функций из Dynamics 365 Human Resources будут применены к среде посредством стандартного процесса обновления одной версии. Можно ожидать, что в среде будут отображаться новые функции, которые становятся доступными в каждом обновлении. Можно использовать управление функциями для включения новых функций, однако следует предусмотреть их проверку. Выполните предусмотренные процессы для проверки других обновлений среды. Дополнительные сведения о том, как обновления применяются к приложениям Финансы и операции, см. в [обзоре одной версии](../fin-ops-core/dev-itpro/lifecycle-services/oneversion-overview.md).
...
- Организации, которые в настоящее время используют модуль HR в приложениях Финансы и операции, получат функциональность управления персоналом для Dynamics 365 Human Resources в составе регулярного обновления одной версии. Новые возможности планируются для общей доступности начиная с января 2022 года.
...
- Организации, которые в настоящее время используют и Dynamics 365 Human Resources, и другие приложения Финансы и операции, будут иметь доступ к средствам миграции, чтобы они могли начать тестирование и начать миграцию начиная с конца 2022 года. Дата, на которую миграция в новую инфраструктуру должна быть завершена, еще не была определена. Однако это будет по крайней мере на один год позже даты, когда станет доступно средство миграции.
...
### <a name="we-are-using-data-integrator-to-move-data-between-dynamics-365-human-resources-and-finance-and-operations-apps-how-will-the-data-that-is-currently-being-integrated-be-affected"></a>Мы используем интегратор данных для перемещения данных между приложениями Dynamics 365 Human Resources и Финансы и операции. Как будут затронуты данные, интеграция которых в данный момент выполняется?

Данные HR, которые в данный момент в Dynamics 365 Human Resources, синхронизируются с Dataverse. Затем интегратор данных может использоваться для односторонней синхронизации с приложениями Финансы и операции. После миграции в новую инфраструктуру данные HR будут собственными для приложений Финансы и операции. Интегратору данных больше не потребуется синхронизировать данные между приложениями Финансы и операции и Human Resources.
...
### <a name="we-are-using-dual-write-to-move-hr-data-between-dataverse-and-other-finance-and-operations-apps-how-will-the-data-that-is-currently-being-integrated-be-affected-by-the-migration-to-the-new-infrastructure"></a>Для перемещения данных HR между Dataverse и другими приложениями Финансы и операции используется двойная запись. Как повлияет перенос в новую инфраструктуру на данные, которые в настоящее время интегрируются?

Данные HR будут собственными для приложений Финансы и операции в среде в новой инфраструктуре. Будет использоваться двойная запись для перемещения данных HR между новой средой и средой Dataverse.
...
Это зависит от конечной точки интеграции. Дополнительные сведения о технологиях интеграции, доступных в приложениях Финансы и операции, и о выборе наилучшей технологии интеграции см. в разделе [Обзор интеграции](../fin-ops-core/dev-itpro/data-entities/integration-overview.md).
...
Однако если приложение Human Resources и приложения Финансы и операции подключены к разным средам Power Apps, они должны быть объединены в рамках миграции. Для этой задачи потребуется повторно развернуть все Power Apps и другие настройки в новой среде.
...
Миграция в новую инфраструктуру переместит управление средами Dynamics 365 Human Resources в проект реализации Финансы и операции в LCS. Если миграция выполняет слияние Dynamics 365 Human Resources с существующей средой Финансы и операции, проект LCS для Human Resources будет объединен в проект реализации LCS для приложения Финансы и операции. Если в настоящее время используется только Dynamics 365 Human Resources, будет создан новый проект внедрения LCS, а существующий проект LCS Human Resources будет перенесен в этот новый проект.

Новый проект будет иметь тот же тип проекта, который используется приложениями Финансы и операции. Он будет иметь те же функции и функциональные возможности для управления средами. Дополнительные сведения см. в разделе [Ресурсы Lifecycle Services](../fin-ops-core/dev-itpro/lifecycle-services/lcs.md).
...
Параметры расширяемости Microsoft Power Platform и параметры расширяемости Финансы и операции будут доступны и могут использоваться для разработки. Дополнительные сведения см. в разделе [Домашняя страница модуля разработки и настройки](../fin-ops-core/dev-itpro/dev-tools/developer-home-page.md).
...
Все экспорты, в настоящее время настроенные для Azure Data Lake Storage в приложениях Финансы и операции, будут сохранять ту же конфигурацию после миграции.
...
Да. Dynamics 365 Human Resources будет включено в объединенную базу кода и инфраструктуру для приложений Финансы и операции. Обновление с Dynamics AX 2012 до Dynamics 365 Human Resources будет использовать тот же самый путь обновления и инструментарий, которые используются для обновления до последней версии приложений Финансы и операции.
