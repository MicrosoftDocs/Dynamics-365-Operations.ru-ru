...
Для задания параметров управления персоналом используются две страницы. Для параметров, которые являются общими для различных компаний, используется страница **Совместно используемые параметры управления персоналом**. Для параметров, которые относятся к конкретной компании, используется страница **Параметры управления персоналом**.
...
> [!NOTE]
> В объединенной инфраструктуре параметр **Валюта** по умолчанию на вкладке **Компенсация** на странице **Параметры управления персоналом** был удален. Далее валюта будет обрабатываться параметром **Валюта книги учета**, чтобы гарантировать отсутствие конфликтов для существующих функций управления финансами и операциями для предотвращения дублирования. Дополнительные сведения о том, как использовать функциональность валюты ГК, см. в разделе [Настройка книг учета](/general-ledger/configure-ledger#configuring-currencies-for-the-ledger.md). 
...
- Информация, которую менеджеры могут просматривать в записях с истекающим сроком действия
