---
title: Создание планов переменной компенсации
description: Эта статья описывает компоненты, которые необходимо настроить перед использованием переменной компенсации и регистрации сотрудника на план с переменной компенсацией.
author: twheeloc
ms.date: 08/24/2021
ms.topic: article
ms.prod: ''
...
Переменная компенсация составляет нерегулярную заработную плату сотрудника, такую как как премии или вознаграждений акциями. В этой статье объясняется, как настроить компоненты, которые необходимы для переменной компенсации и регистрации сотрудника на план переменных компесационных выплат.
...
Дополнительно компании могут настроить **Положения о передаче прав на льготы**. **Положения о передаче прав на льготы** описывают распределение переменного вознаграждения по времени. Например, правило вестирования может задавать, что сотрудник получает по 25 процентов от общего вознаграждения каждый год в течение следующих четырех лет. Правила вестирования являются только информационными.

## <a name="variable-compensation-plans"></a>Планы переменной компенсации
**План переменной компенсации** содержит правила, методы расчета и значения по умолчанию для расчета переменной компенсации для зарегистрированных сотрудников. При создании плана переменной компенсации необходимо задать тип переменной компенсации. Тип переменной компенсации определяет, будет ли система рассчитывать в качестве вознаграждения валютную сумму или количество единиц. 

Параметр **Ограничить доступ выбранными ролями** ограничивает доступ к плану компенсаций выбранными ролями безопасности, назначенными этому плану в модуле Human Resources. Например, при создании планов компенсаций, предназначенных для руководителей, и которые не должны быть видимыми для всех ролей, связанных с HR, этот параметр можно использовать для ограничения доступа к этим планам компенсаций. 

Также следует задать метод расчета:
...
**Правило найма** указывает, должны ли все сотрудники получить одинаковое повышение независимо от даты найма (**Правило найма** = **Нет**) или должны ли сотрудники получать процент от вознаграждения на основании продолжительности их занятости в цикле (**Правило найма** = **Процент**). 

**Зависимость** корректирует вознаграждение сотрудника на основании производительности подразделения сотрудника. Показатели производительности можно настроить для каждого подразделения на странице **Подразделения** в разделе **Связанные формы** &gt; **Компенсация** &gt; **Производительность**. Вознаграждение, которое получают сотрудники в этом подразделении, зависит от значения в поле **Процент достигнутого целевого уровня**, которое указывает производительность подразделения:
