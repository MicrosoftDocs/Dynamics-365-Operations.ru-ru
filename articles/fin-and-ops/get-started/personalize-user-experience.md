...
На навигационной панели вы найдете изображение шестеренки, которое служит для вызова кнопки меню **Настройки**. В меню **Настройки** содержится ряд вариантов. При выборе варианта **Параметры** будет открыта страница **Параметры** пользователя. На ней находится четыре вкладки с параметрами: 

-   **Внешний вид** — служит для выбора цветовой темы и размера по умолчанию элементов на ваших страницах.
...
- **Информационные поля** — на некоторых страницах есть раздел, который называется *информационным полем*. Эта область содержит доступную только для чтения информацию, связанную с предметом текущей страницы. Каждый раздел в информационной области называется информационным полем. Вы можете развернуть или свернуть информационное поле, и Finance and Operations запомнит ваше предпочтение. В некоторых случаях сворачивание информационного поля может улучшить производительность, потому что Finance and Operations не нужно будет извлекать информацию для этой информационного поля до тех пор, пока вы его не развернете.
...
Некоторые рабочие области содержат плитки с подсчетом (плитки, на которых отображаются числа), которые вы также можете захотеть включить на свою панель мониторинга. В рабочей области щелкните правой кнопкой мыши плитку счетчика, выберите **Персонализация**, затем выберите **Закрепить на панели мониторинга**. Когда вы в следующий раз перейдете на выбранную панель мониторинга (и обновите ее), вы увидите результат подсчета рядом с навигационной плиткой этой рабочей области на панели мониторинга.
...
- **Применить** — вы можете импортировать или выбрать персонализацию для одного или нескольких пользователей. Чтобы применить персонализацию к одному или нескольким пользователям, выберите роль и пользователей внутри этой роли, затем выберите существующую персонализацию или импортируйте файл персонализации для применения к выбранным пользователям. Персонализации будут быть проверены и применены для всех выбранных пользователей, когда они в следующий раз откроют выбранную страницу.

- **Очистить** – вы можете очистить персонализации страницы или рабочей области для одного или нескольких пользователей. Выберите страницу, чтобы просмотреть список пользователей, у которых персонализирована эта страница. Затем выберите пользователей, для которых необходимо очистить персонализацию этой страницы, и выберите **Очистить**. Все персонализации выбранные пользователей, которые были применены к выбранной странице или рабочей области, будут очищены. Это действие невозможно отменить. Однако если для страницы или рабочей области есть сохраненная персонализация, ее можно повторно импортировать.

- **Управление по отдельным пользователям** — выберите пользователя, чтобы просмотреть список страниц, которые были персонализированы этим пользователем.  Затем можно включить или отключить для него возможность использования персонализации для конкретных страниц или для всей системы.  Можно также импортировать, экспортировать или очистить персонализацию для этого пользователя.

- **Система** — здесь можно временно отключить все персонализации для всех пользователей в системе. Это не приведет к удалению персонализаций, просто все страницы будут сброшены в состояние по умолчанию для всех пользователей. Если вы впоследствии снова включаете персонализацию, все эти настройки будут заново применены. Можно также безвозвратно удалить все персонализации для всех пользователей в системе. Перед выполнением этого действия обязательно экспортируйте все персонализации, которые могут понадобиться в дальнейшем, поскольку удаленные персонализации невозможно потом восстановить. 

## <a name="personalization-of-inventory-dimensions"></a>Персонализация складских аналитик

При персонализации настройки складских аналитик на странице примите во внимание параметры, созданные с помощью команды **Отобразить аналитики**. Например, если вы используете персонализацию для скрытия столбца для складской аналитики "Номер партии" и при следующем открытии страницы столбец отображается, это может быть связано с тем, что параметры отображения аналитик определяют, какие столбцы складских аналитик отображаются. 

Параметры отображения аналитик применяются ко всем страницам, и эти параметры будут переопределять собой всю персонализированную настройку полей складских аналитик на отдельных страницах. 

В примере со складской аналитикой "Номер партии" эту аналитику нужно будет удалить в рамках команды **Отобразить аналитики**, чтобы соответствующий столбец не отображался в таблице. В итоге это изменение будет применяться не только на конкретной странице, но на всех страницах.
