...
-    **Заблокированные представления**: некоторые представления (такие как классическое представления и любые представления, опубликованные для вашей роли) имеют значок замка рядом с ними в селекторе представления, указывая на то, что вы не можете изменить эти представления. Однако неявная персонализация по использованию страниц автоматически сохраняется, например изменение ширины столбца сетки или разворачивание или сворачивание экспресс-вкладки. Однако можно создать личное представление на основе заблокированного представления, используя действие **Сохранить копию**, если имеются привилегии личной настройки.
...
Представление по умолчанию — это представление, которое система будет пытаться открыть при первом переходе к странице. Следует задать для этого представление, которое планируется использовать часто.  
...
В процессе публикации представления могут быть назначены одной или нескольким ролям безопасности, то есть все пользователи, имеющие эту роль, смогут получать доступ к этому представлению и использовать его, но они не могут редактировать представление. В настоящее время только системные администраторы имеют права на действие **Опубликовать** в раскрывающемся меню селектора представления.  
...
Чтобы включить сохраненные представления, системный администратор должен выполнить следующие действия: 
1.  Перейдите на страницу **Персонализация**, используя навигационный поиск. 
2.  Выберите вкладку **Настройки**.
3.  Для параметра **Включить сохраненные представления** задайте значение **Да**.

После включения этой функции все последующие сеансы пользователей будут запускаться с включенными представлениями.  

Обратите внимание, что если персонализация для среды отключена, представления будут включены даже при выполнении вышеперечисленных шагов. Это обусловлено тем, что функция представлений построена на основе подсистемы персонализации.
...
В настоящее время системные администраторы являются единственными пользователями, обладающими правами публиковать представления.  Планируется новая роль безопасности, предоставляющая клиентам большую гибкость в отношении пользователей, которым разрешена публикация.  
...
- Соответствующая страница может не поддерживать должным образом представления, поскольку она может полностью игнорировать запрос представления. Сообщайте о подобных случаях через механизм **Обратная связь**. Чтобы перейти на страницу обратной связи, щелкните **Справка и поддержка**, затем **Обратная связь**.  
