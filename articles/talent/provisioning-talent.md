...
5. Выберите **Да**, чтобы принять условия соглашения и начать развертывание.
...
5. Следующие среды PowerApps не могут быть использованы для Talent и будут отфильтрованы из списка выбора в LCS:
 ...
 > Чтобы различать среды CDS 1.0 и 2.0 на портале администрирования выберите среду и посмотрите **Сведения**. Среды CDS 2.0 все ссылаются на тот факт, что "Вы можете управлять этими параметрам в центре администрирования Dynamics 365", указывают на версию экземпляра и не имеют вкладки базы данных. 
   ...
   **Тестовые среды** Среды с именем, таким как "TestDrive — alias@domain", создаются со сроком действия 60 дней и через это время срок их действия истекает, в результате чего ваша среда будет автоматически удалена.
...
2. Распакуйте все содержимое файла ProvisionCDSEnviroinment.zip в папку.

3. Запустите программу Windows PowerShell или Windows PowerShell ISE от лица администратора.

   Посмотрите раздел [Установка политики выполнения](https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.security/set-executionpolicy?view=powershell-6) для получения дополнительных сведений о задании политики выполнения, позволяющей выполнять сценарии.
  
4. В PowerShell перейдите в папку, куда вы разархивировали загруженный файл, и выполните следующую команду, заменив значения, как указано ниже:
 
   ```.\ProvisionCDSEnvironment -EnvironmentName MyNewEnvironment -Location YourLocation```

    
   **EnvironmentName** следует заменить именем своей среды. Это имя будет отображаться в LCS и будет отображаться, когда пользователь выбирает среду Talent для использования. 

   **YourLocation** следует заменить одним из поддерживаемых регионов для Talent: unitedsates, europe, australia. 
...
5. Продолжите процесс подготовки.
