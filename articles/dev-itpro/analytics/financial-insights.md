...
**Финансовый анализ** использует Microsoft Power BI для объединения финансовых ключевых индикаторов (KPI), диаграмм и финансовых отчетов. Power BI внедрен в Microsoft Dynamics 365 Finance and Operations, Enterprise Edition.
...
Не назначайте никакой другой тип счета ГК, такой как **Балансовый отчет** или **Прибыли/Убытки**, для своих счетов ГК. Отчетность не может определить тип счета ГК, если назначены другие типы счета ГК, поскольку они не достаточно детализированы. Тип счета ГК должен быть определен для отображения задолженности и выручки в виде положительных сумм в финансовых отчетах.
...
В настоящее время данные во встроенных отчетах Power BI не могут быть ограничены юридическими лицам, к которым пользователь имеет доступ. Таким образом, интегрированные отчеты Power BI управляются с помощью обязанностей в настройках безопасности. Обязанности, которые были определены, разрешают доступ к данным всех компаний или только активной компании. В следующей таблице показаны обязанности, которые существуют, и роли, которым они назначены. Обязанности могут быть удалены или назначены различным ролям, в соответствии с требованиями вашей организации.
...
| Просмотр финансового анализа текущей компании | •   Бухгалтер •    Главный бухгалтер •    Супервизор по учету • Аудитор •   Менеджер бюджета •    Генеральный директор •   Финансовый директор •   Финансовый контролер  |   • Эти полномочия предоставляют доступ к рабочей области "Финансовый анализ". • По умолчанию активная компания используется в качестве фильтра. Невозможно добавить другие юридические лица.            |
...
•   Отчет о доходах по регионам

•   Отчет о доходах - факт/бюджет

•   Отчет о доходах с расхождениями

•   Отчет о доходах с учетом тенденций за 12 месяцев
...
В начальном выпуске рабочей области **Финансовый анализ** ни один из визуальных объектов не может быть изменено. В будущих выпусках пользователи, имеющие соответствующую безопасность, получат возможность создания новых визуальных объектов, копирования существующих визуальных объектов и изменения визуальных объектов. Хотя файлы PBIX, которые содержат отчеты, доступны как ресурсы, не рекомендуется изменять отчеты по умолчанию. Дополнительные изменения будут выполнены для модели данных, отчетов по умолчанию и визуального элемента пользовательского финансового отчета, которые используются для создания финансовых отчетов. Таким образом, чтобы воспользоваться преимуществами новых функций и изменений в модели данных в следующей версии, необходимо будет повторно внести любые изменения, внесенные в отчеты по умолчанию через Microsoft Power BI Desktop.


## <a name="filtering"></a>Фильтрация
Пользователи могут фильтровать отчеты, используя область **Фильтр** слева. Эта область является той же областью, которая доступна через Power BI Desktop.
Существуют различные уровни фильтрации, некоторые из которых могут быть недоступны, в зависимости от того, что выбрано на странице (вкладке), или от использования возможностей детализации:
...
•   **Фильтр детализации** — этот фильтр фильтрует из "исходного" визуального элемента в текущий визуальный элемент при выполнении детализации из исходного визуального элемента в текущий визуальный элемент.

![Фильтр](./media/filter.png)


Чтобы удалить значение определенного фильтра, выберите символ очистки рядом с ним. Не следует удалять фильтр, выбрав X. При выборе X поле, к которым применяется этот фильтр, удаляется как параметр фильтра. Если вы случайно удалили поле из фильтра, закройте рабочую область и откройте ее снова. Параметры фильтра по умолчанию будет применены повторно.
...
Фильтр **Финансовый календарь** необходим, чтобы правильный календарь использовался для визуального объекта. По умолчанию фильтр уровня отчета задается на финансовый календарь активного юридического лица. Если изменить фильтр на финансовый календарь, который имеет другую начальную или конечную дату, не будут включены начальные сальдо. Таким образом, ваши финансовые отчеты **Балансовый отчет** не будут отображаться правильные сальдо. При выборе дополнительного финансового календаря в фильтре имеется дополнительный набор столбцов. Каждый дополнительный набор столбцов отображает суммы для другого финансового календаря.
...
Визуальный элемент **Сальдо по банковским счетам** использует суммы в валюте по банковским счетам.

## <a name="dimensions"></a>Измерения

Финансовые отчеты по умолчанию не включают никакие финансовые аналитики, они используют только счета ГК. Поддержка финансовых аналитик будет доступна в будущих выпусках, когда появится возможность изменения отчетов. Тогда организации смогут фильтровать по значениям финансовой аналитики.

Некоторые финансовые отчеты содержат аналитики, основанные на проводках по вспомогательной книге. Цель новых финансовых отчетов заключается в том, чтобы обеспечить фильтрацию по аналитикам, которые не настроены как финансовые аналитики. Например, отчет по умолчанию "Расходы по поставщикам" позволяет выйти вниз за пределы счета ГК, чтобы можно было просмотреть сальдо с разбивкой по поставщикам. Поставщик не настроен в качестве финансовой аналитики. Вместо этого система возвращается к исходной проводке вспомогательной книги, чтобы найти поставщика.
...
При обратной детализации из финансовых отчетов производится переход в проводник источника учета (ASE), а не в проводки по ваучеру. ASE не отображает только записи учета в главной книге. Вместо этого он содержит сведения по проводке во вспомогательной книге. Таким образом, вы получите существенно более подробную информацию об исходной проводке и можете использовать ее для анализа. Например, можно увидеть, кто был поставщиком или клиентом, что приобрел клиент или продал поставщик, и даже по какому проекту была проводка.
...
Если вы развернули строку недостаточно вниз, детализация не работает. Например, если развернуть только до категории счета ГК, невозможно выполнить детализацию в ASE по сальдо, так как счет ГК является обязательным полем для фильтрации в ASE.
...
Power BI не дает возможность скрывать и отображать пустые строки. Если строка не содержит каких-либо данных, строка не будет отображена в визуальном элементе.

## <a name="what-is-coming-in-future-releases"></a>Что ожидается в будущих версиях?
Новые рабочие области и финансовые отчеты, использующие Power BI, будут продолжать улучшаться. Ниже приведены некоторые из новых функций, которые рассматриваются для будущих выпусков:

 - Возможность копирования, редактирования, удаления и создания визуальных элементов или даже финансовых отчетов                                                  
 - Дополнительные отчеты по умолчанию                                                                                                            
    - Поддержка дополнительных данных вспомогательных книг                                                                                            
 - Поддержка валюты отчетности                                                                                                      
 - Добавление пользовательских вычислений для строк и столбцов                                                                                          
 - Возможность экспортировать финансовые отчеты в Microsoft Excel                                                                     
   - Сохранение формата финансового отчета при экспорте.                                                                          
   - Анализ данных в Excel путем создания сводной таблицы, использующей сведения из визуального элемента.                                              
 - Поддержка языковых стандартов                                                                                                                        
 - Возможность определения иерархии отчетности, чтобы можно было определить иерархии счетов ГК или организационную иерархию, которые можно использовать в финансовых отчетах для конструирования, фильтрации и безопасности.                                                                    
 - Поддержка печати

О новых возможностях будет сообщаться на веб-сайте дорожной карты по мере начала работы: https://roadmap.dynamics.com/.

## <a name="additional-resources-for-power-bi"></a>Дополнительные ресурсы для Power BI

Сведения в следующих ресурсах не требуются для включения внедренных отчетов для рабочей области **Обзор финансового директора** или **Финансовый анализ** в производственной среде. Однако они полезны для окон разработки и при необходимости внедрения собственных отчетов Power BI в Finance and Operations.
