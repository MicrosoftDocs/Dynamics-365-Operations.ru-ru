...
**Управление бизнес-документами** основано на среде электронной отчетности и позволяет бизнес-пользователям редактировать шаблоны деловых документов с помощью службы Microsoft Office 365 или соответствующего приложения Microsoft Office для настольных ПК. Изменения в документы могут включать изменение дизайна деловых документов и добавление заполнителей для дополнительных данных из Dynamics 365 for Finance and Operations без изменений исходного кода и новых развертываний. Для обновления шаблонов бизнес-документов знание структуры электронной отчетности не требуется.
...
Для использования управления бизнес-документами для редактирования шаблонов в формате Excel или Word с помощью приложений Microsoft Office для рабочего стола необходимо установить Microsoft Office 2010 или более позднюю версию. Это поддерживается в облачном и в локальном развертывании Finance and Operations.

## <a name="business-document-availability"></a>Доступность бизнес-документов

Следующие отчеты, с шаблонами на основе Excel, будут доступны в выпуске открытой предварительной версии:

**Модуль "Расчеты с клиентами"** (август 2019 г.)

- Авансовый счет продаж
- Отборочная накладная заказа на продажу

**Модуль "Расчеты с поставщиками"** (август 2019 г.)

- Авансовый счет покупки
- Заказ на покупку
- Отборочная накладная заказа на покупку

Будут доступны дополнительные отчеты. Специальные уведомления о дополнительных отчетах будут отправлены отдельно. 

Полный список всех отчетов, запланированных для выпуска в октябре 2019 г., можно найти в [Настраиваемая отчетность бизнес-документов в Word и Excel](https://docs.microsoft.com/en-us/dynamics365-release-plan/2019wave2/dynamics365-finance-operations/configurable-business-documents-reporting-word-excel-pdf#feature-details).

# <a name="example-enable-configure-and-use-business-document-management"></a>Пример: активация, настройка и использование управления бизнес-документами
...
Так как управление бизнес-документами выполняется на основе среды ER, необходимо настроить параметры ER, чтобы начать работу с системой управления бизнес-документами. Для этого необходимо настроить параметры электронной отчетности, как описано в разделе [Настройка платформы электронной отчетности](electronic-reporting-er-configure-parameters.md). Также необходимо добавить нового поставщика конфигурации, как описано в разделе [Создание поставщиков конфигураций и пометка их как активных](tasks/er-configuration-provider-mark-it-active-2016-11.md).
...
Необходимо импортировать конфигурации ER, содержащие шаблоны бизнес-документов, в текущий экземпляр Finance and Operations. Загрузите и локально сохраните следующие файлы для выполнения этой процедуры.
...
1. Выполните вход в Finance and Operations как пользователь, имеющий доступ к странице **Параметры бизнес-документов**.
...
По умолчанию, когда доступ к разрешениям управления бизнес-документами отключен, все пользователи, имеющие доступ к рабочей области управления бизнес-документами, увидят все шаблоны решений ER, доступные в Finance and Operations. В рабочей области управления бизнес-документами будут показаны только шаблоны, находящиеся в конфигурациях формата ER, которые помечены тегом **Тип бизнес-документов**.
...
1. Выполните вход в Finance and Operations как пользователь, имеющий доступ к странице **Конфигуратор разрешений доступа**.
...
С помощью этого параметра каждый пользователь, которому назначена любая роль безопасности, которая настроена для выполнения обязанностей **Управление шаблонами бизнес-документов** (имя AOT **ERBDManageTemplates**), может открыть рабочую область управления бизнес-документами и может редактировать любой шаблон, доступный в Finance and Operations.
...
1. Выполните вход в Finance and Operations как пользователь, имеющий доступ к рабочей области управления бизнес-документами.
...
Будет открыта страница **Редактор шаблона BDM** из Finance and Operations. Выбранный шаблон будет доступен для оперативного редактирования с помощью Office 365.

![Страница рабочей области управления бизнес-документами](./media/BDM-Overview-EditingLayout1.png)

### <a name="edit-a-template-in-office-365"></a>Редактирование шаблона в Office 365

Измените шаблон, используя функции Office 365. Например, в Office Online измените шрифт приглашений полей в заголовке шаблона с **Обычный** на **Полужирный**. Эти изменения автоматически сохраняются для редактируемого шаблона, который хранится в хранилище первичного шаблона (по умолчанию это хранилище больших двоичных объектов Azure), которое настроено для среды ER.

![Страница редактора шаблона управления бизнес-документами](./media/BDM-Overview-EditingLayout2.png)

### <a name="edit-a-template-in-the-office-desktop-application"></a>Редактирование шаблона в приложении Office для рабочего стола
...
1. В Finance and Operations перейдите в компанию **USMF**.
...
7. Выберите **Печать**, затем щелкните **Выбранное**.
...
2. Если выбрать **ОК**, чтобы подтвердить публикацию, черновая версия производного формата ER **Копия отчета FTI клиента (GER)**, который содержит измененный шаблон, будет помечена как завершенная. Измененный шаблон становится доступным для других пользователей Finance and Operations. Завершенные версии этого формата ER будут хранить только последнюю активную версию шаблона. Другие редакции будут удалены. Выберите **Отмена** для просмотра других вариантов.

## <a name="frequently-asked-questions"></a>Вопросы и ответы

#### <a name="i-selected-new-document-but-instead-of-opening-the-bdm-template-editor-page-in-finance-and-operations-i-have-been-sent-to-the-office-365-web-page"></a>Был выбран пункт **Создать документ**, но вместо того, чтобы открыть страницу **Редактор шаблонов BDM** в Finance and Operations, открылась веб-страница Office 365.
Это известная проблема, связанная с переадресацией Office 365 обратно в Finance and Operations. Это происходит при первом входе в Office 365. Чтобы обойти эту проблему, следует нажать кнопку **Назад** в браузере, чтобы вернуться в Finance and Operations.

#### <a name="i-understand-how-to-edit-a-template-by-using-office-365-in-the-first-application-session-and-how-to-use-the-template-in-the-second-application-session-adjusting-the-template-to-see-how-my-changes-affect-the-generated-business-document-can-i-do-this-using-the-office-desktop-application"></a>Понятно, как редактировать шаблон с помощью Office 365 в первом сеансе приложения и как использовать шаблон во втором сеансе приложения, настраивая шаблон, чтобы увидеть, как изменения влияют на созданный бизнес-документ. Можно ли это сделать с помощью настольного приложения Office?
...
#### <a name="i-get-the-error-value-cannot-be-null-parameter-name-externalid-when-i-select-open-in-desktop-app-how-do-i-work-around-this"></a>Получена ошибка "Значение не может быть равно null. Имя параметра: externalId" при выборе **Открыть в классическом приложении**. Как это обойти? 
Скорее всего вы выполнили вход в текущий экземпляр Finance and Operations домена Azure AD, отличающегося от домена Azure AD, который использовался для развертывания данного экземпляра Finance and Operations. Поскольку служба SharePoint, используемая для хранения шаблонов, чтобы сделать их доступными для редактирования с помощью приложений Office для рабочего стола, относится к тому же домену, что и Finance and Operations, у нас нет прав доступа к службе SharePoint. Чтобы решить эту проблему, выполните вход в текущий экземпляр Finance and Operations, используя учетные данные пользователя с правильным доменом Azure AD.
...
[Настройка электронной отчетности для загрузки данных в Power BI](general-electronic-reporting-report-configuration-get-data-powerbi.md)
