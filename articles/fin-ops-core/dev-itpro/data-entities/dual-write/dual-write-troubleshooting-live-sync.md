...
description: В этой статье содержатся сведения об устранении неполадок, которые могут быть полезны при разрешении проблем с синхронизацией в режиме реального времени.
...
Эта статья предоставляет информацию по устранению неполадок для интеграции двойной записи между приложениями для управления финансами и операциями и Microsoft Dataverse. А именно, в нем содержатся сведения об устранении неполадок при синхронизации в режиме реального времени.

> [!IMPORTANT]
> Для устранения некоторых проблем, рассмотренных в этой статье, может потребоваться роль системного администратора или учетные данные администратора клиента Azure Active Directory (Azure AD). В каждом разделе объясняется, требуются ли конкретные роль или учетные данные.
...
При создании строки в приложении для управления финансами и операциями может появиться следующее сообщение об ошибке:
...
При попытке сохранить данные таблицы в приложении для управления финансами и операциями может появиться следующее сообщение об ошибке:
...
## <a name="handle-read-or-write-privilege-errors-when-you-create-data-in-a-finance-and-operations-app"></a>Обработка ошибок привилегии чтения или записи при создании данных в приложении для управления финансами и операциями

При создании данных в приложении для управления финансами и операциями может появиться сообщение об ошибке "Ошибочный запрос".
...
1. В приложении для управления финансами и операциями найдите подразделение, сопоставленное в наборе подключений интеграции данных.
...
При создании данных в приложении для управления финансами и операциями может появиться следующее сообщение об ошибке:
...
Эта ошибка возникает в том случае, когда среда Dataverse ошибочно сбрасывается при попытке создать данные в приложении для управления финансами и операциями.
...
Чтобы решить эту проблему, необходимо выполнить шаги как в Dataverse, так и в приложении для управления финансами и операциями.

1. В приложении для управления финансами и операциями выполните следующие действия:

    1. Откройте объект **DualWriteProjectConfigurationEntity** с помощью надстройки Excel. Чтобы использовать надстройку, включите режим конструктора в надстройке Excel для управления финансами и операциями и добавьте **DualWriteProjectConfigurationEntity** на лист. Дополнительные сведения см. в разделе [Просмотр и обновление данных сущностей с помощью Excel](../../office-integration/use-excel-add-in.md).
    2. Выбор и удаление записей, имеющих проблемы в сопоставлении и проекте с двойной записью. Имеется две записи для каждого сопоставления с двойной записью.
    3. Опубликуйте изменения, используя надстройку Excel. Этот шаг важен, поскольку он удаляет записи из объекта и соответствующих таблиц.
    4. Чтобы предотвратить возникновение ошибок при повторной связи между средами для управления финансами и операциями или Dataverse убедитесь, что не осталось конфигураций двойной записи.
...
3. В приложении для управления финансами и операциями выполните следующие действия:
...
## <a name="data-from-finance-and-operations-apps-isnt-synced-to-dataverse"></a>Данные из приложений для управления финансами и операциями не синхронизированы с Dataverse

Во время синхронизации в режиме реального времени может возникнуть проблема, при которой синхронизируются только части данных из приложений для управления финансами и операциями с Dataverse или данные не синхронизируются.
...
+ Для сопоставляемого источника данных необходимо зарегистрировать бизнес-события. Некоторые источники данных могут использовать внешнее соединение и могут быть помечены как доступные только для чтения в приложениях для управления финансами и операциями. Эти источники данных не отслеживаются.
...
1. Проверьте сопоставления полей на странице администрирования двойной записи. Если поле не сопоставляется из приложений для управления финансами и операциями в Dataverse, оно не будет отслеживаться. Например, на следующем рисунке поле **Описание** отслеживается с Dataverse, но не из приложений для управления финансами и операциями. Изменения в этом поле внутри приложений для управления финансами и операциями отслеживаться не будут.
...
## <a name="error-when-you-create-a-record-where-multiple-records-are-sent-from-a-finance-and-operations-app-to-dataverse-in-the-same-batch"></a>Ошибка при создании записи, в которой несколько записей отправляются из приложения для управления финансами и операциями в Dataverse в одном пакете

Для любой проводки приложение для управления финансами и операциями создает данные в пакете и отправляет их как пакет в Dataverse. Если две записи создаются как часть одной и той же проводки и ссылаются друг на друга, может быть выведено сообщение об ошибке, напоминающее следующий пример в приложении для управления финансами и операциями:
...
Чтобы устранить проблему, создайте отношения с сущностью в приложении для управления финансами и операциями, чтобы указать, что две сущности связаны друг с другом и что связанные записи обрабатываются в одной и той же проводке.

## <a name="enable-verbose-logging-of-error-messages"></a>Включение подробного протоколирования сообщений об ошибках

В приложении для управления финансами и операциями могут возникнуть ошибки, связанные с данной средой Dataverse. Сообщение об ошибке может не содержать полного текста сообщения или других соответствующих данных. Чтобы получить дополнительные сведения, можно включить подробное протоколирование, установив флаг **IsDebugMode**, который присутствует в сущности **DualWriteProjectConfigurationEntity** во всех конфигурациях проектов в приложениях для управления финансами и операциями.
...
При попытке добавления адреса для клиента или контакта в приложениях для управления финансами и операциями или Dataverse может появиться следующее сообщение об ошибке:
...
Когда клиент создается в Dataverse, создается новый номер субъекта. Сообщение об ошибке отображается, если запись клиента вместе с субъектом синхронизирована с приложениями для управления финансами и операциями, но уже существует запись клиента с иным номером субъекта.
...
В Dataverse таблице **msdyn_party** имеется номерная серия. Когда учетная запись создается в Dataverse, создается новый субъект (например, **Party-001** типа **Организация**). Эти данные отправляются в приложение для управления финансами и операциями. Если среда Dataverse была сброшена или среда для управления финансами и операциями связана с другой средой Dataverse, а затем создается новая запись контакта в Dataverse, создается новое значение субъекта, начинающееся с **Party-001**. Создается запись субъекта **Party-001** типа **Физическое лицо**. Когда эти данные синхронизированы, приложения для управления финансами и операциями отображают предыдущее сообщение об ошибке, поскольку запись субъекта **Party-001** для типа **Организация** уже существует.
