...
При создании строки в приложении Финансы и операции может появиться следующее сообщение об ошибке:
...
При попытке сохранить данные таблицы в приложении Финансы и операции может появиться следующее сообщение об ошибке:
...
## <a name="handle-read-or-write-privilege-errors-when-you-create-data-in-a-finance-and-operations-app"></a>Обработка ошибок привилегии чтения или записи при создании данных в приложении Финансы и операции

При создании данных в приложении Финансы и операции может появиться сообщение об ошибке "Ошибочный запрос".
...
1. В приложении Финансы и операции найдите подразделение, сопоставленное в наборе подключений интеграции данных.
...
При создании данных в приложении Финансы и операции может появиться следующее сообщение об ошибке:
...
Эта ошибка возникает в том случае, когда среда Dataverse ошибочно сбрасывается при попытке создать данные в приложении Финансы и операции.
...
Чтобы решить эту проблему, необходимо выполнить шаги как в Dataverse, так и в приложении Финансы и операции.

1. В приложении Финансы и операции выполните следующие действия:

    1. Откройте объект **DualWriteProjectConfigurationEntity** с помощью надстройки Excel. Чтобы использовать надстройку, включите режим конструктора в надстройке Финансы и операции Excel и добавьте **DualWriteProjectConfigurationEntity** на лист. Дополнительные сведения см. в разделе [Просмотр и обновление данных сущностей с помощью Excel](../../office-integration/use-excel-add-in.md).
    2. Выбор и удаление записей, имеющих проблемы в сопоставлении и проекте с двойной записью. Имеется две записи для каждого сопоставления с двойной записью.
    3. Опубликуйте изменения, используя надстройку Excel. Этот шаг важен, поскольку он удаляет записи из объекта и соответствующих таблиц.
    4. Чтобы предотвратить возникновение ошибок при повторной связи между средами Финансы и операции или Dataverse убедитесь, что не осталось конфигураций двойной записи.
...
3. В приложении Финансы и операции выполните следующие действия:
...
## <a name="data-from-finance-and-operations-apps-isnt-synced-to-dataverse"></a>Данные из приложений Финансы и операции не синхронизированы с Dataverse

Во время синхронизации в режиме реального времени может возникнуть проблема, при которой синхронизируются только части данных из приложений Финансы и операции с Dataverse или данные не синхронизируются.
...
+ Для сопоставляемого источника данных необходимо зарегистрировать бизнес-события. Некоторые источники данных могут использовать внешнее соединение и могут быть помечены как доступные только для чтения в приложениях Финансы и операции. Эти источники данных не отслеживаются.
...
1. Проверьте сопоставления полей на странице администрирования двойной записи. Если поле не сопоставляется из приложений Финансы и операции в Dataverse, оно не будет отслеживаться. Например, на следующем рисунке поле **Описание** отслеживается с Dataverse, но не из приложений Финансы и операции. Изменения в этом поле внутри приложений Финансы и операции отслеживаться не будут.
...
## <a name="error-when-you-create-a-record-where-multiple-records-are-sent-from-a-finance-and-operations-app-to-dataverse-in-the-same-batch"></a>Ошибка при создании записи, в которой несколько записей отправляются из приложения Финансы и операции в Dataverse в одном пакете

Для любой проводки приложение Финансы и операции создает данные в пакете и отправляет их как пакет в Dataverse. Если две записи создаются как часть одной и той же проводки и ссылаются друг на друга, может быть выведено сообщение об ошибке, напоминающее следующий пример в приложении Финансы и операции:
...
При попытке добавления адреса для клиента или контакта в приложениях Финансы и операции или Dataverse может появиться следующее сообщение об ошибке:
...
Когда клиент создается в Dataverse, создается новый номер субъекта. Сообщение об ошибке отображается, если запись клиента вместе с субъектом синхронизирована с приложениями Финансы и операции, но уже существует запись клиента с иным номером субъекта.
...
В Dataverse таблице **msdyn_party** имеется номерная серия. Когда учетная запись создается в Dataverse, создается новый субъект (например, **Party-001** типа **Организация**). Эти данные отправляются в приложение Финансы и операции. Если среда Dataverse была сброшена или среда Финансы и операции связана с другой средой Dataverse, а затем создается новая запись контакта в Dataverse, создается новое значение субъекта, начинающееся с **Party-001**. Создается запись субъекта **Party-001** типа **Физическое лицо**. Когда эти данные синхронизированы, приложения Финансы и операции отображают предыдущее сообщение об ошибке, поскольку запись субъекта **Party-001** для типа **Организация** уже существует.
