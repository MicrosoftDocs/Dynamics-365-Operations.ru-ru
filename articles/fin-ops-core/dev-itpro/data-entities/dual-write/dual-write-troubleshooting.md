...
Эта тема предоставляет общая информацию по устранению неполадок для интеграции двойной записи между приложениями Finance and Operations и Common Data Service.
...
4. Запустите **PackageDeployer.exe**, чтобы выбрать среду Common Data Service и установить все решения.

    ![Содержимое папки Tools](media/paste_copied_files.png)

## <a name="enable-and-view-the-plug-in-trace-log-in-common-data-service-to-view-error-details"></a>Включение и просмотр журнала трассировки подключаемого модуля в Common Data Service для просмотра подробных сведений об ошибке
...
1. Выполните вход в приложение Finance and Operations, откройте страницу **Параметры**, затем в области **Система** выберите **Администрирование**.
2. На странице **Администрирование** выберите **Параметры системы**.
3. На вкладке **Настройка** в поле **Трассировка подключаемого модуля и пользовательского действия рабочего процесса** выберите **Все**, чтобы включить журнал трассировки подключаемого модуля. Если требуется вести журналы трассировки только при возникновении исключений, вместо этого можно выбрать **Исключение**.


Чтобы просмотреть журнал трассировки, выполните следующие действия.

1. Выполните вход в приложение Finance and Operations, откройте страницу **Параметры**, затем в области **Настройка** выберите **Журнал трассировки подключаемого модуля**.
2. Найдите журналы трассировки, для которых в поле **Имя типа** указано значение **Microsoft.Dynamics.Integrator.CrmPlugins.Plugin**.
...
Ошибки двойной записи, возникающие в Common Data Service, могут проявляться в приложении Finance and Operations. В некоторых случаях полный текст сообщения об ошибке недоступен, поскольку сообщение слишком длинное или содержит личную идентификационную информацию (PII). Подробное протоколирование ошибок можно включить, выполнив следующие действия.

1. Все конфигурации проектов в приложениях Finance and Operations имеют свойство **IsDebugMode** в объекте **DualWriteProjectConfiguration**. Откройте объект **DualWriteProjectConfiguration** с помощью надстройки Excel.

    > [!TIP]
    > Простой способ открыть объект — включить режим **Конструктор** в надстройке Excel, затем добавить **DualWriteProjectConfigurationEntity** на лист. Дополнительные сведения см. в разделе [Открытие данных объекта в Excel и из обновление с помощью надстройки Excel](../../office-integration/use-excel-add-in.md).
...
## <a name="unlink-and-link-another-common-data-service-environment-from-a-finance-and-operations-app"></a>Удаление связи и связывание с другой средой Common Data Service из приложения Finance and Operations

**Необходимые учетные данные для отмены связи со средой:** администратор клиента Azure AD
