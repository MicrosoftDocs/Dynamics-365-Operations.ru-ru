...
description: В этой статье приводятся сведения о продаже перспективному клиенту в случае двойной записи.
...
> [!NOTE]
> В Dynamics 365 Sales и перспективный клиент, и клиент ссылаются на запись в таблице **Учетная запись**, где столбец **RelationshipType** — это **Перспективный клиент** или **Клиент**. Если бизнес-логика включает процесс квалификации **счета**, когда запись **Счет** сначала создается и квалифицируется как перспективный клиент, а затем как клиент, эта запись синхронизируется с приложением для управления финансами и операциями только тогда, когда она является клиентом (`RelationshipType=Customer`). Если вы хотите, чтобы строка **Учетная запись** синхронизировалась как перспективный клиент, то необходимо создать пользовательское сопоставление для интеграции данных перспективных клиентов.
...
[Заголовки накладной заказа на продажу V2](mapping-reference.md#118) | накладные | Таблица заголовков накладных по продаже V2 в приложении для управления финансами и операциями содержит накладные для заказов на продажу и накладные с произвольным текстом. Фильтр применяется в Dataverse для двойной записи, которая отфильтровывает все документы накладных с произвольным текстом. |
