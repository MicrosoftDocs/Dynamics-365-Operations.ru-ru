...
В сущность **Заказ** добавлены новые поля, которые отображаются на странице. Большая часть этих полей отображается на вкладке **Интеграция** в модуле Sales. Для получения дополнительных сведений о том, как сопоставляются поля статуса, обратитесь к разделу документации [Настройка сопоставления для полей статуса заказа на продажу](https://review.docs.microsoft.com/en-us/dynamics365/fin-ops-core/dev-itpro/data-entities/dual-write/sales-status-map?branch=robin-dw-status-map)
