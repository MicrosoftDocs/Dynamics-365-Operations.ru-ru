...
В Supply Chain Management поля **Сайт** и **Склад** являются обязательными для строк предложений с расценками и строк заказа. Если задан сайт и склад в параметрах заказа по умолчанию, эти поля будут автоматически установлены при добавлении продукта в строку предложения с расценками или в строку заказа. 
...
+ К продукту в предложении можно добавить скидку. В этом случае скидка будет синхронизирована с Supply Chain Management. Поля **Скидка**, **Расходы** и **Налог** в заголовке управляются настройкой в Supply Chain Management. Эта настройка не поддерживает сопоставление интеграции. Вместо этого поля **Цена**, **Скидка**, **Расходы** и **Налог** управляются и обрабатываются в Supply Chain Management.
+ Поля **% скидки**, **Скидка** и **Сумма транспортировки** в заголовке предложения по продаже доступны только для чтения.
+ Поля **Условия фрахта**, **Условия поставки**, **Способ доставки** и **Режим доставки** не являются частью сопоставлений по умолчанию. Чтобы сопоставить эти поля, необходимо настроить преобразование значений, характерное для данных в организациях, между которыми синхронизируется объект.
...
+ Активация и синхронизация заказов из Sales возможна только в том случае, если все продукты в заказе получены из приложений Finance and Operations. Таким образом, не может быть вписываемых продуктов.
...
В сущность **Заказ** добавлены новые поля, которые отображаются на странице. Большая часть этих полей отображается на вкладке **Интеграция** в модуле Sales. Имеется несколько специальных полей:

+ В поле **Состояние обработки** отображается статус обработки заказа в Supply Chain Management. Это поле заблокировано и отображает только статус заказа из модуля Supply Chain Management. Доступны следующие значения:
    ...
    В следующей таблице показано, как статус обработки сопоставляется со значением параметра **Код статуса CRM**.

    | Статус обработки           | Код статуса CRM    |
    |-----------------------------|--------------------|
    | Активные сотрудники                      | Новый/Ожидание/Отложено |
    | Подтверждено/Скомплектовано            | В работе        |
    | Частично доставлено         | Частичный            |
    | Доставлено                   | Выполнено           |
    | Выставлен счет/Выставлен частичный счет | Выставлено накладных           |
    | Отменено                    | Без денег           |
...
+ Поля **Условия фрахта**, **Условия поставки** и **Способ доставки** не включены в сопоставления по умолчанию. Чтобы сопоставить эти поля, необходимо настроить преобразование значений, характерное для данных в организациях, между которыми синхронизируется объект.

## <a name="templates"></a>Шаблоны

Продажи перспективному клиенту включают коллекцию сопоставлений основных объектов, которые работают совместно во время взаимодействия данных клиентов, как показано в следующей таблице.
...
Ниже приведены соответствующие сопоставления базовых объектов для продажи перспективному клиенту:
