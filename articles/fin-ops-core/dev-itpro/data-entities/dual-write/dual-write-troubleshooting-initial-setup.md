...
Эта тема предоставляет информацию по устранению неполадок для интеграции войной записи между приложениями Финансы и операции и Dataverse. Конкретно, в этом разделе содержатся сведения об устранении неполадок, которые могут возникнуть в процессе начальной настройки интеграции двойной записи.
...
## <a name="you-cant-link-a-finance-and-operations-app-to-dataverse"></a>Невозможно связать приложение Финансы и операции с Dataverse

**Необходимая роль для настройки двойной записи:** системный администратор в приложениях Финансы и операции и Dataverse.
...
Необходимы учетные данные администратора клиента Azure AD, чтобы можно было связать среды Финансы и операции и Dataverse. После связывания сред пользователи могут войти в систему, используя учетные данные своих учетных записей, и обновить существующее сопоставление таблиц.
...
## <a name="error-when-opening-the-dual-write-page-in-finance-and-operations-apps"></a>Ошибка при открытии страницы двойной записи в приложениях Финансы и операции
...
+ Это приложение требуется для Dataverse для связи с приложениями Финансы и операции.
...
## <a name="finance-and-operations-environment-is-not-discoverable"></a>Среда Финансы и операции не является видимой

Возможно следующее сообщение об ошибке:

*Среда приложений Финансы и операции \*\*\*.cloudax.dynamics.com не обнаруживается.*

Есть две вещи, которые могут стать причиной проблем со средой, которая не может быть обнаружена:

+ Пользователь, используемый для входа, не является тем же клиентом, что и экземпляр Финансы и операции.
+ Существуют устаревшие экземпляры Финансы и операции, которые были размещены корпорацией Майкрософт и которые столкнулись с проблемой обнаружения. Чтобы исправить это, обновите экземпляр Финансы и операции. Среда становится видимой с любым обновлением.
