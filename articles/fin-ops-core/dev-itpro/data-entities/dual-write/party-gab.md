...
Не все роли субъектов приложений Финансы и операции поддерживаются функцией двойной записи. Полный список ролей субъектов см. в разделе [Обзор глобальной адресной книги](../../../fin-ops/organization-administration/overview-global-address-book.md).
...
Электронные адреса доступны только в этой сетке. В будущих выпусках все поля электронных и почтовых адресов будут удалены из других вкладок, например со вкладок **Сводка** и **Сведения**. Контактная информация, отображенная на вкладке **Сведения** — это копии "только для чтения" основного электронного адреса, например основного телефона, основного адреса электронной почты, основного телефона, основного факса и основного идентификатора Twitter. В процессе квалификации интереса можно указать номер рабочего телефона и номер мобильного телефона. Номер рабочего телефона считается основным, если **IsMobile=No**, а номер мобильного телефона считается вторичным, если **IsMobile=Yes**.

> [!TIP]
> Используйте вкладки **Адреса** и **Электронные адреса** в формах **Организация** и **Контакт** для управления почтовым адресом и адресом электронной почты. Это гарантирует, что данные адреса будут синхронизированы с приложениями Финансы и операции.

## <a name="setup"></a>Настройка

1. Откройте среду приложений для взаимодействия с клиентами.

2. Установите последнюю версию (2.2.2.60 или более позднюю) [решения оркестрации управления приложения двойной записи](https://aka.ms/dual-write-app).

3. Установка [решений субъекта и глобальной адресной книги с двойной записью](https://aka.ms/dual-write-gab).
...
5. Примените оба решения, установленные в шагах 2 и 3, используя функцию [Применить решение](link-your-environment.md).
...
11. Если вы являетесь существующим пользователем с двойной записью, выполните инструкции в [Обновление модели субъекта и глобальной адресной книги](upgrade-party-gab.md), а затем обновите свои данные.

12. Выполните сопоставления в следующем порядке. Если возникнет ошибка "Сбой проверки проекта. Отсутствует поле назначения…", откройте сопоставление и выберите **Обновить таблицы**. Затем запустите сопоставление.

    Приложение Финансы и операции | Приложение для взаимодействия с клиентами  
    ----------------------------|------------------------
...
+ В приложениях Финансы и операции при создании клиента вместе с адресом и сохранении его адрес может не синхронизироваться с таблицей **Адрес**. Это обусловлено проблемой последовательностей платформ с двойной записью. Чтобы избежать этого, сначала создайте клиента и сохраните его. Затем добавьте адрес.
+ В приложениях Финансы и операции, когда у записи клиента есть основной адрес и создается новый контакт для этого клиента, запись контакта наследует основной адрес из связанной записи клиента. Это происходит также и для контакта поставщика. Dataverse в настоящее время не поддерживает такое поведение. Если включен режим двойной записи, контакты клиента, наследуемые с использованием основного адреса из приложения Финансы и операции, синхронизируются с Dataverse с его адресом.
...
+ В приложениях Финансы и операции можно создать запись о контакте в форме **Добавить контакт**. При попытке создать новый контакт из формы **Просмотр контакта** действие завершается ошибкой. Это известная проблема.
...
+ Вы не можете почтовый адрес с будущим числом с помощью приложения Финансы и операции с двойной записью, поскольку Dataverse не поддерживает дату. При вводе почтового адреса, датированного будущим числом, при помощи приложения Финансы и операции оно будет полностью синхронизировано с Dataverse, и адрес в интерфейсе пользователя сразу появится на экране. Все обновления этой записи приведут к ошибке, поскольку она задана на будущее, а не на актуальный день в приложении Финансы и операции.
