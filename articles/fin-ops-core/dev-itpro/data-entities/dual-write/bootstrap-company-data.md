...
---
# <a name="bootstrap-with-company-data-faq"></a><span data-ttu-id="55210-103">Часто задаваемые вопросы о начальной загрузки данных организации</span><span class="sxs-lookup"><span data-stu-id="55210-103">Bootstrap with company data FAQ</span></span>
 
[!include [banner](../../includes/banner.md)]

[!include [preview-banner](../../includes/preview-banner.md)]

## <a name="why-do-i-need-bootstrapping"></a><span data-ttu-id="55210-104">Зачем нужна начальная загрузка?</span><span class="sxs-lookup"><span data-stu-id="55210-104">Why do I need bootstrapping?</span></span> 
<span data-ttu-id="55210-105">Возможно, имеется существующий Common Data Service или другой экземпляр приложения Dynamics 365 с бизнес-данными, и вы хотите включить подключение с двойной записью.</span><span class="sxs-lookup"><span data-stu-id="55210-105">You might have an existing Common Data Service or other Dynamics 365 app instance with business data, and you want to enable dual-write connection against it.</span></span> <span data-ttu-id="55210-106">В этом случае необходимо выполнить начальную загрузку данных Common Data Service или приложения Dynamics 365 с данными о компании, прежде чем включить подключение двойной записи.</span><span class="sxs-lookup"><span data-stu-id="55210-106">In this case, you need to bootstrap Common Data Service or other Dynamics 365 app data with company information before enabling dual-write connection.</span></span>  
 
## <a name="when-should-i-use-bootstrapping"></a><span data-ttu-id="55210-107">Когда следует использовать начальную загрузку?</span><span class="sxs-lookup"><span data-stu-id="55210-107">When should I use bootstrapping?</span></span> 
<span data-ttu-id="55210-108">Начальную загрузку следует использовать перед включением сопоставления объектов с двойной записью (на шаге 5).</span><span class="sxs-lookup"><span data-stu-id="55210-108">You should use bootstrapping before enabling dual-write entity maps (during step #5).</span></span>  
1. <span data-ttu-id="55210-109">Чтобы настроить подключение с двойной записью между экземплярами приложения Finance and Operations и Common Data Service или приложением Dynamics 365, выполните вход в приложение Finance and Operations в качестве администратора.</span><span class="sxs-lookup"><span data-stu-id="55210-109">To setup the dual-write connection between instances of your Finance and Operations app and the Common Data Service or other Dynamics 365 app, log in to the Finance and Operations app as an administrator.</span></span> 
2. <span data-ttu-id="55210-110">Перейдите к модулю **Управление данными** и нажмите кнопку **Двойная запись**.</span><span class="sxs-lookup"><span data-stu-id="55210-110">Go to the **Data Management** module and click the **Dual-Write** button.</span></span> <span data-ttu-id="55210-111">При этом будет запущен **Интегратор данных**.</span><span class="sxs-lookup"><span data-stu-id="55210-111">This launches the **Data Integrator**.</span></span> 
3. <span data-ttu-id="55210-112">Создайте подключение с двойной записью для одной или нескольких компаний.</span><span class="sxs-lookup"><span data-stu-id="55210-112">Create the dual-write connection for one or more companies.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="55210-113">![Создание подключения с двойной записью](media/dual-write-boot-1.png)</span><span class="sxs-lookup"><span data-stu-id="55210-113">![Create dual-write connection](media/dual-write-boot-1.png)</span></span>
4. <span data-ttu-id="55210-114">Включите сопоставление объекта **Cdm_companies**.</span><span class="sxs-lookup"><span data-stu-id="55210-114">Enable the **Cdm_companies** entity map.</span></span> <span data-ttu-id="55210-115">Выполняется синхронизация компаний из приложения Finance and Operations в Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="55210-115">This synchronizes companies from the Finance and Operations app to Common Data Service.</span></span>  
    > [!div class="mx-imgBorder"]
    > <span data-ttu-id="55210-116">![Включение сопоставления объектов](media/dual-write-boot-2.png)</span><span class="sxs-lookup"><span data-stu-id="55210-116">![Enable the entity map](media/dual-write-boot-2.png)</span></span>
5. <span data-ttu-id="55210-117">Выполните код начальной загрузки примера в Common Data Service или другом экземпляре приложения Dynamics 365.</span><span class="sxs-lookup"><span data-stu-id="55210-117">Run the sample bootstrapping code on the Common Data Service or other Dynamics 365 app instance.</span></span>  
6. <span data-ttu-id="55210-118">Когда начальная загрузка завершена и система готова к реальной синхронизации, включите сопоставления объектов.</span><span class="sxs-lookup"><span data-stu-id="55210-118">When the bootstrapping is done and the system is ready for the live sync, enable the entity maps.</span></span>  

    <span data-ttu-id="55210-119">Включение сопоставления объектов инициирует начальную синхронизацию данных для включенных сопоставлений объектов.</span><span class="sxs-lookup"><span data-stu-id="55210-119">Enabling the entity maps triggers the initial data sync for the enabled entity maps.</span></span> <span data-ttu-id="55210-120">Данные, относящиеся к компаниям, выбранным для подключения с двойной записью, синхронизируются между приложением Finance and Operations и Common Data Service.</span><span class="sxs-lookup"><span data-stu-id="55210-120">The data corresponding to the companies chosen on the dual-write connection is synchronized between the Finance and Operations app and Common Data Service.</span></span> 
 
## <a name="how-to-i-use-the-code-sample"></a><span data-ttu-id="55210-121">Как использовать образец кода?</span><span class="sxs-lookup"><span data-stu-id="55210-121">How to I use the code sample?</span></span>
<span data-ttu-id="55210-122">Пример кода — это приложение C#, в котором можно выполнить загрузку в Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="55210-122">The sample code is a C# application that you can load in Visual Studio.</span></span> <span data-ttu-id="55210-123">В нем используются зависимости пакета NuGet в Common Data Service SDK, которые можно обновлять с помощью стандартных средств Visual Studio.</span><span class="sxs-lookup"><span data-stu-id="55210-123">It takes NuGet package dependencies on the Common Data Service SDK, that you can refresh through standard Visual Studio tooling.</span></span> 

<span data-ttu-id="55210-124">После распаковки и открытия решения в Visual Studio и восстановления пакетов NuGet выполните поиск **TODO** в коде.</span><span class="sxs-lookup"><span data-stu-id="55210-124">After unzipping and opening the solution in Visual Studio and restoring the NuGet packages, search for **TODO** in the code.</span></span> <span data-ttu-id="55210-125">Каждое решение должно быть связано с тем, как необходимо отметить сведения о начальной загрузке компании с помощью **TODO** с образцом кода для канонической реализации.</span><span class="sxs-lookup"><span data-stu-id="55210-125">Each decision you need to make about how you want to bootstrap company information is noted by a **TODO**, with sample code for a canonical implementation.</span></span> 

<span data-ttu-id="55210-126">Пример кода содержит только один из нескольких способов классификации записей объектов по компании.</span><span class="sxs-lookup"><span data-stu-id="55210-126">The sample code shows only one of many ways you might categorize entity records by company.</span></span> <span data-ttu-id="55210-127">Изменяя логику в разделах **TODO**, можно создать собственную классификацию.</span><span class="sxs-lookup"><span data-stu-id="55210-127">By changing the logic in the **TODO** sections, you can create your custom categorization.</span></span> 
 
## <a name="what-should-i-expect"></a><span data-ttu-id="55210-128">Чего ожидать?</span><span class="sxs-lookup"><span data-stu-id="55210-128">What should I expect?</span></span>
<span data-ttu-id="55210-129">По умолчанию образец приложения позволяет предоставить словарь для сопоставлений кода "бизнес-единица — компания".</span><span class="sxs-lookup"><span data-stu-id="55210-129">By default, the sample application lets you provide a dictionary of business unit-to-company code mappings.</span></span> <span data-ttu-id="55210-130">Любая сущность с начальной загрузкой с полем **OwningBusinessUnit** автоматически будет настроена на использование указанной компании.</span><span class="sxs-lookup"><span data-stu-id="55210-130">Any entity you bootstrap with an **OwningBusinessUnit** field is automatically set to use the specified company.</span></span> <span data-ttu-id="55210-131">Любая сущность без поля **OwningBusinessUnit**, например продукт, будет определять компанию на основе сопоставления с пустым значением бизнес-единицы.</span><span class="sxs-lookup"><span data-stu-id="55210-131">Any entity without an **OwningBusinessUnit** field, such as product, will set the company based on the mapping with an empty business unit value.</span></span>

<span data-ttu-id="55210-132">Консольное приложение ожидает один параметр **–simulate** или **–apply**.</span><span class="sxs-lookup"><span data-stu-id="55210-132">The console application expects one parameter, either **–simulate** or **–apply**.</span></span> <span data-ttu-id="55210-133">Если используется параметр командной строки **–simulate**, то данные не обновляются.</span><span class="sxs-lookup"><span data-stu-id="55210-133">If you use the **–simulate** command line parameter, then no data is updated.</span></span> <span data-ttu-id="55210-134">Создаются только файлы **simulation_<entityname>.csv** в каталоге с инструментом, по одному для каждого объекта, который был бы обновлен.</span><span class="sxs-lookup"><span data-stu-id="55210-134">Only **simulation_<entityname>.csv** files are generated in the same directory as the tool, one for each entity that would have been updated.</span></span> <span data-ttu-id="55210-135">Можно итеративно просматривать эти файлы во время работы, чтобы гарантировать правильное обновление значений компаний со стороны кода.</span><span class="sxs-lookup"><span data-stu-id="55210-135">You can iteratively review these files while working to ensure the code updates company values as expected.</span></span> 

<span data-ttu-id="55210-136">После завершения имитации обновлений используйте параметр **–apply**.</span><span class="sxs-lookup"><span data-stu-id="55210-136">When you finish with the simulated updates, then use the **–apply** parameter.</span></span> <span data-ttu-id="55210-137">При этом обновляются все записи, которые в настоящее время имеют неправильное значение компании, в пакетах по 1000 записей за один раз (по умолчанию).</span><span class="sxs-lookup"><span data-stu-id="55210-137">This updates all records that currently have an incorrect company value, in batches of 1000 records at a time (by default).</span></span> <span data-ttu-id="55210-138">Код идемпотентный, что означает, что его можно запустить повторно, и будут обновлены только неправильно назначенные компании.</span><span class="sxs-lookup"><span data-stu-id="55210-138">The code is idempotent as provided, meaning you can re-run it and only the incorrectly assigned companies will be updated.</span></span> <span data-ttu-id="55210-139">При запуске с помощью **–apply** код выводит CSV-файлы изменений с именем **applied_<entityname>.csv**.</span><span class="sxs-lookup"><span data-stu-id="55210-139">When running with **–apply**, the code outputs CSV files of the changes made, which are named **applied_<entityname>.csv**.</span></span> 
