...
Представлены следующие три шаблона фабрики данных. Они помогают сопоставлять данные обоих приложений Finance and Operations и приложений взаимодействия с клиентами.
...
|---|---|
| FONewParty.csv | Этот файл помогает создавать новые записи **Субъект** в приложении Finance and Operations. |
| ImportFONewPostalAddressLocation.csv | Этот файл помогает создавать новые записи **Местоположение почтового адреса** в приложении Finance and Operations. |
| ImportFONewPartyPostalAddress.csv | Этот файл помогает создавать новые записи **Почтовый адрес субъекта** в приложении Finance and Operations. |
| ImportFONewPostalAddress.csv | Этот файл помогает создавать новые записи **Почтовый адрес** в приложении Finance and Operations. |
| ImportFONewElectronicAddress.csv | Этот файл помогает создавать новые записи **Электронный адрес** в приложении Finance and Operations. |
...
> [!IMPORTANT]
> При работе с шаблонами почтового адреса субъекта и электронных адресов субъекта имеются специальные инструкции. Сначала необходимо выполнить шаблон субъекта, затем шаблон почтового адреса субъекта, а затем шаблон электронного адреса субъекта.
...
+ **Моментальный снимок:** создание моментального снимка для приложений Finance and Operations и приложений для взаимодействия с клиентами. Затем при необходимости можно использовать эти моментальные снимки для восстановления предыдущего состояния.
...
1. В фабрике данных на вкладке **Управление** в области **Глобальные параметры** создайте следующие глобальные параметры.
    ...
    | Номер | Имя | Тип | Значение |
    |---|---|---|---|
    | 1 | IsFOSource | логический | Этот параметр определяет, какие основные системные адреса заменяются в случае конфликтов. Если значение **true**, основные адреса в приложениях Finance and Operations заменят основные адреса в приложениях для взаимодействия с клиентами. Если значение **false**, основные адреса в приложениях для взаимодействия с клиентами заменят основные адреса в приложениях Finance and Operations. |
    | 2 | ElectronicAddressIdPrefix | строка | Этот параметр добавляет серийный номер к вновь созданным электронным адресам в качестве префикса. Не забудьте предоставить строку, не конфликтующую с электронными адресами в приложениях Finance and Operations и приложениях для взаимодействия с клиентами. В этом примере будет использоваться **ADF-EAD-**. |
...
1. Остановите сопоставления двойной записи **Учетная запись**, **Контактное лицо** и **Поставщик**, которые используют приложение Finance and Operations:
...
4. В приложении Finance and Operations выполните **Начальная синхронизация** для следующих таблиц, если они содержат данные.
...
10. Чтобы обновить приложение Finance and Operations, используя эти данные, необходимо преобразовать CSV-файлы в рабочую книгу Excel и [импортировать их в приложение Finance and Operations](/data-entities/data-import-export-job). Если вам подходит импорт CSV, можно также напрямую импортировать файлы CSV. Этот шаг может занять несколько часов в зависимости от объема.
...
2. Шаги с 7-1 по 7-9 извлекают данные из приложения Finance and Operations и приложения для взаимодействия с клиентами и подготовить эти данные для обновления.
3. Шаги с 8 по 9 сравнивают номер субъекта для записей **Организация**, **Контакт** и **Поставщик** между приложением Finance and Operations и приложением для взаимодействия с клиентами. Любые записи, не имеющие номера субъекта, будут пропущены.
4. На шаге 10 создаются два CSV-файла для записей субъекта, которые должны быть созданы в приложении для взаимодействия с клиентами и в приложении Finance and Operations.
...
1. Шаги с 1-1 по 1-10 извлекают данные из приложения Finance and Operations и приложения для взаимодействия с клиентами и подготовить эти данные для обновления.
2. Шаг 2 отменяет нормализацию данных почтового адреса в приложении Finance and Operations путем объединения почтового адреса и почтового адреса субъекта.
...
4. На шаге 4 создаются CSV-файлы для приложения Finance and Operations, чтобы создать новые данные адресов на основе адресов организаций, контактов и поставщиков.
5. На шаге 5-1 создаются файлы с расширением .csv для приложения для взаимодействия с клиентами, чтобы создать все данные адресов на основе приложения Finance and Operations и приложения для взаимодействия с клиентами.
6. Шаг 5-2 преобразует файлы .csv в формат импорта Finance and Operations для ручного импорта.
...
1. Шаги с 1-1 по 1-5 извлекают данные из приложения Finance and Operations и приложения для взаимодействия с клиентами и подготовить эти данные для обновления.
...
3. На шаге 3 осуществляется слияние данных основного электронного адреса из приложения для взаимодействия с клиентами и приложения Finance and Operations.
4. На шаге 4 создаются файлы .csv.

    - Создайте новые данные электронного адреса для приложения Finance and Operations на основе адресов организаций, контактов и поставщиков.
    - Создайте новые данные электронного адреса для приложения для взаимодействия с клиентами на основе электронного адреса, адресов организации, контакта и поставщика в приложении Finance and Operations.
