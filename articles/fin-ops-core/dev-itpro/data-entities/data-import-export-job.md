...
## <a name="job-history-clean-up-and-archival-available-for-preview-in-platform-update-39-or-version-10015"></a>Очистка и архивация журналов заданий (доступно для предварительной версии в обновлении платформы 39 или версии 10.0.15)
...
Одним из основных изменений функций очистки является использование системных пакетных заданий для очистки истории. Использование системного пакетного задания позволяет приложениям Finance and Operations иметь автоматическое планирование и выполнение пакетного задания очистки, как только система будет готова к работе. Больше не требуется планировать пакетное задание вручную. В этом режиме выполнения по умолчанию пакетное задание будет выполняться каждый час, начиная с 12 по полуночи, и сохраняет историю выполнения за последние 7 дней. Очищенный журнал архивируется для дальнейшего извлечения.

> [!NOTE]
> Поскольку эта функция доступна как предварительная версия, системное пакетное задание не удаляет журнал выполнения до тех пор, пока не будет включено через предоставление доступа DMFEnableExecutionHistoryCleanupSystemJob. Когда функция станет общедоступной в будущем выпуске, это предоставление доступа не потребуется, а системное пакетное задание начнет удалять и архивировать после готовности системы на основе определенного графика, как описано выше. 

> [!NOTE]
> В будущем выпуске предыдущие версии функций очистки будут удалены из приложений Finance and Operations.
