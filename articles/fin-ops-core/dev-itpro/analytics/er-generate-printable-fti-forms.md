...
description: В этой статье объясняется, как использовать структуру электронной отчетности (ER) для создания печатных форм накладной с произвольным текстом (FTI) в виде документов Microsoft Office.
author: kfend
ms.date: 07/24/2018
ms.topic: article
ms.prod: ''
...
ms.locfileid: "9274090"
---
# <a name="generate-printable-fti-forms"></a>Создание печатных форм FTI

[!include[banner](../includes/banner.md)]

Инфраструктура электронной отчетности (ER) позволяет создавать печатные формы накладной с произвольным текстом (FTI) в виде документов Microsoft Office. В этой статье представлена информация о том, как создавать собственные конфигурации, а также сведения о доступных шаблонах конфигурации.
...
> [!NOTE]
> Если вы хотите начать с обзора существующих конфигураций электронной отчетности для данного примера решения печатных форм FTI, вы можете перейти непосредственно к разделу **Загрузка образцов конфигураций электронной отчетности для создания печатных форм FTI** далее в этой статье.
...
Ваше приложение должно включать конфигурацию модели данных электронной отчетности, содержащую модель данных, которая описывает бизнес-среду выставления накладной клиента. Имеется требование, что имя модели данных должно быть **CustomersInvoicing**. Сведения о создании моделей данных ER см. в разделе [Электронная отчетность — Разработка модели данных для конкретного домена](tasks/er-design-domain-specific-data-model-2016-11.md).
...
Дополнительные сведения о создании сопоставлений моделей ER см. в разделе [Определение сопоставлений моделей электронной отчетности и выбор источников данных для них](tasks/er-define-model-mapping-select-data-sources-2016-11.md).
...
Сведения о настройке форматов электронной отчетности см. в разделе [Создание конфигурации формата для ER (ноябрь 2016 г.)](tasks/er-format-configuration-2016-11.md). Сведения о разработке форматов электронной отчетности для создания отчетов в формате OpenXML см. в разделе [Разработка конфигурации для создания отчетов в формате OpenXML для ER (ноябрь 2016 г.)](tasks/er-design-reports-openxml-2016-11.md).
...
При использовании форматов электронной отчетности для печати форм FTI таким образом используется файлы назначения электронной отчетности по умолчанию. Место назначения изменить невозможно. Дополнительные сведения о настройке мест назначения электронной отчетности для форматов электронной отчетности см. в разделе [Места назначения электронной отчетности (ER)](electronic-reporting-destinations.md).
...
- **Экран** — Microsoft 365 Excel используется для предварительного просмотра созданных форм FTI в формате Excel.
...
## <a name="download-sample-er-configurations-to-generate-printable-fti-forms"></a>Загрузка образцов конфигураций электронной отчетности для создания печатных форм FTI
...
> [!NOTE]
> Необходимо сначала установить шрифт, который используется для создания штрихкода на локальном компьютере, где созданная форма накладной в формате Excel будет просматриваться.
...
![Отправить.](media/FTIbyGER-Email.PNG)
