...
Сопоставления модели электронной отчетности (ER) можно настроить с реализацией универсальной модели данных ER, но имеют с учетом Dynamics 365 Finance. В этой теме объясняется, как разработать несколько сопоставлений модели ER для модели данных ER, чтобы управлять их использованием соответствующими форматами ER, которые выполняются в компаниях с различными контекстами стран или регионов.
...
Чтобы разработать необходимые компоненты ER, следуйте указаниям в [приложении 2](#appendix2) данного раздела. Теперь имеются конфигурации сопоставления модели **Сопоставление (общее)** и **Настраиваемое сопоставление (общее)**, каждая из которых содержит сопоставление модели для определения **Точка входа 1**.
...
Чтобы разработать необходимые компоненты ER, следуйте указаниям в [приложении 3](#appendix3) данного раздела. Теперь имеются конфигурации сопоставления модели **Сопоставление (общее)**, **Настраиваемое сопоставление (общее)** и **Сопоставление (FR)**, которые содержат сопоставление модели для определения **Точка входа 1**.
...
- Определение сопоставления модели, которое использует формат ER, указано (**Точка входа 1** в примерах данного раздела).
- Все конфигурации сопоставления, содержащие сопоставление с указанным определением и удовлетворяющие всем настраиваемым ограничениям контекста страны или региона, потенциально могут использоваться для запуска формата ER (**Сопоставление (общее)**, **Настраиваемое сопоставление (общее)** и **Сопоставление (FR)** в примерах данного раздела).
- Любое сопоставление модели по умолчанию с ограничениями контекста страны или региона имеет наивысший приоритет для выбора (**Сопоставление (FR)** в примерах данного раздела).
- Любое сопоставление модели по умолчанию без ограничений контекста страны или региона имеет более высокий приоритет для выбора (**Настраиваемое сопоставление (общее)** в примерах данного раздела).
...
## <a name="appendix1"></a>Приложение №1
...
## <a name="appendix2"></a>Приложение №2
...
## <a name="appendix3"></a>Приложение №3
