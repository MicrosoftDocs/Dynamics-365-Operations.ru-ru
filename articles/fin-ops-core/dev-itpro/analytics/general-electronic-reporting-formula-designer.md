...
### <a name="documents-content-control"></a>Управление содержимым документов

Конструктор формул электронной отчетности можно использовать для настройки выражений, определяющих, какие данные будут помещены в создаваемые электронные документы во время выполнения. Выражения могут включать или отключать вывод конкретных элементов формата, в зависимости от обрабатываемых данных и настроенной логики. Это выражение может быть введено для одного элемента форматирования в поле **Включено** на вкладке **Сопоставление** на странице **Конструктор операций** в качестве логического условия, возвращающего логическое значение **Boolean**:

-   Если возвращается значение **True**, выполняется текущий элемент форматирования.
-   Если возвращается значение **False**, текущий элемент форматирования пропускается.

На следующем рисунке показаны выражения этого типа (версия **11.12.11** конфигурации формата **ISO20022 Перенос кредита (NO)**, предоставленная Майкрософт, является примером). Компонент формата **XMLHeader** настроен для описания структуры сообщения о переносе кредита, следуя стандартам XML-сообщений ISO 20022. Компонент формата **XMLHeader/Document/CstmrCdtTrfInitn/PmtInf/CdtTrfTxInf/RmtInf/Ustrd** сконфигурирован для добавления к созданному сообщению, элементу XML **Ustrd**, и размещения информации о предъявлении к оплате в неструктурированный формат в виде текста следующих элементов XML:

-   Компонент **PaymentNotes** используется для вывода текста платежных примечаний.
-   Компонент **DelimitedSequence** выводит номера накладных с разделителями-запятыми, которые используются для сопоставления текущего переноса кредита.

[![Конструктор операций](./media/GER-FormulaEditor-ControlContent-1.png)](./media/GER-FormulaEditor-ControlContent-1.png)

> [!NOTE]
> Компоненты **PaymentNotes** и **DelimitedSequence** обозначены с использованием вопросительного знака. Это означает, что использование обоих компонентов является условным, в зависимости от следующих критериев:

-   Определенное для компонента **PaymentNote** выражение **@.PaymentsNotes<>""** разрешает (возвращая **TRUE**) заполнение XML-элемента **Ustrd**, тестом примечаний платежа, когда этот текст для текущего переноса кредита не пуст.

[![Конструктор операций](./media/GER-FormulaEditor-ControlContent-2.png)](./media/GER-FormulaEditor-ControlContent-2.png)

-   Определенное для компонента **DelimitedSequence**, выражение **@.PaymentsNotes=""** разрешает (возвращая **TRUE**) заполнение XML-элемента **Ustrd** разделенными запятыми номерами накладных, которые используются для сопоставления текущего переноса кредита, когда текст примечаний платежа для этого переноса кредита пуст.

[![Конструктор операций](./media/GER-FormulaEditor-ControlContent-3.png)](./media/GER-FormulaEditor-ControlContent-3.png)
...
При добавлении источника данных **System** в сопоставление ER, которое относится к классу **Global** приложения, выражение возвращает логическое значение **FALSE**. Измененное выражение **System.' isLanguageRTL'("AR")** возвращает логическое значение **TRUE**.
...
| NOW () | Возврат текущей даты и времени сервера приложений в качестве значения даты/времени. | |
| TODAY () | Возврат текущей даты сервера приложений в качестве значения даты. | |
| NULLDATE () | Возврат значения даты **null**. | |
| NULLDATETIME () | Возврат значение даты/времени **null**. | |
| DATETIMEFORMAT (дата и время, формат) | Преобразование указанного значения даты/времени в строку в указанном формате. (Сведения о поддерживаемых форматах см. разделы [стандартный](https://msdn.microsoft.com/library/az4se3k1(v=vs.110).aspx) и [настраиваемый](https://msdn.microsoft.com/library/8kb3ddd4(v=vs.110).aspx).) | **DATETIMEFORMAT (NOW(), "дд-ММ-гггг")** возвращает текущую дату сервера приложений, 24 декабря 2015, как **"24-12-2015"**, на основе указанного настраиваемого формата. |
| DATETIMEFORMAT (дата и время, формат, культура) | Преобразование указанного значения даты/времени в строку в указанном формате и [культуре](https://msdn.microsoft.com/goglobal/bb896001.aspx). (Сведения о поддерживаемых форматах см. разделы [стандартный](https://msdn.microsoft.com/library/az4se3k1(v=vs.110).aspx) и [настраиваемый](https://msdn.microsoft.com/library/8kb3ddd4(v=vs.110).aspx).) | **DATETIMEFORMAT (NOW(), "d", "de")** возвращает текущую дату сервера приложений, 24 декабря 2015, как **"24.12.2015"**, на основе выбранного немецкого языкового стандарта. |
| SESSIONTODAY () | Возврат текущей даты сессии приложений в качестве значения даты. | |
| SESSIONNOW () | Возврат текущей даты и времени сессии приложений в качестве значения даты/времени. | |
| DATEFORMAT (дата, формат) | Возвращает строковое представление указанной даты в указанном формате. | **DATEFORMAT (SESSIONTODAY (), "dd-MM-yyyy")** возвращает дату текущей сессии приложения, 24 декабря 2015, как **"24-12-2015"**, на основе указанного настраиваемого формата. |
| DATEFORMAT (дата, формат, культура) | Преобразование указанного значения даты в строку в указанном формате и [культуре](https://msdn.microsoft.com/goglobal/bb896001.aspx). (Сведения о поддерживаемых форматах см. разделы [стандартный](https://msdn.microsoft.com/library/az4se3k1(v=vs.110).aspx) и [настраиваемый](https://msdn.microsoft.com/library/8kb3ddd4(v=vs.110).aspx).) | **DATETIMEFORMAT (SESSIONNOW (), "d", "de")** возвращает дату текущей сессии приложения, 24 декабря 2015, как **"24.12.2015"**, на основе выбранного немецкого языкового стандарта. |
...
<tr>
<td>SPLIT (ввод, разделитель)</td>
<td>Разделение указанной строки ввода на подстроки на основе указанного разделителя.</td>
<td><strong>SPLIT (&quot;XAb aBy&quot;, &quot;aB&quot;)</strong> возвращает новый список, который состоит из трех записей, которые имеют поле <strong>STRING</strong>. Поле в первой записи содержит текст <strong>&quot;X&quot;</strong>, поле во второй записи содержит текст &quot;&nbsp;&quot;, и поле в третьей записи содержит текст <strong>&quot;y&quot;</strong>. Если разделитель пуст, новый список возвращается, который состоит из одной записи с полем <strong>STRING</strong>, содержащим введенный текст. Если входные данные пусты, возвращается новый пустой список.
Если ввод или разделитель не указаны (null), возникает исключение приложения.</td>
</tr>
<tr>
...
<blockquote>[!NOTE] На основе параметров языка родительских элементов формата FILE и FOLDER переведенный текст для меток и описаний вводится в выходные данные формата электронной отчетности.</blockquote>
</td>
</tr>
<tr>
...
<p>В этом случае можно использовать следующее выражение для получения метки значения перечисления на немецком языке (Швейцария), если этот перевод доступен. Если перевод со швейцарского на немецкий не поддерживается, подпись появляется на немецком языке.</p>
...
<td>Если <strong>Поставщик</strong> настраивается в качестве источника данных ER, который ссылается на таблицу VendTable, <strong>FILTER(Vendors, Vendors.VendGroup = &quot;40&quot;)</strong> возвращает список только поставщиков, которые относятся к группе поставщиков 40. Если <strong>Поставщик</strong> настроен в качестве источника данных электронной отчетности, который ссылается на таблицу VendTable и если <strong>parmVendorBankGroup</strong> настроен как источник данных электронной отчетности, который возвращает значение строкового типа данных <strong>String</strong>, <strong>FILTER (Vendor.'&lt;Relations'.VendBankAccount, Vendor.'&lt;Relations'.VendBankAccount.BankGroupID = parmVendorBankGroup)</strong> возвращает список только счетов поставщиков, входящих в конкретную банковскую группу.</td>
</tr>
<tr>
<td>NDEX (список, индекс)</td>
<td>Эта функция возвращает запись, выбранную с помощью определенного числового индекса в списке. Если индекс выходит за пределы диапазона записей в списке, создается исключение.</td>
<td>Если введен источник данных <strong>DS</strong> для типа <strong>Вычисляемое поле</strong> и он содержит выражение <strong>SPLIT ("A|B|C", “|”), 2</strong>, выражение <strong>DS.Value</strong> возвращает текстовое значение, "B". Выражение <strong>INDEX (SPLIT ("A|B|C", “|”), 2).Value</strong> также возвращает текстовое значение “B”.</td>
</tr>
</tbody>
</table>
...
| VALUEIN (ввод, список, выражение элемента списка) | Определяет, соответствует ли заданный ввод какому-либо значению элемента в указанном списке. Возвращает значение **TRUE,**, если заданный ввод соответствует результату выполнения указанного выражения по крайней мере для одной записи. В противном случае возврат **FALSE**. Параметр **ввод** представляет путь к элементу источника данных. Значение этого элемента будет сопоставляться. Параметр **список** представляет путь к элементу источника данных типа списка записей как список записей, содержащий выражение. Значение этого элемента будет сравниваться с заданным вводом. Аргумент **выражение элемента списка** представляет выражение, которое либо указывает на, либо содержит одно поле указанного списка, которое должно использоваться для сопоставления. | Например, см. раздел [Примеры: VALUEIN (ввод, список, выражение элемента списка)](#examples-valuein-input-list-list-item-expression), как указано ниже. |

#### <a name="examples-valuein-input-list-list-item-expression"></a>Пример: VALUEIN (ввод, список, выражение элемента списка)
Как правило, функция **VALUEIN** переводится в набор условий **OR**:

(ввод = list.item1.value) OR (ввод = list.item2.value) OR …

##### <a name="example-1"></a>Пример 1
Вы определяете следующий источник данных в вашей модели сопоставления: **Список** (тип **Рассчитываемое поле**). Этот источник данных содержит выражение **SPLIT ("a,b,c", ",")**.

При вызове источника данных, который настроен как выражение **VALUEIN ("B", List, List.Value)**, он возвращает **TRUE**. В этом случае функция **VALUEIN** переводится в следующий набор условий:

**(("B" = "a") или ("B" = "b") или ("B" = "c"))**, где **("B" = "b")** равно значению **TRUE**

При вызове источника данных, который настроен как выражение **VALUEIN ("B", List, LEFT(List.Value, 0))**, он возвращает **FALSE**. В этом случае функция **VALUEIN** переводится в следующее условие:

**("B" = "")**, которое не равно **TRUE**

Обратите внимание, что верхний предел для числа символов в тексте таких условий составляет 32 768 знаков. Таким образом, не следует создавать источники данных, которые могут превысить этот предел во время выполнения. Если предел превышен, приложение перестанет работать, и будет создано исключение. Например, такая ситуация возможна, если источник данных настроен как **WHERE (List1, VALUEIN (List1.ID, List2, List2.ID)** и списки **List1** и **List2** содержат большой объем записей.
...
- Элемент списка функции **VALUEIN** ссылается на поле (не выражения или метод) указанного источника данных.
...
- **In** (тип **Записи таблицы**), который ссылается на таблицу Intrastat
- **Port** (тип **Записи таблицы**), который ссылается на таблицу IntrastatPort
...
- **Le** (тип **Вычисляемое поле**), которое содержит выражение **SPLIT ("DEMF,GBSI,USMF", ",")**
- **In** (тип **Записи таблицы**), которой ссылается на таблицу Интрастат и для которого параметр **Межфирменные** включен
...
| Функция | описание | Пример |
|----------|-------------|---------|
| NULLCONTAINER (список) | Возвращает запись **NULL** с той же структурой, как указанный список записей или запись.<blockquote>[!NOTE] Эта функция является устаревшей. Используйте вместо **EMPTYRECORD**.</blockquote> | **NULLCONTAINER (SPLIT ("abc", 1))** возвращает новый пустой запись, который имеет такую же структуру, как список, который возвращен функцией **SPLIT**. |
| EMPTYRECORD (запись) | Возвращает запись **NULL** с той же структурой, как указанный список записей или запись.<blockquote>[!NOTE] Запись **null** является записью, в которой все поля имеют пустое значение. Пустое значения равно **0** (ноль) для чисел, пустой строке для строк и т. д.</blockquote> | **EMPTYRECORD (SPLIT ("abc", 1))** возвращает новый пустой запись, который имеет такую же структуру, как список, который возвращен функцией **SPLIT**. |
...
<blockquote>[!NOTE] Строка, возвращаемая этой функцией, зависит от кодировки, выбранной в родительском элементе формата FILE. Список поддерживаемых кодировок см. в разделе <a href="https://msdn.microsoft.com/en-us/library/system.text.encoding(v=vs.110).aspx">Класс Encoding</a>.</blockquote>
</td>
</tr>
<tr>
...
<blockquote>[!NOTE] Выражение <strong>&quot;abc&quot; &amp; &quot;def&quot;</strong> также возвращает <strong>&quot;abcdef&quot;</strong>.</blockquote>
</td>
</tr>
<tr>
...
<td>Когда определенный параметр <strong>флаг регулярного выражения</strong> имеет значение <strong>true</strong>, возвращает указанную строку после ее изменения путем применения регулярного выражения, которое определяется как аргумент <strong>шаблон</strong> для этой функции. Это выражение используется для обнаружения символов, которые необходимо заменить. Символы определенного аргумента <strong>замена</strong> используются для замены найденных символов. Когда определенный параметр <strong>флаг регулярного выражения</strong> имеет значение <strong>false</strong>, эта функция ведет как <strong>TRANSLATE</strong>.</td>
...
<tr>
<td>TEXT (ввод)</td>
<td>Возврат указанного ввода после его преобразования в текстовую строку, которая отформатирована в соответствии с параметрами языкового стандарта сервера текущего экземпляра приложения. Для значений типа <strong>real</strong> преобразование строки ограничено до 2 десятичных знаков.</td>
<td>Если языковой стандарт экземпляра сервера Finance and Operations определен как <strong>EN-US</strong>, <strong>TEXT (NOW ())</strong> возвращает текущую дату сеанса приложения, 17 декабря 2015, как текстовую строку <strong>&quot;12/17/2015 07:59:23 AM&quot;</strong>. <strong>TEXT (1/3)</strong> возвращает <strong>&quot;0.33&quot;</strong>.</td>
</tr>
<tr>
...
<p>Если отчет обрабатывается для клиента <strong>Litware Retail</strong> 17 декабря 2015 г., в культуре <strong>EN-US</strong> и языке <strong>EN-US</strong>, эта формула возвращает следующий текст, который можно представить для пользователя в виде сообщения исключения:</p>
...
<blockquote>[!NOTE] Следующий синтаксис применяется в формулах ER для меток:
<ul>
<li><strong>Для меток из ресурсов приложений Finance and Operations:</strong> <strong>@&quot;X&quot;</strong>, где <strong>Х</strong> — идентификатор метки в репозитории прикладных объектов (AOT)</li>
<li><strong>Для меток, которые находятся в конфигурациях:</strong> <strong>@&quot;GER_LABEL:X&quot;</strong>, где <strong>Х</strong> — код метки в конфигурации ER</li>
</ul>
</blockquote>
</td>
</tr>
...
<blockquote>[!NOTE] Параметры <strong>флаг печати названия валюты</strong> и <strong>десятичные знаки</strong> анализируются только для следующих кодов языков:<strong>CS</strong>, <strong>ET</strong>, <strong>HU</strong>, <strong>LT</strong>, <strong>LV</strong>, <strong>PL</strong>и <strong>RU</strong>. Кроме того, <strong>флаг печати названия валюты</strong> анализируется только для компаний, в которых контекст страны или региона поддерживает склонение названий валюты.</blockquote>
</td>
...
<td>Преобразование заданного ввода из типа данных <strong>Строка</strong> в элемент данных из тип данных <strong>GUID</strong>.<blockquote>[!NOTE] Чтобы выполнить преобразование в обратном направлении (то есть, для преобразования указанного ввода с типом данных <strong>GUID</strong> в элемент данных с типом данных <strong>Строка</strong>), можно использовать функцию <strong>TEXT()</strong>.</blockquote></td>
<td>Определите следующие источники данных в соответствии вашей модели:
<ul>
...
| SUMIF (ключевая строка для суммирования, строка диапазона критериев, строка значения критериев) | Возвращает сумму значений, которые были собраны для XML-узлов (где имя определено как ключ) во время выполнения формата и который удовлетворяет указанному условию (диапазон и значение). Возвращает значение **0** (ноль), когда флаг **Сбор сведений о результате** для текущих файлов выключен. | |
...
| COUNTIF (строка диапазона критериев, строка значения критериев) | Возвращает количество XML-узлов, которые были собраны во время выполнения формата и которые удовлетворяют указанному условию (диапазон и значение). Возвращает значение **0** (ноль), когда флаг **Сбор сведений о результате** для текущих файлов выключен. | |
| COLLECTEDLIST (ключевая строка для суммирования, строка критериев range1, строка критериев value1 \[, строка критериев range2, строка критериев value2,…\]) | Возвращает список значений, которые были собраны для XML-узлов во время выполнения формата и которые удовлетворяют указанным условиям (диапазон и значение). Возвращает пустой список, когда флаг **Сбор сведений о результате** для текущих файлов выключен. | |

### <a name="other-business-domainspecific-functions"></a>Другие функции (характерные для конкретных бизнес-доменов)

| Функция | описание | Пример |
|----------|-------------|---------|
| CONVERTCURRENCY (сумма, валюта источника, валюта целевая, дата, компания) | Преобразование указанной денежной суммы от указанной исходной валюты в указанную целевую валюту, используя настройки определенной компании на указанную дату. | **CONVERTCURRENCY (1, "EUR", "USD", TODAY(), "DEMF")** возвращает эквивалент одного евро в долларах США на текущую дату сеанса на основе настроек для компании DEMF. |
| ROUNDAMOUNT (число, десятичные знаки, правило округления) | Округление указанной суммы до указанного количества десятичных знаков в соответствии с указанным правилом округления.<blockquote>[!NOTE] Правило округления необходимо определить как значение перечисления **RoundOffType**.</blockquote> | Если параметр **model.RoundOff** задан как **Downward**, **ROUNDAMOUNT (1000.787, 2, model.RoundOff)** возвращает значение **1000.78**. Если параметр **model.RoundOff** задан как **Normal** или **Rounding-up**, **ROUNDAMOUNT (1000.787, 2, model.RoundOff)** возвращает значение **1000.79**. |
...
| ISOCredRef (цифры) | Возврат ссылки кредитора международной организации по стандартизации (ISO) на основе цифр и алфавитных символов определенного номера накладной.<blockquote>[!NOTE] Чтобы исключить символы из алфавитов, не совместимых с ISO, входной параметр необходимо перевести до передачи к этой функции.</blockquote> | **ISOCredRef ("VEND-200002")** возвращает **"RF23VEND-200002"**. |
...
| GetCurrentCompany () | Возвращает текстовое представление кода для юридического лица (компании), в которую выполнил вход пользователь. | **GETCURRENTCOMPANY ()** возвращает **USMF** для пользователя, выполнившего вход в компанию **Contoso Entertainment System USA**. |
...
| NUMSEQVALUE (код номерной серии, область, код области) | Возвращает новое сформированное значение номерной серии на основе указанного кода номерной серии, области и кода области. Область должна быть указана как значение перечисления **ERExpressionNumberSequenceScopeType** (**Общие**, **Юридическое лицо** или **Компания**). Для области **Общие** укажите пустую строку как код области. Для областей **Компания** и **Юридическое лицо** укажите код компании как код области. Для областей **Компания** и **Юридическое лицо** если указать пустую строку в качестве кода области, используется код текущей компании. | Определите следующие источники данных в соответствии вашей модели:<ul><li>**enumScope** (тип **Перечисление Dynamics 365 for Operations**), который ссылается на перечисление **ERExpressionNumberSequenceScopeType**</li><li>**NumSeq** (тип **Вычисляемое поле**), который содержит выражение **NUMSEQVALUE ("Gene\_1", enumScope.Company, "")**</li></ul>При вызове источника данных **NumSeq** он возвращает новое сформированное значение номерной серии **Gene\_1**, которая была настроена для компании, предоставляющая контекст, в котором выполняется формат электронной отчетности. |
| NUMSEQVALUE (код номерной серии) | Возвращает новое сформированное значение номерной серии на основе указанной номерной серии, области **Компания** и (как код области) код компании, предоставляющий контекст, в котором выполняется формат электронной отчетности. | Вы определяете следующий источник данных в вашей модели сопоставления: **NumSeq** (тип **Вычисляемое поле**). Этот источник данных содержит выражение **NUMSEQVALUE ("Gene\_1")**. При вызове источника данных **NumSeq** он возвращает новое сформированное значение номерной серии **Gene\_1**, которая была настроена для компании, предоставляющая контекст, в котором выполняется формат электронной отчетности. |
| NUMSEQVALUE (код записи номерной серии) | Возвращает новое сформированное значение номерной серии на основе указанного кода записи номерной серии. | Определите следующие источники данных в соответствии вашей модели:<ul><li>**LedgerParms** (тип **Таблица**), который ссылается на таблицу LedgerParameters</li><li>**NumSeq** (тип **Вычисляемое поле**), который содержит выражение **NUMSEQVALUE (LedgerParameters.'numRefJournalNum()'.NumberSequenceId)**</li></ul>При вызове источника данных **NumSeq** он возвращает новое сформированное значение номерной серии, которая была настроена в параметрах главной книги для компании, предоставляющая контекст, в котором выполняется формат электронной отчетности. Эта номерная серия уникальным образом идентифицирует журналы и выступает в качестве номера партии, который связывает проводки друг с другом. |
