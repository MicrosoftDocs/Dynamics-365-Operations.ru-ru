...
description: В этой статье объясняется, как можно отлаживать источники данных выполняемого формата электронной отчетности, чтобы лучше понимать настроенные поток и преобразование данных.
author: kfend
ms.date: 04/22/2020
ms.topic: article
ms.prod: ''
...
В процессе [настройки](tasks/er-format-configuration-2016-11.md) решения электронной отчетности (ER) для создания исходящих электронных документов определяются методы, используемые для извлечения данных из приложения и их ввода в формируемые выходные данные. Чтобы обеспечить более эффективную поддержку жизненного цикла решения ER, решение должно состоять из модели данных ER и ее компонентов сопоставления, а также формата ER и компонентов сопоставления, чтобы сопоставление моделей было привязано к приложению, в то время как другие компоненты остаются независимыми от приложения. Поэтому некоторые компоненты ER могут влиять на процесс ввода данных в формируемые выходные данные.
...
В этой статье приводятся указания по запуску отладки источников данных для выполняемых форматов ER. В нем объясняется, как эти сведения могут помочь в понимании потока данных и преобразования данных. В примерах этой статьи используется бизнес-процесс обработки платежей поставщикам.
...
- Выполните шаги в [Приложении 1](#appendix1) этой статьи, чтобы загрузить компоненты решения для электронной отчетности (ER) от Майкрософт, необходимые для обработки платежей поставщикам.
- Выполните шаги в [Приложении 2](#appendix2) этой статьи, чтобы подготовить модуль "Расчеты с поставщиками" для обработки платежей поставщикам с использованием решения для ER, которое вы загрузите.

## <a name="process-a-vendor-payment-to-get-a-payment-file"></a>Обработка платежа поставщику для получения файла платежа

1. Выполните шаги в [Приложении 3](#appendix3) этой статьи, чтобы обработать платежи поставщикам.
...
В дополнение к выбранному формату ER следующие [конфигурации](general-electronic-reporting.md#Configuration) должны быть автоматически импортированы в ваш экземпляр Microsoft Dynamics 365 Finance в рамках решения ER  **Перенос кредита ISO20022**:

- **Модель платежа** Конфигурация модели данных ER
- **Перенос кредита ISO20022** Конфигурация формата ER
...
3. Выберите **Конструктор**, а затем проверьте открытый формат.
    ...
    ![Элемент формата BankIBAN на странице конструктора формата.](./media/er-data-debugger-format.png)
