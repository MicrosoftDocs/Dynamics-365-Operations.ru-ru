...
description: В этой статье объясняется, как использовать метки электронной отчетности (ER) для разработки и создания многоязыковых отчетов.
author: NickSelin
ms.date: 05/31/2022
ms.topic: article
ms.prod: ''
...
Каждая метка ER определяется идентификатором, который уникален в области конфигурации ER, в которой содержится эта метка. Каждая метка может содержать текст метки для каждого языка, поддерживаемого в текущем экземпляре Microsoft Microsoft Dynamics 365 Finance. Эти поддерживаемые языки включают в себя языки развернутых настроек.
...
> [!TIP]
> Вы можете использовать **ПАПКА** и различные типы компонентов **ФАЙЛ** в редактируемом формате электронной отчетности (ER) для указания способа создания исходящего файла. Чтобы присвоить имя созданному файлу, настройте [выражение](er-formula-language.md) ER для параметра **Имя файла** компонента. В настроенном выражении можно использовать метки. Поскольку параметр **Имя файла** по умолчанию не зависит от языка, текст всех меток, на которые вы ссылаетесь в этом выражении, во время выполнения отображается на языке по умолчанию (EN-US). Однако в версии 10.0.28 и более поздних версиях можно включить функцию **Применить параметр "Предпочитаемый язык" к выражению "Имя файла"**. В этом случае при вычислении выражения **Имя файла** будет учитываться параметр **Языковые предпочтения**.
...
> [!TIP]
> При разработке решения ER можно создать собственный компонент ER [модели данных](er-overview-components.md#data-model-component) из предоставленного. В этой производной модели данных можно ввести свои собственные метки ER и использовать их во всех форматах ER, которые будут использовать модель данных в качестве источника данных. После этого можно создать собственный компонент [формата](er-overview-components.md#format-component) ER из предоставленного, выбрав производную модель данных ER вместо указанной. В версии 10.0.28 и более новых версиях можно включить функцию **Расширенный доступ к меткам восходящей модели данных электронной отчетности** для получения доступа к меткам восходящей модели данных электронной отчетности в производных компонентах формата электронной отчетности, даже если для производного компонента электронной отчетности выбрана модель данных электронной отчетности, которая отличается от модели данных, используемой в базовом компоненте электронной отчетности.
>
> Если в производном компоненте и его восходящих компонентах используется аналогичное имя метки, ваш перевод этой метки используется как наиболее соответствующий.
...
Как описывалось ранее в этой статье, с меткой ER, доступной в соответствующем компоненте электронной отчетности, можно связать атрибуты **Метка** и **Описание** для каждого значения перечисления ER [модели](#LinkModelEnum) или [формата](#LinkFormatEnum). Можно настроить выражение ER для вызова функции **LISTOFFIELDS**, используя в качестве аргумента перечисление ER. Это выражение возвращает список, содержащий запись для каждого значения перечисления ER, которое было определено в качестве аргумента данной функции. Каждая запись содержит значение метки ER, связанное со значением перечисления ER:
