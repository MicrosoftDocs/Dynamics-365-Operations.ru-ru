...
description: В этой теме объясняется, как разработать формат электронной отчетности (ER), которые разбивает на страницы созданные документы в Microsoft Excel.
author: NickSelin
ms.date: 09/14/2021
ms.topic: article
ms.prod: ''
...
В этом разделе поясняется, как пользователь с ролью "Системный администратор" или "Функциональный консультант по электронной отчетности" может настроить формат [электронной отчетности (ER)](general-electronic-reporting.md) для создания исходящих документов в Microsoft Excel и управления разбивкой документа на страницы.
...
Процедуры в этой теме могут быть выполнены в компании **DEMF**. Написание кода не требуется. Перед началом необходимо загрузить и сохранить следующие файлы.
...
1. Выполните действия, указанные в разделе [Замена текущего шаблона Excel в пользовательском формате электронной отчетности](#replace-template) данной темы. Однако на шаге 7 выберите файл **ERIntrastatReportDemo2.xlsx**.
...
1. Убедитесь, что у вас имеются 24 проводки на странице **Интрастат**. Повторите шаги в разделе [Создание контрольного отчета о декларации Интрастат](#generate-intrastat-control-report) данной темы, чтобы создать и просмотреть контрольный отчет.
