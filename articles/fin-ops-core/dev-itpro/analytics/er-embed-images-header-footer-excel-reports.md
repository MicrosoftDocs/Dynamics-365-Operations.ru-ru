...
description: В этой статье объясняется, как использовать электронную отчетность (ER) для создания бизнес-документов, содержащих изображения и фигуры, внедренные в верхние или нижние колонтитулы страницы.
author: kfend
ms.date: 08/11/2021
ms.topic: article
ms.prod: ''
...
В этой статье объясняется, как пользователь с ролью системного администратора или функционального консультанта по электронной отчетности может выполнять следующие задачи:
...
Процедуры в этой статье могут быть выполнены в компании **USMF**. Написание кода не требуется. Перед началом необходимо загрузить и сохранить следующий файл.
...
Чтобы добавить стандартные конфигурации электронной отчетности в ваш текущий экземпляр Dynamics 365 Finance, вы должны импортировать их из [репозитория](general-electronic-reporting.md#Repository) электронной отчетности, который был настроен для этого экземпляра.
...
4. В дополнение к выбранному формату электронной отчетности **Накладная с произвольным текстом (Excel)** были импортированы другие необходимые конфигурации электронной отчетности. Убедитесь, что в дереве конфигураций доступны следующие конфигурации электронной отчетности:
    ...
    - **Модель накладных** — эта конфигурация содержит компонент модели данных электронной отчетности, который представляет структуру данных бизнес-домена накладных.
    - **Сопоставление модели накладных** — эта конфигурация содержит компонент сопоставления моделей электронной отчетности, описывающий заполнение модели данных данными приложения во время выполнения.
    - **Накладная с произвольным текстом (Excel)** — эта конфигурация содержит формат и компоненты ER сопоставления форматов. В компоненте формата указывается макет отчета, основанный на шаблоне в формате Excel. Компонент сопоставления формата содержит источник данных модели и определяет, как источник данных используется для заполнения макет отчета во время выполнения.
...
Создана версия 240.112.1 конфигурации формата ER **настраиваемой накладной с произвольным текстом (Excel)**. Эта версия имеет статус **Черновик** и может редактироваться. Текущее содержимое пользовательского формата электронной отчетности соответствует содержимому формата, предоставленного корпорацией Майкрософт.
