...
description: Эта статья представляет собой учебник по настройке и установке средства Regression Suite Automation Tool (RSAT).
author: tonyafehr
ms.date: 09/20/2019
ms.topic: article
ms.prod: ''
...
Эта статья представляет собой учебник, который поможет вам настроить и начать работу с RSAT и средствами, связанными с использованием RSAT.
...
### <a name="configure-rsat"></a>Настроить RSAT
...
3. Нажмите кнопку **Настройки** (символ шестеренки) в верхнем правом углу, затем в появившемся диалоговом окне заполните следующие поля:
    ...
    - **Имя узла SOAP** — введите имя узла SOAP тестовой среды. Обычно имя узла SOAP совпадает с именем хоста, но имеет суффикс **soap**. Например: **\<myaos\>soap.cloudax.dynamics.com**. Не включайте префикс **https://** или **http://**.
