...
description: В этой статье показано, как создавать настраиваемые поля с помощью пользовательского интерфейса для адаптации приложения к потребностям вашего бизнеса.
author: jasongre
ms.date: 12/15/2022
ms.topic: article
ms.prod: ''
...
1. Перейдите на страницу, в которой требуется новое поле.
...
4. Выберите область формы, в которой требуется разместить новое поле. После выбора в диалоговом окне **Вставка полей** будет выведен список имеющихся полей, которые могут быть вставлены в выбранной области страницы.
...
8. Если это единственное поле, которое требуется создать для этой страницы, нажмите кнопку **Сохранить**. Если необходимо создать дополнительные поля, щелкните **Сохранить и создать** и вернитесь к шагу 7. 

>[!Note] 
> В настоящее время имеется ограничение в **20 настраиваемых полей на таблицу**.

9. При закрытии диалогового окна **Создать новое поле** производится возврат в диалоговое окно **Вставка полей**. Все пользовательские поля, которые только что были добавлены, автоматически помечаются в списке полей для вставки на страницу.
10. Щелкните **Вставить** для вставки отмеченных полей в выбранную область страницы.
...
Также может быть важно разрешить, чтобы пользовательские поля были видимы в информационных объектах. Информационные объекты используются в функции [Обзор интеграции с Office](../../dev-itpro/office-integration/office-integration.md), и для сценариев импорта и экспорта данных.
...
### <a name="how-can-i-move-custom-fields-between-environments"></a>Как можно перемещать настраиваемые поля между средами? 
...
1. Перейдите на страницу **Пользовательские поля**, выберите эту таблицу из раскрывающегося списка. 
2. В целевой среде выполните процедуру, описанную ранее в этой статье, чтобы повторно создать каждое поле. 
3. После создания всех полей щелкните **Применить изменения**.  
4. Переместите все личные настройки, содержащие настраиваемые поля, путем экспортирования личных настроек из исходной среды и импортирования их в целевую среду.  
