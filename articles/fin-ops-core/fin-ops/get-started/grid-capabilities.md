...
## <a name="developer-topics"></a>Темы для разработчика
...
### <a name="developer-handling-grids-with-non-react-extensible-controls"></a>[Разработчик] Обработка сеток с расширяемыми элементами управления, не основанными на React
Если при загрузке сетки система встречает расширяемый элемент управления, который не основан на React, система выполнит принудительную прорисовку старой сетки. Когда пользователь впервые сталкивается с этой ситуацией, отображается сообщение, указывающее на необходимость обновления страницы. Затем эта страница будет автоматически загружать старую сетку без дальнейших уведомлений пользователя до следующего обновления системы. 

Чтобы окончательно преодолеть эту ситуацию, авторы расширяемых элементов управления могут создать версию React элемента управления для использования в сетке.  После разработки класс X++ для этого элемента управления можно снабдить атрибутом **FormReactControlAttribute**, чтобы указать местоположение комплекта React для загрузки этого элемента управления. См. класс `SegmentedEntryControl` в качестве примера.  
...
- После включения функции **Новый элемент управления сетки** для некоторых страниц будет использоваться существующий элемент управления "сетка". Это произойдет в следующих ситуациях:
    ...
    - [Устранена] Проблема 762533: непредвиденная ошибка клиента при выборе строки в списке карточек.
    - [Разрешено] На странице есть список карточек, отображаемый в нескольких столбцах.
        - Этот тип списка карт поддерживается **новым элементом управления grid**, начиная с версии 10.0.30. Любое использование forceLegacyGrid() для этой цели может быть удалено. 
    - [Разрешено] На странице есть сгруппированный список карточек.
        - Сгруппированные списки карточек поддерживаются **новым элементом управления grid**, начиная с версии 10.0.30. Любое использование forceLegacyGrid() для этой цели может быть удалено. 
    - [Разрешено] Столбец сетки с расширяемым элементом управления, не основанным на React.
        - Расширяемые элементы управления могут предоставлять версию React своего элемента управления, которая будет загружаться при размещении в сетке, и изменять определение их элемента управления для загрузки этого элемента управления при использовании в сетке. Для получения дополнительных сведений см. соответствующий раздел разработчика. 
