...
Для поддержки этого нового поведения новый столбец для статуса строки был добавлен в верхнюю часть сетки, когда сетка находится в режиме редактирования. Этот столбец указывает один из следующих статусов:
...
- Списки карточек, которые отображались как несколько столбцов, теперь отображаются в виде одного столбца.
- Сгруппированные списки не отображаются как группы или в отдельных столбцах.
- Всплывающие подсказки для изображений не отображаются.
- Отображение линий сетки не работает для всех типов полей.
- Периодически невозможно щелкнуть вне сетки после выбора нескольких строк.
- Параметры регистратора задач **Проверить** и **Копировать** недоступны для элементов управления даты или числа.

### <a name="fixed-as-part-of-10012"></a>Исправлено как часть 10.0.12

> [!Note]
> Следующие сведения предоставляются, чтобы вы могли планировать в соответствии с этим. Дополнительные сведения о планируемом расписании выпуска версии 10.0.12 см. в разделе [Доступность обновления службы](../../fin-ops/get-started/public-preview-releases.md).

- [Проблема 429126] Элементы управления вне сетки не обновляются после удаления последней записи.
- [Проблема 430575] Элементы управления таблицы не обновляют содержимое отображаемых элементов.
...
- [KB 4558575] Поля не обновляются после того, как обработка изменения строки или сетки зависает после удаления строки.
- [Проблема 436980] Стили, связанные с панелью списка **ExtendedStyle**, не применяются.
- [KB 4558573] Ошибки проверки не могут быть исправлены, если требуемое изменение находится вне сетки.
    
### <a name="quality-update-for-10011"></a>Обновление качества для 10.0.11
