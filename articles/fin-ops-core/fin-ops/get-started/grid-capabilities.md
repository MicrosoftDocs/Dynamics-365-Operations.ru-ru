...
Бизнес-пользователям часто приходится выполнять нерегламентированный анализ данных. Хотя это можно сделать, если экспортировать данные в Microsoft Excel и с помощью сводных таблиц, функция **(Предварительная версия) Группировка в сетках**, которая зависит от новой функции элемента управления сетки, позволяет пользователям организовывать свои табличные данные нужным образом в приложениях Finance and Operations. Поскольку эта функция расширяет функцию **Итог**, **Группирование** позволяет получить осмысленные сведения о данных, представляя промежуточные итоги на уровне группы.
...
-  Имя столбца (эта информация будет особенно полезна после поддержки нескольких уровней группировки).  
...
Если выбрано значение **Группировать по этому столбцу** для отдельном столбце, исходная группировка заменяется, поскольку в версии 10.0.9 с обновлением платформы 33 поддерживается только один уровень группировки.
...
Этот API будет обрабатываться до выпуска в октябре 2021 года, когда новый элемент управления "сетка" станет обязательным. Сообщите в Microsoft о проблемах, требующих использования этого API. 
