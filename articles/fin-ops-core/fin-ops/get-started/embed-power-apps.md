...
Microsoft Power Apps — это служба, которая позволяет разработчикам и обычным пользователям создавать пользовательские бизнес-приложения для мобильных устройств, планшетов и Интернета без необходимости написания кода. Интеграция поддержки приложений Финансы и операции с Power Apps. Приложения на основе холста, разработанные вами, вашей организации или более широкой экосистемой, можно внедрить в приложения Финансы и операции для расширения функциональности продукта. Например, можно создать приложение на основе холста из Power Apps, которое дополняет приложение Финансы и операции информацией, извлеченной из другой системы.
...
Существует три способа внедрения приложения на основе холста в приложение Финансы и операции. Можно использовать подход, который наилучшим образом подходит для вашего сценария. 
...
Возможно, потребуется внедрить приложение на основе холста с панели мониторинга, если приложение не связано с существующей страницей или если требуется только реализовать приложение как полностраничное внутри приложения Финансы и операции.
...
> [!NOTE]
> Если доступ к приложению на основе холста был предоставлен внешним пользователям, эти пользователи не могут использовать внедренное приложение внутри приложений Финансы и операции. Однако они имеют доступ к приложению непосредственно внутри Power Apps. Внешние пользователи включают гостей и пользователей, не принадлежащих к каталогу Microsoft 365 Azure Directory, в котором развернуто приложение Финансы и операции.
...
## <a name="building-a-canvas-app-that-uses-data-that-is-sent-from-finance-and-operations-apps"></a>Создание приложения на основе холста, в котором используются данные, отправленные из приложений Финансы и операции

При создании приложения на основе холста, которое будет внедрено в приложение Финансы и операции, одна важная часть процесса — это использование входных данных из этого приложения Финансы и операции. Из процесса разработки Power Apps доступ к входным данным, которые передаются из приложения Финансы и операции, возможен с помощью переменной **Param("EntityId")**. Кроме того, с версии 10.0.19 текущее юридическое лицо также будет передано приложению на основе холста через переменную **Param("cmp")**. 

Например, в функции OnStart приложения задать входные данные из приложений Финансы и операции для переменной следующим образом:
