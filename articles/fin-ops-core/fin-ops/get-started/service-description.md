...
| **Обновления службы** | |
| Корпорация Майкрософт применяет обновления служб к указанным производственных и непроизводственным экземплярам. | Х | Х |
| Загрузите обновление из LCS, определите, разработайте и проверьте обновление и предоставьте пакет обновления кода обратно в LCS. | | Х |
| Запрос на применение обновлений расширений к производственному экземпляру. | | Х |
...
| Следите за производительностью базы данных производственного экземпляра. | Х | Х |
| Настройте производительность базы данных производственного экземпляра. | Х | Х |
| Инициируйте копирование базы данных производственного экземпляра в непроизводственный экземпляр. | | Х |
...
| Изучайте и устраняйте неполадки. | Х | Х |
...
| <p>При возникновении сбоя Microsoft определяет, должна ли быть выполнена отработка отказа для клиента, и будут ли потеряны данные. Потеря данных может состоять не более чем из пяти секунд. Дополнительные сведения см. в разделе [Географическое восстановление базы данных Azure SQL](https://azure.microsoft.com/blog/azure-sql-database-geo-restore).</p><p>В случае потери данных корпорация Майкрософт запросит у клиента отработку отказа.</p> | В случае потери данных клиенту, возможно, придется предоставить письменный запрос для запуска отработки отказа. |
