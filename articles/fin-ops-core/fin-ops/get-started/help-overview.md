...
description: Эта статья содержит обзор справочной системы для приложений для управления финансами и операциями.
author: edupont04
ms.date: 08/16/2022
ms.topic: overview
ms.prod: ''
...
## <a name="help-on-microsoft-learn"></a>Справка по Microsoft Learn

([Документация по Microsoft Dynamics 365](/dynamics365/)) в Microsoft Learn является источником по умолчанию для документации по продукту для перечисленных выше приложений. Этот сайт содержит следующие возможности:
...
Содержимое в Microsoft Learn можно найти с помощью любой поисковой системы. Для оптимальных результатов рекомендуется использовать поиск по сайту, например **site:learn.microsoft.com dynamics 365 "искомый термин"**.

## <a name="get-notified-about-changes-through-an-rss-feed"></a>Получение уведомлений об изменениях через RSS-канал

Чтобы подписаться на RSS-канал для всех обновлений, которые сделаны в содержимом технической документации Майкрософт в приложениях для управления финансами и операциями, используйте следующую ссылку:

[RSS-канал](/api/search/rss?$filter=scopes%2fany(t%3A%20t%20eq%20%27dynamics365-finops%27)&locale=en-us)

> [!NOTE]
> RSS-канал возвращает список последних 100 тем, которые были обновлены последними. Список сортируется по дате, но может занять до недели, прежде чем последние обновленные статьи будут внесены в список.  
...
При наличии отзывов или вопросов по статье оставьте свой комментарий в нижней части страницы.
...
Можно вносить вклад и делать изменения в документации. Чтобы начать работу, нажмите кнопку **Изменить** (символ карандаша) в статье. Следующий видеоролик показывает, как можно внести свой вклад в нашу документацию.
...
Дополнительные сведения см. в [руководстве для участников разработки портала "Документация Microsoft"](/contribute), которое публикуется рабочей группой, которая создала сайт Microsoft Learn.
...
Microsoft выпустила библиотеки проводника по задачам для версий продуктов через выпуск Dynamics 365 для управления финансами и операциями от Декабрь 2017. В разделе [Доступ к руководствам по задачам в области "Справка"](#accessing-task-guides-from-the-help-pane) в этой статье поясняется, как найти правильные руководства по задачам для вашего продукта.
...
В области **Справка** можно получить доступ к разделам, которые применяются к клиенту. При первом открытии области **справки** вкладки **Справка** отображает разделы, которые применяются к странице, на которой вы находитесь в данный момент. Если разделы не найдены, можно ввести ключевые слова для уточнения поиска. При выборе статьи в области **Справка** она открывается на новой вкладке в браузере.
...
| Сайт | Описание |
|------|-------------|
| [Документация Microsoft Dynamics 365](/dynamics365/) | Этот сайт предоставляет всю документацию по продуктам для Dynamics 365 или ссылки на нее. |
| [Обучение Microsoft Learn](/training/) | Этот сайт является бесплатным сайтом электронного обучения Microsoft. |
...
| [Предыдущие версии](/previous-versions/dynamics/) | Этот сайт размещает содержимое из предыдущих выпусков. |
