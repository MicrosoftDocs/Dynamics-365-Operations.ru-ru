...
description: В этой статье описываются четыре принципа очистки, которые используются для потребления сырья.
...
> [!NOTE]
> Принцип очистки "Готово" нельзя использовать вместе с номенклатурами планирования. Рекомендуется использовать вместо него принцип очистки "Начать". Плановая номенклатура имеет тип продукции *Плановая номенклатура*, а в партионных заказах, созданных для плановой номенклатуры, приемка может быть выполнена только для побочных и попутных продуктов.
