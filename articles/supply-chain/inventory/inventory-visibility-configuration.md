...
- [Конфигурация предварительной загрузки запросов (необязательно)](#query-preload-configuration)
...
| *OnHandIndexQueryPreloadBackgroundService* | Эта функция периодически извлекает и сохраняет набор данных о запасах в наличии на основе предварительно настроенных аналитик. Она предоставляет сводку запасов, которая включает только те измерения, которые относятся к ежедневному бизнесу и совместимы с номенклатурами, доступными для процессов управления складом (WMS). Дополнительные сведения см. в разделах [Включение и настройка предварительно загруженных запросов запасов в наличии](#query-preload-configuration) и [Предварительная загрузка оптимизированного запроса запасов в наличии](inventory-visibility-power-platform.md#preload-streamlined-onhand-query). |
...
> [!IMPORTANT]
> Рекомендуется использовать функцию *OnHandIndexQueryPreloadBackgroundService* или *OnHandMostSpecificBackgroundService*, но не обе. Включение обеих функций оказывает влияние на производительность.
...
Каждый источник данных представляет систему, из которой поступают данные. Примерами имен источника данных являются `fno` (что соответствует Supply Chain Management) и `pos` (что означает "POS"). По умолчанию Supply Chain Management настраивается в качестве источника данных по умолчанию (`fno`) в видимости запасов.
...
Например, уже создан источник данных с именем `ecommerce`, и он включает в себя аналитику цвета продукта. В этом случае для выполнения сопоставления можно сначала добавить `ProductColor` в поле **Имя измерения** в источнике данных `ecommerce`, а затем выбрать `ColorId` в поле **В базовую аналитику**.
...
### <a name="extended-dimensions"></a>Расширенные измерения

Клиенты, желающие использовать внешние источники данных в источнике данных, могут воспользоваться преимуществами расширяемости, предлагаемыми Dynamics 365, путем создания [расширений классов](../../fin-ops-core/dev-itpro/extensibility/class-extensions.md) для классов `InventOnHandChangeEventDimensionSet` и `InventInventoryDataServiceBatchJobTask`.

После создания расширений обязательно выполните синхронизацию с базой данных, чтобы добавить настраиваемые поля в таблицу `InventSum`. Затем можно обратиться к разделу "Измерения" ранее в этой статье, для сопоставления пользовательских аналитик с любой из восьми расширенных аналитик в `BaseDimensions` в запасах.

> [!NOTE] 
> Дополнительные сведения о создании расширений см. в разделе [Домашняя страница расширяемости](../../fin-ops-core/dev-itpro/extensibility/extensibility-home-page.md).
...
## <a name="turn-on-and-configure-preloaded-on-hand-queries-optional"></a><a name="query-preload-configuration"></a>Включение и настройка предварительно загруженных запросов запасов в наличии (необязательно)

Видимость запасов может периодически извлекать и сохранять набор данных о запасах в наличии на основе предварительно настроенных аналитик. Это обеспечивает следующие преимущества:

- Более ясное представление, в котором хранится сводка запасов, которая включает только те измерения, которые относятся к ежедневному бизнесу.
- Сводка запасов, совместимая с номенклатурами, доступными для процессов управления складом (WMS).

Для получения дополнительных сведений о том, как работать с этим компонентом после настройки, см. в разделе [Предварительная загрузка оптимизированного запроса запасов в наличии](inventory-visibility-power-platform.md#preload-streamlined-onhand-query).

> [!IMPORTANT]
> Рекомендуется использовать функцию *OnHandIndexQueryPreloadBackgroundService* или *OnHandMostSpecificBackgroundService*, но не обе. Включение обеих функций оказывает влияние на производительность.

Чтобы настроить эту функцию, сделайте следующее.

1. Выполните вход в приложение Power Apps видимости запасов.
1. Перейдите в раздел **Настройка \> Управление функциями и параметры**.
1. Если функция *OnHandIndexQueryPreloadBackgroundService* уже включена, рекомендуется выключить ее, поскольку для выполнения процесса очистки может потребоваться очень много времени. Вы включите ее позже в этой процедуре.
1. Откройте вкладку **Настройка предварительной загрузки**.
1. В разделе **Шаг 1. Очистка памяти для предварительной загрузки** выберите команду **Очистить**, чтобы очистить базу данных и подготовить ее к приему новых параметров группирования.
1. В разделе **Шаг 2. Настройка значений группировки** в поле **Группировка результатов** введите разделенный запятыми список имен полей, по которым группируются результаты запроса. После получения данных в базе данных предварительной загрузки вы не сможете изменить эту настройку, пока не очистите базу данных, как описано в предыдущем шаге.
1. Перейдите в раздел **Настройка \> Управление функциями и параметры**.
1. Включите функцию *OnHandIndexQueryPreloadBackgroundService*.
1. Выберите **Обновить конфигурацию** в верхнем правом углу страницы **Конфигурация**, чтобы зафиксировать ваши изменения.
