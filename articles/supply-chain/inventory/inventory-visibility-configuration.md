...
В этом разделе описывается, как настроить видимость запасов для с помощью приложения Видимость запасов в Power Apps.
...
В Power Apps страница **Конфигурация** [приложения Видимость запасов](inventory-visibility-power-platform.md) позволяет настроить конфигурацию в наличии и конфигурацию предварительного резервирования. После установки надстройки конфигурация по умолчанию включает значение от Microsoft Dynamics 365 Supply Chain Management (источник данных `fno`). Можно просмотреть параметры по умолчанию. Кроме того, в зависимости от бизнес-требований и требований разноски запасов для внешней системы, можно изменить конфигурацию, чтобы стандартизировать способ разноски, упорядочения и запроса изменений запасов в нескольких системах. В остальных разделах этой темы объясняется, как использовать каждую часть страницы **Конфигурация**.
...
| OnhandChangeSchedule | Функция позволяет использовать функции графика изменения запасов в наличии и доступности для заказа (ATP). Дополнительные сведения см. в разделе [Графики изменения запасов в наличии и доступность для заказа](inventory-visibility-available-to-promise.md). |
...
Каждый источник данных представляет систему, из которой поступают данные. Примерами имен источника данных являются `fno` (то есть "приложения Dynamics 365 Finance и Operations") и `pos` (то есть "POS"). По умолчанию Supply Chain Management настраивается в качестве источника данных по умолчанию (`fno`) в видимости запасов.
...
> 
> Если необходимо запросить только запасы, агрегированные по всем комбинациям аналитик, можно настроить один индекс, содержащий базовое аналитику `Empty`.
