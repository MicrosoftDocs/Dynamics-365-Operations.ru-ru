...
description: В этой статье описывается, как настроить и управлять учетными записями пользователей, которые позволяют сотрудникам входить в систему и использовать приложение склада.
author: Mirzaab
ms.date: 09/15/2021
ms.topic: article
ms.search.form: ''
...
1. На вкладке **Настройки** установите следующие поля:
    ...
    - **Язык** — выберите язык, который предпочитает работник. Это поле также управляет форматом чисел, отображаемых в приложении склада.
    - **Формат даты, времени и чисел** — выберите формат даты и времени, который предпочитает сотрудник. Приложение склада использует числовой формат, связанный с языком, выбранным для поля **Язык**, вместо этого параметра.
