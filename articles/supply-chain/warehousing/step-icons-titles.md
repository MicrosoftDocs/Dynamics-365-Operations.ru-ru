...
description: В этой статье описывается, как назначать значки и названия шагов для новых или настроенных потоков задач для мобильного приложения Warehouse Management.
author: Mirzaab
ms.date: 05/17/2021
ms.topic: article
...
Пример использования этих идентификаторов шагов и классов см. в реализации метода `WHSMobileAppStepInfoBuilder.stepId()` в разделе [Пример: назначение значков и заголовков шагов для настраиваемого потока](#example) далее в этой статье.
...
Чтобы назначить значок шага, установите `defaultStepIcon` на один из кодов значков, которые указаны в разделе [Доступные значки шагов](#step-icons) ранее в этой статье.
