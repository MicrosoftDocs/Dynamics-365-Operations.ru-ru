...
description: В этой статье объясняется, как использовать функцию подтверждения и переноса, которая позволяет пользователям отгружать загрузки со склада до выполнения всех работ, связанных с этими загрузками.
author: Mirzaab
ms.date: 07/01/2020
ms.topic: article
ms.prod: ''
...
- Никакие запасы в данный момент не ожидают упаковки в местонахождении упаковки. (Функция *Подтвердить и переместить* не поддерживает запасы, которые были скомплектованы на станцию упаковки, но еще не упакованы, за исключениями случая, когда контейнеры, которые упакованы, помещены в промежуточные местоположения с созданной работой загрузки.)
...
Эта настройка не является обязательной для всех ситуаций. Показанный здесь пример гарантирует, что работа может быть прервана отгрузкой для поддержки примера сценария, приведенного ниже в этой статье. Кроме того, существуют другие способы достижения этого результата.
