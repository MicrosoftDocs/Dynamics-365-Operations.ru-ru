...
Когда иерархия резервирования запасов типа *партия-ниже\[местоположение\]* связана с продуктами, компании, которые продают продукты с отслеживанием партий и выполняют свою логистику в качестве операций, включенных для процессов управления складом (WMS), не могут резервировать отдельные партии этих продуктов для заказов на продажу для клиентов.
...
В этой статье описывается политика резервирования запасов, которая позволяет этим компаниям резервировать отдельные партии или грузоместа, даже если продукты связаны с иерархией резервирования *партия-ниже\[местоположение\]*.
...
### <a name="turn-flexible-license-plate-reservation-on-or-off"></a>Включение или выключение гибкого резервирования грузомест

Прежде чем использовать гибкое резервирование грузомест, должны быть включены две функции для системы. Администраторы могут использовать параметры [управления компонентами](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md) для проверки статуса этих функций и их включения, если это требуется. Необходимо включить функции в следующем порядке:

1. *Гибкое резервирование аналитик на уровне склада*<br>(В Supply Chain Management версии 10.0.29 эта функция обязательна и не может быть отключена.)
1. *Гибкое резервирование номерных знаков с привязкой к конкретному заказу*<br>(В Supply Chain Management версии 10.0.29 эта функция обязательна и не может быть отключена.)
...
1. Нажмите кнопку **Удалить** ![Стрелка назад.](media/backward-button.png) чтобы переместить выбранные записи в список **Доступно**.
1. Нажмите **ОК**.
...
- [Номера партий в модуле "Управление складом"](/dynamicsax-2012/appuser-itpro/batch-numbers-in-warehouse-management)
