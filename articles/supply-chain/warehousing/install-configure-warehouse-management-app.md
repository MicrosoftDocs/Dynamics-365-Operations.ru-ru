...
> [!NOTE]
> В этой теме описывается, как настроить новое мобильное приложение управления складом, которое в настоящее время находится в режиме общедоступной предварительной версии. Если вы ищите сведения о настройке старого приложения склада, см. раздел [Установка и подключение приложения склада](../../supply-chain/warehousing/install-configure-warehousing-app.md).

Общедоступная предварительная версия мобильного приложения управления складом доступна для загрузки в центре приложений Microsoft. Оно предоставляется как отдельный компонент. Поэтому необходимо загрузить его на каждое устройство, а затем настроить его для подключения к среде Microsoft Dynamics 365 Supply Chain Management.

В этом разделе описан порядок установки мобильного приложения управления складом на каждом из мобильных устройств и его настройки для подключения к среде Supply Chain Management. Можно настроить каждое устройство вручную или импортировать параметры подключения из файла или путем сканирования QR-кода.
...
- **Windows (UWP):** [Программа предварительных версий центра приложений — Windows](https://go.microsoft.com/fwlink/?linkid=2154406)

    Поскольку это приложение является предварительной версией, для его установки требуется несколько дополнительных шагов. Подробные сведения см. в разделе [Установка сборки из центра приложений](https://docs.microsoft.com/appcenter/distribution/installation).

- **Android:** [Программа предварительных версий центра приложений — Android](https://go.microsoft.com/fwlink/?linkid=2154613)

    Поскольку это приложение является предварительной версией, для его установки требуется несколько дополнительных шагов. Подробности см. в разделе [Тестирование приложений Android](https://docs.microsoft.com/appcenter/distribution/testers/testing-android).

Для небольших развертываний может потребоваться установить приложение на каждое устройство из соответствующего магазина, а затем вручную настроить подключение к используемым средам. Однако можно также автоматизировать развертывание и/или настройку приложения. Такой подход удобен, если вы управляете многими устройствами, и используете решение управления мобильными устройствами и мобильным приложением, такое как [Intune](https://docs.microsoft.com/mem/intune/fundamentals/what-is-intune). Сведения об использовании Intune для добавления приложений см. в разделе [Добавление приложений в Microsoft Intune](https://docs.microsoft.com/mem/intune/apps/apps-add).
...
1. Свяжите приложение Azure AD с пользователем мобильного приложения управления складом:
    ...
    1. Создайте строку.
    1. Введите код клиента, для которого была создана заметка в предыдущем разделе, присвойте ему имя и выберите только что созданного вами пользователя. Рекомендуется отмечать все свои устройства. Затем, если устройство будет потеряно, можно легко удалить его доступ к Supply Chain Management с этой страницы.
