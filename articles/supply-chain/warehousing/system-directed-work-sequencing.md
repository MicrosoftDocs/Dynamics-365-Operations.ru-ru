...
Для работы с этим сценарием с помощью значений, представленных в этой теме, следует работать с системой, в которой установлены стандартные демонстрационные данные. Дополнительно необходимо выбрать юридическое лицо **USMF**. В сценарии используется склад *51* из демонстрационных данных.
...
При использовании данных Contoso по умолчанию изменения запроса для действия директивы расположения не требуются. Однако, чтобы гарантировать, что директивы расположения будут фиксировать номенклатуры в заказах на продажу при применении этой функции в среде, отличной от Contoso, создайте новую директиву расположения. Чтобы проверить настройки в демонстрационной среде, выполните следующие действия.
