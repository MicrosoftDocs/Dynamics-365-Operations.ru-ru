...
description: В этой статье приведен обзор вариантов миграции продуктов и управления складом.
author: perlynne
ms.date: 06/20/2017
ms.topic: article
ms.prod: ''
...
В этой статье приводится обзор процесса обновления с Microsoft Dynamics AX 2012 R3, в котором выполняется модуль WMSII, до Supply Chain Management.
...
Это изменение необходимо, поскольку в Supply Chain Management отслеживание номенклатур является составляющей процессов управления складом (WMS). Для этих процессов все склады и их местонахождения должны быть связаны с профилем местонахождения. Если вы хотите использовать WMS, необходимо настроить следующее:
-   Существующие склады необходимо активировать для использования процессов WMS 
-   Существующие выпущенные продукты необходимо связать с группой аналитик хранения, предполагающей использование процессов WMS 

Если в исходных группах аналитик хранения используется складская аналитика "Код палеты", местонахождения существующих запасов в наличии, в которых использовалась складская аналитика "Код палеты", должны быть связаны с профилем местонахождения, где выбран параметр **Использовать отслеживание грузоместа**. Если существующие склады не нужно активировать для использования процессов WMS, можно изменить группы аналитик хранения существующих запасов в наличии на группы, в которых фигурируют только складские аналитики "Сайт", "Склад" и "Местонахождение". 
...
-   Изменить группу аналитик хранения для номенклатуры на группу аналитик хранения, которая предполагает использование процессов WMS. В результате этого изменения будет использоваться складская аналитика "Грузоместо".

## <a name="configure-wms"></a>Настроить WMS
...
### <a name="enable-warehouses-to-use-wms"></a>Включение складов для использования WMS
...
### <a name="change-the-storage-dimension-group-for-items-so-that-it-uses-wms"></a>Изменение группы аналитик хранения для номенклатур так, чтобы она предполагала использование процессов WMS
