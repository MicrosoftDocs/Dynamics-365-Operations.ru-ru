...
description: В этой статье описывается, как настроить штрих-коды GS1 и QR-коды, чтобы этикетки можно было просканировать на складе.
author: Mirzaab
ms.date: 03/21/2022
ms.topic: article
ms.search.form: ''
...
При использовании штрих-кодов GS1 рекомендуется также включить функцию *Расширенный анализатор для штрихкодов GS1*. Эта функция обеспечивает улучшенную реализацию средства синтаксического анализа штрих-кода GS1. Добавлены следующие усовершенствования:
...
1. На экспресс-вкладке **Штрих-коды** задайте следующие поля:
    ...
    - **Максимальная длина идентификатора** — указывает максимальное количество символов, разрешенных для идентификатора приложения. Это поле не является обязательным, если для системы включена функция *Расширенный синтаксический анализатор для штрихкодов GS1*.
