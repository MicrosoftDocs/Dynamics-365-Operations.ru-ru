...
description: В этом разделе описывается, как создавать и обрабатывать заказы на перемещение из функции приложения склада
author: perlynne
manager: tfehr
ms.date: 09/02/2020
ms.topic: article
...
1. Выберите **Создать** для добавления нового пункта меню. Затем установите следующие параметры, чтобы начать работу:
    ...
    - **Заголовок** — назначьте имя меню, которое должно быть представлено работникам в приложении склада.
    - **Режим** — устанавливается в значение *Косвенно* (это приложение склада не будет создавать работу).
...
Приложение склада имеет простой процесс создания заказов на перемещение на основе грузомест. Для этого работник выполняет следующие действия, используя приложение склада:
...
1. С помощью приложения склада вы выбрали пункт меню, который использует код действия **Создание заказа на перемещение из грузоместа**.
...
1. В приложении склада, когда выбрана кнопка **Завершить заказ**, создается новое событие приложения склада, **Завершить заказ на перемещение**, и связанное с ним существующее событие, **Создание заказа на перемещение**, изменяет статус на **В очереди**.
...
### <a name="warehouse-app-processes"></a>Процессы приложения склада
...
#### <a name="can-several-warehouse-app-users-add-license-plates-to-the-same-transfer-order-at-the-same-time"></a>Могут ли несколько пользователей приложения склада добавлять грузоместа в один и тот же заказ на перемещение в одно и то же время?
...
#### <a name="how-can-i-find-existing-transfer-orders-to-be-used-via-the-select-transfer-order-button-in-the-warehouse-app-if-the-order-has-not-yet-been-created-in-the-backend-system"></a>Как можно найти существующие заказы на перемещение для использования с помощью кнопки "Выбор заказа на перемещение" в приложении склада, если заказ еще не был создан в серверной системе?
...
#### <a name="can-i-manually-select-the-transfer-order-number-to-be-used-from-the-warehouse-app"></a>Можно ли вручную выбрать номер заказа на перемещение для использования из приложения склада?
...
Эта функция поддерживает только положительные физические количества в наличии. Убедитесь, что на складе и в уровне статуса запасов имеются положительные физические количества в наличии, прежде чем назначать грузоместа заказу на перемещение.
