...
description: В этой статье описывается, как создавать и обрабатывать заказы на перемещение из мобильного приложения управления складом
author: perlynne
ms.date: 08/09/2022
ms.topic: article
ms.prod: ''
...
## <a name="turn-on-this-feature-and-its-prerequisites"></a><a name="enable-create-transfer-order-from-warehouse-app"></a>Включение этой функции и необходимые для этого условия
...
1. Включите следующие две функции (по порядку) в рабочей области [Управление функциями](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md). В Supply Chain Management версии 10.0.25 обе эти функции включены по умолчанию.
    1. *Обработка событий приложения склада*<br>(В Supply Chain Management версии 10.0.29 эта функция обязательна и не может быть отключена.)
    1. *Создать и обработать заказы на перемещение из приложения склада*<br>(В Supply Chain Management версии 10.0.29 эта функция обязательна и не может быть отключена.)
...
Можно обеспечить сотрудникам возможность искать номера заказов на перемещение в мобильном приложении Warehouse Management, используя его возможность [запроса данных](warehouse-app-data-inquiry.md). Например, можно создать элемент меню [переключения](warehouse-app-detours.md) для мобильного устройства, который будет отправлять запросы на доступ к данным, отображаемым на странице **События приложения склада** на веб-клиенте (`WHSMobileDeviceQueueMessageCollection`) как часть шага *Выбор заказа — MobileDeviceQueueMessageCollectionIdentifierId*. Номер заказа на перемещение соответствует значению, указанному в поле **Идентификатор**. См. также [Запрос событий приложения склада](#inquire-the-warehouse-app-events).
...
Поддерживаются только автоматически созданные номера заказов на перемещение через номерные серии. См. также ответ на предыдущий вопрос, относящийся к настройке кнопки **Выбрать заказ на перемещение**. Дополнительные сведения о том, как найти номера заказов на перемещение, см. в разделе [Запрос событий приложения склада](#inquire-the-warehouse-app-events).
