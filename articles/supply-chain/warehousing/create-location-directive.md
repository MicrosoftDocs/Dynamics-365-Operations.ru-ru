...
description: В этой статье описывается, как работать с директивами местоположения. Директивы местонахождения — это определяемые пользователем правила, с помощью которых можно определить местонахождения комплектации и размещения для перемещения запасов.
author: Mirzaab
ms.date: 09/28/2022
ms.topic: article
ms.prod: ''
...
> [!NOTE]
> Эта статья относится к функциям в модуле **Управление складом**. Она не применяется к функциям в модуле [Управление запасами](../inventory/inventory-home-page.md).
...
1. Создание местонахождений, типов местонахождений, профилей местонахождений и форматов местонахождений. Дополнительные сведения см. в [Настройка местонахождений на складе, поддерживающем WMS](./tasks/configure-locations-wms-enabled-warehouse.md).
1. Создание сайтов, зон и групп зон. Дополнительные сведения см. в [Настройка складов](../../commerce/channels-setup-warehouse.md) и [Настройка местонахождений на складе, поддерживающем WMS](./tasks/configure-locations-wms-enabled-warehouse.md).

## <a name="turn-the-location-directive-scopes-feature-on-or-off"></a><a name="scopes-feature"></a>Включение или выключение функции области действия директив местонахождений

[!INCLUDE [preview-banner-section](../../includes/preview-banner-section.md)]
<!-- KFM: Preview until 10.0.31 GA -->

Функция *Области действия директив местонахождений* обеспечивает более широкие возможности при проектировании директив местоположения и позволяет сократить избыточные конфигурации. Она добавляет параметр **Области**, который заменяет предыдущий параметр **Несколько SKU**. В то время как параметр **Несколько SKU** может принимать только значения *Да* или *Нет*, параметр **Области** предоставляет не только эти два значения (через значения *Одна номенклатура* и *Несколько номенклатур*), но также два других (через *Одна номенклатура или заказ* и *Все*). Дополнительные сведения об этих параметрах см. в разделе [Экспресс-вкладка директив местоположения](#location-directives-tab).

Когда она включена, параметр **Область** заменяет параметр **Несколько SKU**, и обеспечивается 100-процентная совместимость с существующими конфигурациями.

Чтобы использовать эту функцию, необходимо включить ее в вашей системе. Администраторы могут использовать параметры [управления функциями](../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md) для проверки статуса функции и ее включения либо выключения. В рабочей области **Управление функциями** эта функция перечисляется следующими способами:

- **Модуль:** *Управление складом*
- **Название функции:** *Области действия директив местонахождений*
...
| Директивы для мест хранения | Область *или* Несколько SKU |
| Строки | Порядковый номер |
| Строки | Количество "От" |
| Строки | Количество "До" |
| Линии | Единица |
...
Оттуда можно просматривать, создавать и изменять директивы местоположения с помощью команд на панели операций. Сведения о том, как использовать все поля, доступные на странице, см. в остальных разделах данной статьи.
...
- **Копировать** — открывает диалоговое окно, в котором можно создать точную копию текущей директивы местоположения.
...
- **Приемочные испытания** — открывает страницу, в которой можно настроить автоматические тесты для определения того, как будут действовать директивы местоположения при различных условиях запуска. Таким образом можно быстро проверять директивы по мере их создания и сопровождения. Дополнительные сведения см. в разделе [Тестирование директив местонахождения с помощью приемочных испытаний](location-directive-acceptance-tests.md).
...
## <a name="location-directives-fasttab"></a><a name="location-directives-tab"></a>Экспресс-вкладка директив местонахождения
...
- **Код директивы** — выберите код директивы для связи с шаблоном работы или шаблоном пополнения. На странице **Код директивы** можно создавать новые коды, которые могут использоваться для подключения шаблонов работы или шаблонов пополнения к директивам местонахождения. Коды директив также могут использоваться для установления связи между любой строкой шаблона работы и директивой местонахождения (такого как дверь или промежуточное местонахождение).
    ...
    > [!TIP]
    > Если этот код директивы задан, система не будет искать директивы местонахождения по порядковому номеру, когда работа должна быть создана. Вместо этого поиск выполняется по коду директивы. Таким образом, можно более конкретно указывать, какая директива местонахождения используется для определенного шага в шаблоне работы, например помещение на промежуточное хранение и обработку материалов.

- **Область действия** — этот параметр используется для указания сценариев, к которым будет применяться данная директива местоположения. Этот параметр заменяет параметр **Несколько SKU** и доступен только в том случае, если в системе включена функция *Области действия директив местонахождений*. (Дополнительные сведения см. в разделе [Включение или отключение функции областей действия директив местонахождений](#scopes-feature).)

    | Настройка области | Один заказ с одной номенклатурой | Несколько заказов с одной и той же номенклатурой | Один заказ с несколькими номенклатурами | Несколько заказов с несколькими номенклатурами |
    |---|---|---|---|---|
    | Одна номенклатура | Да | Да | Нет | Нет |
    | Несколько номенклатур | Нет | Нет | Да | Да |
    | Одна номенклатура или заказ | Да | Да | Да | Нет |
    | Все | Да | Да | Да | Да |

    В следующей таблице описывается, когда доступны области и разрешены ли они для функции **Изменить запрос**.

    | Область | Поддерживаемый тип работы | Поддерживаемые типы заказа на работу | Разрешение на изменение запроса |
    |---|---|---|---|
    | Одна номенклатура | Все | Все | Да |
    | Несколько номенклатур | Все | Все | Нет |
    | Одна номенклатура или заказ | Размещения | Размещение побочных и попутных продуктов, размещение готовых товаров, размещение канбанов, заказы на покупку, заказы контроля качества, пополнение, заказы на возврат, заказы на продажу, выдача для перемещения и приемка перемещения | Да |
    | Все | Размещения | Все | Нет |

    > [!NOTE]
    > - Чтобы выполнить размещение как для нескольких номенклатур, так и для одной номенклатуры, необходимо обеспечить существование директив местоположения, охватывающих оба сценария. Например, можно настроить одну или несколько директив местоположения *Одна номенклатура или заказ* для покрытия сценариев, требующих тонкой настройки (например, путем внесения изменений в запрос), затем одну или несколько директив местоположения *Все* для покрытия остальных сценариев.
    > - Хотя области *Одна номенклатура* и *Несколько номенклатур* могут использоваться для размещения, такой подход обычно приводит к избыточным конфигурациям. Вместо этого следует использовать области *Одна номенклатура или заказ* и *Все*, так как такой подход приведет к более чистой настройке.

- **Несколько SKU** — этот параметр используется для указания сценария, к которому будет применяться данная директива местоположения. Этот параметр заменяется параметром **Область**, если в системе включена функция *Области действия директив местонахождений*. (Дополнительные сведения см. в разделе [Включение или выключение функции области действия директив местонахождений](#scopes-feature).) Установите для этого параметра значение *Да*, чтобы включить использование нескольких единиц складского учета для местоположения. Например, для местоположения двери отсека должно быть включено несколько номеров SKU. Если активировано несколько SKU, местоположение размещения будет указано в работе, как ожидаемое. Однако это местоположение размещения может обрабатывать только размещение нескольких номенклатур (если работа включает разные SKU, которые должны быть скомплектованы и размещены). Оно не сможет обрабатывать размещение с одним SKU, Если для этого параметра установить значение *Нет*, местоположение размещения будет указано только в том случае, если выбранное размещение имеет только один тип SKU.
...
После создания директив местонахождения можно связать каждый код директивы с кодом шаблона работы для создания работы. Дополнительные сведения см. в разделе [Контроль работы склада с использованием шаблонов работы и директив местонахождения](./control-warehouse-location-directives.md).
...
- Статья справки: [Контроль работы склада с шаблонами работы и директивами для мест хранения](control-warehouse-location-directives.md)
