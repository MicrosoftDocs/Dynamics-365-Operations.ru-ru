...
Если используется процесс *выпуска на склад*, создаются [*складские заказы*](cloud-edge-warehouse-order.md), и владелец связанного потока получения назначается единице масштабирования. Концентратор не сможет регистрировать входящую приемку.
...
> [!NOTE]
> Пункты меню и кнопки мобильного устройства для неподдерживаемых функциональных возможностей не отображаются в _приложении склада_, когда оно связано с развертыванием единиц масштабирования.
...
- Использование изображения данных шаблонов продуктов (например, в приложении склада)
...
| Получение заказа на покупку с недопоставкой                        | <p>Да, если отсутствует складской заказ</p><p>Нет, если имеется складской заказ</p> | Нет, так как можно отменить только полные количества строки складского заказа |
