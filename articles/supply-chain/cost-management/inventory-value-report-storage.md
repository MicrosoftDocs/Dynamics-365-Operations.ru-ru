...
1. В открывшемся диалоговом окне **Стоимость запасов** установите следующие значения, чтобы определить, какие записи включаются в отчет:
    ...
    - На экспресс-вкладке **Параметры** введите уникальное имя для отчета и используйте поля в разделе **Интервал дат**, чтобы определить, какие записи включаются в отчет. Чтобы определить интервал дат, можно либо выбрать предварительно определенный диапазон ( относительно даты создания отчета) в поле **Код интервала дат**, либо выбрать определенные даты в полях **Начальная дата** и **Конечная дата**. <!-- KFM: What is the ID setting for here? What do its values mean? -->
