---
title: Утвержденные спланированные заказы
description: В этой статье объясняется, как утвердить спланированные заказы. При подтверждении спланированных заказов они становятся фактическими заказами на покупку, заказами на перемещение или производственными заказами.
author: t-benebo
ms.date: 08/09/2022
...
В этой статье подробно описывается каждый из методов.

## <a name="enable-the-features-that-are-described-in-this-article"></a><a name="enable-features"></a>Включите функции, описанные в этой статье

Большинство функций спланированного заказа доступны во всех стандартных установках Microsoft Dynamics 365 Supply Chain Management. Однако несколько функций, описанных в этой статье, должны быть включены в управлении функциями, прежде чем их можно будет использовать.

### <a name="turn-parallelized-firming-of-planned-orders-on-or-off"></a>Включение или выключение параллельного подтверждения спланированных заказов

Параллельное подтверждение помогает ускорить процесс подтверждения путем параллельного выполнения в нескольких потоках. Этот подход может быть полезен, если подтверждено много спланированных заказов. Для использования этой функции необходимо включить в системе функцию *Параллельное утверждение спланированных заказов*. 

В Supply Chain Management версии 10.0.21 эта функция включена по умолчанию. В Supply Chain Management 10.0.25 эта функция обязательна и не может быть отключена. При запуске версии, более старой, чем 10.0.25, вы можете включать или выключать эту функцию, перейдя к пункту [Управление функциями](../../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md) и выполнив поиск функции *Параллельное утверждение спланированных заказов*.

### <a name="turn-planned-order-firming-with-filtering-on-or-off"></a>Включение и выключение подтверждения спланированного заказа с фильтрацией
...
Чтобы использовать эту функцию, ее необходимо включить для системы. В Supply Chain Management версии 10.0.25 эта функция включена по умолчанию. В Supply Chain Management версии 10.0.29 эта функция обязательна и не может быть отключена. При запуске версии, более старой, чем 10.0.29, администраторы могут включать или выключать эту функцию путем поиска функции *Подтверждение спланированных заказов с фильтрацией* в рабочей области [Управление функциями](../../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md).

### <a name="turn-auto-firming-for-planning-optimization-on-or-off"></a>Включение или выключение автоматического подтверждения для оптимизации планирования
...
В Supply Chain Management версии 10.0.21 эта функция включена по умолчанию. В Supply Chain Management 10.0.29 эта функция обязательна и не может быть отключена. При запуске версии, более старой, чем 10.0.29, вы можете включать или выключать эту функцию, перейдя к пункту [Управление функциями](../../../fin-ops-core/fin-ops/get-started/feature-management/feature-management-overview.md) и выполнив поиск функции *Автоматическое подтверждение для оптимизации планирования*.
...
1. В диалоговом окне **Подтверждение** на экспресс-вкладке **Параметры** установите следующие поля. (Многие из этих полей получают значения по умолчанию из вкладки **Стандартное обновление** на странице **Параметры сводного планирования**.)
    ...
    > [!NOTE]
    > Каждый из параметров "Группировать по" заставляет систему преобразовывать каждый спланированный заказ в строку в отдельном заказе на покупку, полученный в результате группировки.
...
> [!IMPORTANT]
> Перед тем как функция, описанная в этом разделе, может быть использована при оптимизации планирования, для вашей системы должна быть включена функция [*Автоматическое подтверждение для оптимизации планирования*](#enable-features), как описано в начале этой статьи. Автоматическое подтверждение всегда может использоваться с устаревшим механизмом сводного планирования.

### <a name="auto-firming-with-planning-optimization-vs-the-deprecated-master-planning-engine"></a>Автоматическое подтверждение с оптимизацией плана и устаревшим механизмом сводного планирования

Для автоматического подтверждения спланированных заказов можно использовать как оптимизацию планирования, так и устаревший механизм сводного планирования. Однако существуют некоторые важные различия. Например, в то время как в оптимизации планирования используется дату заказа (то есть дата начала), чтобы определить, какие спланированные заказы должны быть подтверждены, в устаревшем механизме сводного планирования используется дата потребности (то есть дата окончания). В приведенной ниже таблице представлена сводная информация о различиях.

| Функция | Оптимизация планирования | Устаревшая подсистема сводного планирования |
|---|---|---|
...
> [!IMPORTANT]
> Перед тем как функция, описанная в этом разделе, может быть использована, для вашей системы должна быть включена функция [*Подтверждение спланированных заказов с фильтрацией*](#enable-features), как описано в начале этой статьи.
