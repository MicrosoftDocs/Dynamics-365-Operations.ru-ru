...
description: В этой статье обсуждается выполнение резервного запаса и порядок настройки количества резервного запаса для номенклатур.
author: t-benebo
ms.date: 8/23/2021
ms.topic: article
...
> [!NOTE]
> Для всех иллюстраций в этой статье по оси x откладываются запасы, по оси y откладывается время в днях, столбцы представляют уровень запасов, стрелки показывают проводки, такие как строки заказов на продажу, строки заказов на покупку или спланированные заказы.
...
[![Планирование номенклатуры с кодом покрытия Период и выполнением Первый расход.](media/FirstIssuePeriod.png)](media/FirstIssuePeriod.png)
