---
title: Календари цепочки поставок и их влияние на сводное планирование
...
ms.locfileid: "363645"
---
# <a name="supply-chain-calendars-and-how-they-affect-master-planning"></a>Календари цепочки поставок и их влияние на сводное планирование
...
Дата поступления покупки показывает дату, когда вы получите товары. Она будет является открытой датой в календаре. Календарь, который будет приниматься во внимание, чтобы показать, в какие дни заказы на покупку могут быть получены, в порядке убывания приоритета: 
    ...
    2. Календарь группы покрытия
    3. Календарь склада для склада приемки
...
Обратите внимание, что календарь группы покрытия может быть задан на различных страницах и будет иметь приоритет в следующем порядке:
    ...
    2. Группа покрытия номенклатуры на странице **Покрытие номенклатуры**
    3. Группа покрытия номенклатуры по умолчанию на странице **Параметры сводного планирования**
...
Дата отгрузки спланированного заказа на перемещение указывает дату отгрузки товаров со склада "Со". Календари, используемые для указания доступной даты отгрузки, перечисляются в порядке приоритета: 
    ...
    2. Календарь группы покрытия (см. резервный порядок для этого календаря выше), если задан календарь склада, дата отгрузки будет являться открытой датой в этом календаре. Если календарь склада не задан, будет взят календарь группы покрытия. 
...
Календари, используемые для указания доступной даты получения, перечисляются в порядке приоритета: 
    ...
    2. Календарь склада "На", если задан календарь покрытия, дата поступления будет являться открытой датой в календаре. Если календарь группы покрытия не задан, будет взят календарь склада. 
