...
description: В этой статье описываются различные параметры, которые могут помочь улучшить производительность сводного планирования или устранения неполадок.
author: t-benebo
ms.date: 12/18/2019
ms.topic: article
ms.prod: ''
...
ms.locfileid: "8897613"
---
# <a name="improve-master-planning-performance"></a>Повышение производительности сводного планирования

[!include [banner](../includes/banner.md)]

В этой статье описываются различные параметры, которые могут помочь улучшить производительность сводного планирования или устранения неполадок. В нем содержатся сведения о параметрах и настройках, а также о рекомендуемых конфигурациях и действиях. Кроме того, в нем содержится сводка всех важных параметров, которые следует учитывать, когда выполняются продолжительные задания сводного планирования.

Эта статья предназначена для администраторов системы или ИТ-пользователей, обладающих возможностью устранения неполадок Он также предназначен для планировщиков производства или поставок, поскольку включает информацию о параметрах, которые относятся к требованиям бизнес-планирования. 

## <a name="parameters-related-to-master-planning-performance"></a>Параметры, которые относятся к производительности сводного планирования
...
Параметр **Количество потоков** позволяет настроить процесс сводного планирования, чтобы повысить производительность для конкретного набора данных. В этом параметре указывается общее число потоков, которые будут использоваться для выполнения сводного планирования. Это приводит к параллельному выполнению сводного планирования, что помогает уменьшить время выполнения. 
...
- **Если ваша отрасль — производство**: (Количество потоков) = (Количество спланированных заказов ÷ 1000)
- **В противном случае:** (Количество потоков) = (Количество номенклатур ÷ 1000)
...
> [!NOTE]
> Задание для параметра **Количество потоков** как **0** (ноль) увеличивает время выполнения сводного планирования. Поэтому рекомендуется всегда задавать значение, большее 0.
...
В общем случае рекомендуется увеличивать число задач, когда количество номенклатур очень велико (сотни тысяч). В противном случае число задач должно быть уменьшено. В следующих конкретных отраслях рассматриваются следующие рекомендации:
...
При просмотре журнала сеанса проанализируйте следующее:

- **Обновление** не должно занимать много времени (в целом оно должно занимать до 30 минут), однако оно однопотоковое.
...
## <a name="automatically-filter-by-items-with-direct-demand"></a>Автоматическая фильтрация по номенклатурам с прямым спросом

Чтобы улучшить время выполнения сводного планирования, вы можете включить только элементы с прямым спросом. Этот фильтр может использоваться только для полного выполнения сводного планирования без каких-либо других фильтров, применяемых в поле **Записи для добавления**. Сводное планирование с фильтрами будет игнорировать настройку **Автоматическая фильтрация по номенклатурам с прямым спросом**.

Поле **Автоматическая фильтрация по номенклатурам с прямым спросом** находится на странице **Параметры сводного планирования** и может использоваться как с настройками предварительной, так и последующей обработки.

### <a name="pre-processing"></a>Предварительная обработка
Параметр **Предварительная обработка: автоматическая фильтрация по номенклатурам с прямым спросом** гарантирует, что этап предварительной обработки сводного планирования включает только элементы, которые соответствуют по крайней мере одному из следующих условий:
  - Номенклатура имеет ожидаемый приход или расход, например, заказ на покупку, заказ на продажу, предложение, заказ на перемещение или производственный заказ. 
  - У номенклатуры есть покрытие номенклатуры с резервным запасом (минимальные запасы в наличии).
  - Для номенклатуры существует прогноз спроса после сегодняшнего дня.
  - Для номенклатуры существует прогноз поставки после сегодняшнего дня.
  - Номенклатура включает в себя любые строки непрерывности из модуля центра обработки вызовов, который еще не создан.

> [!NOTE]
> Номенклатура с физически доступными запасами в наличии не будет отображать проводку потребности из-за отсутствия спроса на номенклатуру.

### <a name="post-processing"></a>Последующая обработка
Параметр **Последующая обработка: автоматическая фильтрация по номенклатурам с прямым спросом** работает только в том случае, если вы зададите **Потребности по версии спецификации** в группах покрытия. В противном случае вам не нужно включать параметр. 

Перед началом этапа покрытия существует этап предварительного покрытия, в ходе которого будут повторно обработаны номенклатуры с параметром покрытия **Потребности по версии спецификации**. Это делается для того, чтобы элементы из требуемой версии спецификаций были запланированы. Элементы, которые считаются востребованными во время предварительной обработки, больше не имеют спроса и поэтому должны быть исключены из выполнения планирования.
...
- **Число задач в наборе задач помощников** — задано значение, большее **0** (нуля). (Используйте формулы, приведенные ранее в этой статье.)
- **Использование кэша** — задано значение **Максимум**, кроме случая, когда в системе не хватает памяти.
- **Количество заказов в наборе утверждения** — задано значение, большее **0** (нуля). (Используйте формулу, приведенную ранее в этой статье.)
