...
|-------------------------|-------------------|
| Заголовки заказов на продажу Dataverse | SalesOrders       |
| Строки заказа на продажу Dataverse   | SalesOrderDetails |
...
В таблице **Заказ** добавлены новые столбцы, которые отображаются на странице:

- **Хранится во внешней системе** — задайте значение **Да**, когда заказ поступает из Supply Chain Management.
- **Состояние обработки** — в этом столбце отображается статус обработки заказа в Supply Chain Management. Доступны следующие значения:
...
3. В столбце **Происхождение заказа на продажу** введите имя происхождения заказа на продажу, например **SalesOrder**.
4. В столбце **Описание** введите описание, например **Заказ на продажу из Sales**.
5. Установите флажок **Назначение типа источника**.
6. Задайте в столбце **Тип источника продаж** значение **Интеграция заказов на продажу**.
7. Нажмите **Сохранить**.
...
> [!NOTE]
> Столбцы **Условия оплаты**, **Условия фрахта**, **Условия поставки**, **Способ доставки** и **Режим доставки** не являются частью сопоставлений по умолчанию. Чтобы сопоставить эти столбцы, необходимо настроить преобразование значений, характерное для данных в организациях, между которыми синхронизируется таблица.

На следующем рисунке показан пример сопоставления шаблона в интеграции данных.

> [!NOTE]
> Сопоставление показывает, какие данные столбцов будут синхронизированы из Sales в Supply Chain Management или из Supply Chain Management в Sales.
