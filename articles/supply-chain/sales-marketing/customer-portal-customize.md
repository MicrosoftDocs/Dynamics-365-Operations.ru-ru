...
description: В этой статье объясняется, как настроить клиентский портал после его добавления в систему.
author: Henrikan
ms.date: 04/22/2020
ms.topic: article
ms.prod: ''
...
В этой статье описываются отдельные страницы, доступные на готовом клиентском портале. Даны объяснения, что представляют собой страницы и как их можно настраивать.
...
Следующие статьи содержат общие сведения о порталах Power Apps и способах настройки порталов:

- [Работа с шаблонами](/powerapps/maker/portals/work-with-templates) — в этой статье представлен общий обзор принципа работы порталов Power Apps и способов выполнения простых настроек порталов.
- [Управление содержимым портала](/dynamics365/portals/manage-portal-content) — в этой статье объясняется, как управлять и настраивать содержимое портала.
- [Правка CSS](/powerapps/maker/portals/edit-css) — в этой статье указано, как выполнить более сложную настройку пользовательского интерфейса (UI) портала.
- [Создать тему для своего портала](/dynamics365/portals/create-theme) — эта статья поможет создать тему пользовательского интерфейса для своего портала.
- [Простое создание и представление содержимого портала](/dynamics365/portals/create-expose-portal-content) — эта статья поможет управлять основными данными и таблицами, используемыми для портала.
- [Настройка контакта для использования на портале](/powerapps/maker/portals/configure/configure-contacts) — в этой статье объясняется, как создавать и настраивать роли пользователей и как работают безопасность и аутентификация на порталах Power Apps.
- [Настройка примечаний для форм таблиц и веб-форм на порталах](/powerapps/maker/portals/configure-notes) — в этой статье объясняется, как добавлять документы и дополнительные хранилища на свой портал.
- [Обработка ошибок для веб-сайта портала](/powerapps/maker/portals/admin/view-portal-error-log) — в этой статье объясняется, как просмотреть журналы ошибок портала и сохранить их в учетной записи хранилища BLOB-объектов Microsoft Azure.
...
Для таблицы "заказ на продажу" автоматически устанавливаются следующие поля:
...
Не удаляйте следующие столбцы из таблицы "заказ на продажу" в Microsoft Dataverse, поскольку они необходимы для создания заказа на продажу в двойной записи:
...
Для номенклатур требуются следующие столбцы:
