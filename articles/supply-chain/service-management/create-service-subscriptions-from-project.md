---
title: Создание подписок на обслуживание из проекта
description: Можно создать подписку на обслуживание из проекта в модуле "Управление и учет по проектам".
author: ShylaThompson
ms.date: 05/01/2018
ms.topic: article
ms.prod: ''
ms.technology: ''
ms.search.form: SMASubscriptionTable
audience: Application User
ms.reviewer: kamaybac
ms.custom: ''
ms.assetid: ''
ms.search.region: Global
ms.author: kamaybac
ms.search.validFrom: 2016-02-28
ms.dyn365.ops.version: AX 7.0.0
ms.openlocfilehash: a9b92bd317aa281c43e65dc16f871468b6e3d491
ms.sourcegitcommit: 0e8db169c3f90bd750826af76709ef5d621fd377
ms.translationtype: HT
ms.contentlocale: ru-RU
ms.lasthandoff: 04/01/2021
ms.locfileid: "5819120"
---
# <a name="create-service-subscriptions-from-a-project"></a>Создание подписок на обслуживание из проекта    

[!include [banner](../includes/banner.md)]


Можно создать подписку на обслуживание из проекта в модуле **Управление и учет по проектам**.

1.  Щелкните **Управление и учет по проектам** \> **Общее** \> **Проекты** \> **Все проекты**. Откройте проект, для которого необходимо создать подписку.

2.  В форме **Проекты**, в разделе **Панель операций**, на вкладке **Управление** щелкните **Обслуживание** \> **Подписка на обслуживание**.

3.  В форме **Подписка**, в поле **Подписка на обслуживание** введите уникальный идентификационный номер. Номер проекта будет автоматически введен в подписку для выбранного проекта.

4.  Введите остальные сведения в форму.

Теперь для этой подписки можно создавать транзакции по подписке.


> [!NOTE]
> <P>Если для группы подписок настроена номерная серия, код подписки будет создан автоматически при указании группы подписок.</P>



## <a name="see-also"></a>См. также

[Цены продажи подписки](subscription-sales-prices.md)

  




[!INCLUDE[footer-include](../../includes/footer-banner.md)]