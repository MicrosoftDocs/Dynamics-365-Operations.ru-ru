...
[Создание накладных с произвольным текстом](../accounts-receivable/create-free-text-invoice-new.md)
[Создание шаблона накладной с произвольным текстом](../accounts-receivable/create-free-text-invoice-template-new.md)
...
>[!NOTE]
> В случае прерывания системы во время предварительного процесса выставления накладных продаж, предварительная накладная может быть исключена Потерянную предварительную накладную можно удалить, запустив периодическое задание **Удалить вручную предварительную накладную** Перейдите в меню **Продажи и маркетинг > Периодические задания > Очистить > Удалить вручную предварительные накладные**.

## <a name="using-sales-order-customer-invoice-data-entities"></a>Использование информационных объектов накладных клиента для заказа на продажу
Можно использовать информационные объекты для импорта и экспорта сведений о накладной клиента для заказа на продажу. Существуют разные объекты для информации в заголовке заказа на продажу и в строках накладной по продаже.

Следующие объекты доступны для информации в заголовке накладной по продаже:

- Объект **Заголовок журнала накладных по продаже** (SalesInvoiceJournalHeaderEntity)
- Объект **Заголовки заказа по продаже V2** (SalesInvoiceHeaderV2Entity)

Рекомендуется использовать объект **Заголовок журнала накладных по продаже**, поскольку он предоставляет более производительный варианта по импорту и экспорту заголовков продаж. Этот объект не содержит столбец **Сумма налога** (INVOICEHEADERTAXAMOUNT), который представляет значение налога в заголовке накладной на продажу. Если бизнес-сценарий требует эту информацию, используйте объект **Заголовки накладной заказа на продажу V2**, чтобы импортировать и экспортировать сведения из заголовка накладной на продажу.

Следующие объекты доступны для информации в строках накладной по продаже:

- Объект **Строки накладной клиента** (BusinessDocumentSalesInvoiceLineItemEntity)
- Объект **Строки накладной заказа на продажу V3** (SalesInvoiceLineV3Entity)

При определении того, какой объект строки должен использоваться для экспорта, следует подумать о том, будет ли использоваться полная принудительная отправка или пошаговая принудительная отправка. Кроме того, рассмотрим состав данных. Объект **Строки накладной по продаже V3** поддерживает более сложные сценарии (например, сопоставление с полями запасов). Он также поддерживает сценарии с экспортом полной принудительной отправки. Для пошаговой принудительной отправки рекомендуется использовать объект **Строки накладной клиента**. Этот объект содержит гораздо более простое составление данных, чем объект **Строки накладной по продаже V3** и является предпочтительным, особенно если интеграция полей запасов не является обязательной. Из-за различий в поддержке сопоставления между объектами строки объект **Строки накладной клиента** обычно имеет более высокую производительность, чем объект **Строки накладной по продаже V3**.
...
## <a name="split-sales-order-invoices-by-site-and-delivery-information"></a>Разбиение накладных заказа на продажу по сайту и сведениям о поставке
Можно настроить разбивку накладных клиента для заказа на продажу по сайту или по адресу доставки на вкладке **Суммарное обновление** на странице **Параметры модуля расчета с клиентами**. 
 - Выберите параметр **Разбивка на основе сайта счета**, чтобы создавать одну накладную на место при разноске. 
 - Выберите параметр **Сведения о поставке**, чтобы создавать одну накладную для каждого адреса поставки строки заказа на продажу при разноске. 

## <a name="post-to-revenue-account-for-sales-order-lines-that-have-no-price-and-no-cost"></a>Разноска строк заказов на продажу, не содержащих цены и стоимости, на счет выручки
У вас будет возможность обновить счет **Выручка** в **Главной книге** для строк заказа на продажу, не содержащих цены и стоимости. Для настройки или просмотра этих сведений перейдите к параметру **Разносить строки накладных по заказам на продажу с нулевой ценой и нулевой стоимостью на счет выручки** на вкладке **Главная книга и налог** на странице **Параметры модуля расчетов с клиентами**. (**Расчеты с клиентами > Настройка > Параметры модуля расчетов с клиентами**). Выберите **Да**, чтобы обновить счет **Выручка** для строк накладной заказа на продажу, не содержащих цены и стоимости. Если выбран этот параметр, ваучер будет содержать 0,00 записей для типов разноски **Сальдо клиента** и **Выручка**. Счет выручки определяется на странице параметров **Разноска запасов** на вкладке определения счета **Заказ на продажу**. Если этот параметр не выбран, строки, не содержащие информации о цене или стоимости, не будут разноситься на счет **Выручка**. Вместо этого ваучер будет содержать запись 0,00 для типа разноски **Сальдо клиента**.

## <a name="line-creation-sequence-number-information"></a>Сведения о номере последовательности создания строк
При разноске строк накладной клиента имеется возможность создания последовательных номеров создания строк. Порядковые номера создания строк назначаются во время процесса разноски. Разрешив непоследовательную нумерацию, можно повысить производительность разноски накладных клиента. Порядковые номера создания строк могут использоваться в интеграционных средствах сторонних производителей, ожидающих последовательный порядок. Проконсультируйтесь с ИТ-отделом по поводу любых расширений, которые могут интегрироваться с порядковыми номерами создания строк.

Для настройки или просмотра этих сведений на странице **Параметры расчетов с клиентами** на вкладке **Обновления** установите флажок **Назначать последовательные номера строк при разноске строк накладной клиента**:

- Установите для этого параметра значение **Нет**, чтобы использовать непоследовательные номера для порядковых номеров создания строк.
- Установите для этого параметра значение **Да**, чтобы использовать последовательную нумерацию. Необходимо установить для этого параметра значение **Да** для юридических лиц, имеющих основной адрес в Италии. Кроме того, необходимо задать значение **Да**, если предоставление доступа **CustInvoiceTransRandLineCreationSeqNumFlight** отключено.
