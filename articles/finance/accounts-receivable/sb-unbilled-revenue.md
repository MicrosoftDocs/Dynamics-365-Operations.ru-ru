...
description: В этой статье объясняется, как настроить номенклатуры и счета для использования функции выручки по невыставленным счетам в выставлении счетов по подпискам.
...
ms.locfileid: "9644177"
---
# <a name="unbilled-revenue"></a>Выручка по невыставленным счетам

В этой статье описывается функция выручки по невыставленным счетам, которая позволяет включать суммы для всех графиков выставления счетов в балансовый отчет. Эти суммы включаются в счет выручки по невыставленным счетам и корр. счет выручки по невыставленным счетам, и счета по контракту выставляются через периодические платежи.
...
Начиная с версии 10.0.29 новый параметр добавляется к параметрам повторяющегося выставления счетов по контракту. Если выбрано значение "Да" для параметра **Использовать корреспондентские счета, по которым не выставлен счет** будут включены два новых счета в разделе **Настройка выручки по невыставленным счетам**. Счет зачета невыставленной выручки и корреспондентский счет скидки, по которому не выставлен счет становятся доступными и их лучше использовать при создании графиков выставления счетов в валюте, отличной от валюты учета. Использование корр. счетов позволяет сторнировать счет выручки по невыставленным счетам и счет скидки по невыставленным счетам с тем же обменным курсом, что и в исходных записях. Исходный процесс **Создать запись в журнале** остается такой же с дебетом по выручке по невыставленным счетам и кредитом по выручке. При использовании скидки исходная запись в журнале остается такой же с дебетом по скидке и скидке по невыставленным счетам. 

В этом примере показано использование выручки по невыставленным счетам для признания всей суммы контракта в балансовом отчете как выручку по невыставленным счетам. Другая сторона записи — это выручка или выручка будущих периодов. Когда вы выставите счет заказчику, выручка по невыставленным счетам отменяется. Признание выручки произойдет либо во время выставления накладной, либо в соответствии с настроенным графиком признания выручки будущих периодов.
...
| Организация | Сумма дебета | Сумма кредита |
|---|---|---|
| Счет выручки по невыставленным счетам | 300,00 долларов США | |
| Счет выручки | | 300,00 долларов США |

| Организация | Сумма дебета | Сумма кредита |
|---|---|---|
| Счет выручки по невыставленным счетам | 90,00 долларов США | |
| Выручка будущих периодов | | 90,00 долларов США |

Для контракта требуется, чтобы счет для клиента был создан в начале каждого года. Для создания накладной используется процесс **Создать накладную**. Когда создается счет, выполняется разноска по следующему ваучеру.

| Организация| Сумма дебета | Сумма кредита |
|---|---|---|
| Счет выручки по невыставленным счетам | | 130,00 долларов США |
| Дебиторская задолженность | 130,00 долларов США | |

Эта же запись журнала будет создаваться накладными, которые разносятся в начале следующих двух лет. Счет выручки по невыставленным счета каждый год сокращается в процессе **создания накладной**. Корреспондентский счет, по которому не выставлен счет, используется, чтобы сбалансировать счет выручки по невыставленным счетам при использовании разных обменных курсов. 

На последнем шаге каждый месяц создается запись в журнале первоначального признания для того, чтобы различать в учете выручку будущих периодов от платы за обслуживание. Запись журнала может быть создана с помощью страницы **Обработка признания**. Кроме того, она может быть создана путем выбора команды **Признать** для строк на страницах **График отсрочки**.

| Счет ГК | Сумма дебета | Сумма кредита |
|---|---|---|
| Выручка будущих периодов | 2,50 доллара США | |
| Выручка | | 2,50 доллара США |
...
| Счет выручки по невыставленным счетам (401250) | 1465,26 долларов США | |
| Счет выручки будущих периодов (250600) | | 1465,26 долларов США |
| **Запись в журнале для номенклатуры 0021** | | | 
| Счет выручки по невыставленным счетам (401250) | 274,74 долл. США | |
| Счет выручки будущих периодов (250600) | | 274,74 долл. США |
| **Счет-фактура** | | |
| Счет выручки по невыставленным счетам | | 1465,26 долларов США |
| Счет выручки по невыставленным счетам | | 274,74 долл. США |
| Cчет дебиторской задолженности (130100) | 1488,16 долл. США | |
