...
> [!NOTE]
> Чтобы уменьшить время, необходимое для создания снимка временных срезов, включите следующие функции в рабочей области **Управления функциями**: 
> - **Улучшение производительности распределения по срокам клиентов** 
> - **Повышение эффективности обработки сроков оплаты по клиентам с помощью пулов клиентов**  
>Если обе функции включены, **Пулы клиентов** могут использоваться при создании снимка временных срезов. 
...
- **Включить клиентов с нулевым сальдо** — установите для этого параметра значение **Да**, чтобы включить всех клиентов, независимо от их сальдо. Если включены все клиенты, рекомендуется включить обе функции: **Повышение эффективности обработки сроков оплаты по клиентам** и **Повышение эффективности обработки сроков оплаты по клиентам с помощью пулов клиентов**. Установите для этого параметра значение **Нет**, чтобы включить только тех клиентов, у которых есть сальдо. Этот параметр поможет повысить производительность, поскольку будут пропускаться клиенты, по которым отсутствует сальдо.
- **Обходить расчеты кредитного лимита во время распределения по срокам** — Если задать для этого параметра значение **Да** процесс распределения по срокам не будет пересчитывать сумму **промежуточного итога отборочной накладной**, сумму **промежуточного итога открытого заказа** и **доступный кредит** для каждого клиента. Эти сальдо отображаются на странице **Сальдо по срокам**, в информационном поле в разделе **Кредитный лимит**. Для ускорения работы в процессе распределения по срокам задайте для этого параметра значение **Да**. Установите значение **Нет** для расчета сальдо при выполнении процесса распределения по срокам. 
...
> [!NOTE]
> Чтобы улучшить производительность пакета при создании снимков временных срезов, введите число в поле **Максимальное число пакетных задач** пакета на экспресс-вкладке **Коллекции по умолчанию** на вкладке **Коллекции** в окне **Параметры модуля расчетов с клиентами**. В поле **Распределить сальдо клиента по срокам** рекомендуется начинать со значения от **12** до **20**, а затем корректировать это значение для оптимизации обработки данных для вашего сценария. Не рекомендуется задавать значение больше **30**, так как это может повлиять на производительность. 
