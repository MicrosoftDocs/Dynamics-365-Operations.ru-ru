---
title: Права доступа для контролеров объектов затрат
description: В этой статье даны сведения о правах доступа для контролеров объектов затрат.
author: AndersGirke
ms.date: 06/24/2017
ms.topic: article
ms.prod: ''
...
ms.locfileid: "8903199"
---
# <a name="access-rights-for-cost-object-controllers"></a>Права доступа для контролеров объектов затрат
...
| Контролер объектов затрат  | Участники группы |

В этой статье объясняется, каким образом выполняется назначение роли **Контролер объектов затрат** менеджеру.

Когда роль **контролера объекта затрат** назначается менеджеру, менеджер может выполнять следующие задачи:
...
> [!NOTE]
> Роль **контролера объекта затрат** не управляет, к каким объектам затрат может получить доступ и просмотр пользователь. Безопасность на уровне строк предоставляется через иерархии аналитик и иерархию списков доступа.
...
> - В этой статье показаны необходимые компоненты, которые должны присутствовать, чтобы можно было использовать рабочую область **Управление затратами**.
