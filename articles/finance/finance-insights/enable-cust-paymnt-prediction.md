...
description: В этой статье объясняется, как включить и настроить функцию прогнозирования платежей клиентов в Finance Insights.
...
ms.locfileid: "8898217"
---
# <a name="enable-customer-payment-predictions"></a>Включение прогнозов платежей клиентов

[!include [banner](../includes/banner.md)]

В этой статье объясняется, как включить и настроить функцию прогнозирования платежей клиентов в Finance Insights. Функция включается в рабочей области **Управление функциями** и вводятся параметры конфигурации на странице **конфигурация Finance Insights**. Кроме того, в этой статье содержатся сведения, которые могут помочь эффективно использовать эту функцию.

> [!NOTE]
> Перед выполнением следующих шагов обязательно выполните предварительные шаги из статьи [Настройка для Finance Insights](configure-for-fin-insites.md).
...
2. Настройте функцию анализа платежей клиентов:
   ...
   > [!NOTE]
   > Функция **Прогнозы платежей клиентов** требует более 100 проводок в течение предыдущих шести-девяти месяцев. Проводки могут включать накладные с произвольным текстом, заказы на продажу и платежи клиентов. Эти данные должны быть распределены между параметрами **Вовремя**, **С задержкой** и **С очень большой задержкой**.    
