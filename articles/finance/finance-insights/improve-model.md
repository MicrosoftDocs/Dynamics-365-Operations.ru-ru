...
При выборе полей, включаемых в модель, имейте в виду, что список включает все доступные поля в сущности Common Data Service, сопоставленной с данными в Azure Data Lake. Некоторые из этих полей **не должны** быть выбраны. Поля, которые не должны быть выбраны, попадают в одну из трех категорий:

- Поле является обязательным для сущности Common Data Service, но в Data Lake нет поддерживающих его данных.
...
 
### <a name="invoice-common-data-model-entity"></a>Сущность накладной Common Data Model

На следующем рисунке показаны поля, которые доступны для сущности накладной.

[![Доступные поля для сущности накладной](./media/available-fields.png)](./media/available-fields.png)
...
### <a name="customer-common-data-model-entity"></a>Сущность клиента Common Data Model

На следующем рисунке показаны поля, которые доступны для сущности клиента.

[![Доступные поля для сущности клиента](./media/related-entities.png)](./media/related-entities.png)
