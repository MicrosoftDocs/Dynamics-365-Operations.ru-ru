...
- **Период консолидации** — поля в этом разделе используются для определения периода консолидации.
    ...
    - **Выбрать сумму консолидации из** — используйте это поле, чтобы указать, будут ли суммы в валюте учета или суммы в валюте отчетности из исходных компаний использоваться для обновления сумм в валюте учета консолидированной компании.

        - Выберите **Валюта учета**, чтобы использовать суммы в валюте учета из исходных компаний для обновления сумм в валюте учета в консолидированной компании. Если выбрано это значение, используйте поле **Консолидировать валюту учета**, чтобы определить, как будут рассчитываться валюты учета в консолидированной компании.
        - Выберите **Валюта отчетности**, чтобы использовать суммы в валюте отчетности из исходных компаний для расчета сумм в валюте учета в консолидированной компании.

            - Если валюта отчетности из исходной компании совпадает с валютой учета консолидированной компании, суммы в валюте отчетности копируются из исходной компании в консолидированную компанию.
            - Если валюта отчетности из исходной компании отличается от валюты учета консолидированной компании, значения пересчитываются с помощью сведений об обмене, которые определены на вкладке **Пересчет в иностранную валюту** на этой странице для расчета значений консолидированной компании.

    - **Консолидировать валюту учета** — это поле доступно только в том случае, если в поле **Выбрать сумму консолидации из** задано значение **Валюта учета**. Используйте его, чтобы указать, должны ли суммы в валюте учета из исходных компаний пересчитываться через валютные курсы или копироваться в консолидированную компанию. Выберите **Использовать пересчет валют** для использования данных валютного курса, которые определены на вкладке **Пересчет в иностранную валюту** для расчета сальдо счетов консолидации. Выберите **Использовать сумму в валюте учета**, чтобы скопировать суммы в валюте учета из исходных компаний в консолидированную компанию.

        - Если валюта учета из исходной компании совпадает с валютой учета консолидированной компании, суммы в валюте копируются из исходной компании в консолидированную компанию.
        - Если валюта учета из исходной компании отличается от валюты учета консолидированной компании, значения пересчитываются с помощью сведений об обмене, которые определены на вкладке **Пересчет в иностранную валюту** для расчета значений консолидированной компании.
...
На вкладке **Пересчет в иностранную валюту** необходимо определить юридическое лицо, счет, тип валютного курса и курс. Если консолидированная компания сопоставляется с другими счетами ГК, отличными от исходной компании, счет ГК консолидированной компании должен быть введен в поля **Начальная дата** и **Конечная дата**, а не в счета ГК исходной компании. Для каждой строки юридического лица и счетов ГК в поле **Применить валютный курс из** будут доступны три варианта:

- **Дата консолидации** — дата, которая определена в поле **Период консолидации до** на вкладке **Критерии** для консолидации, будет использоваться для того, чтобы получить валютный курс. Этот курс эквивалентен спот-курсу или курсу на конец месяца. Вы увидите предварительный просмотр курса, однако редактировать его не сможете.
