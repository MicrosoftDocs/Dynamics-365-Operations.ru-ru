...
description: Эта статья описывает возможности в Microsoft Dynamics 365 Finance, которые могут облегчить обработку общего журнала, а также обеспечить получение правильных данных и гарантировать, что внутреннее управление не скомпрометировано.
author: kweekley
ms.date: 02/28/2022
ms.topic: article
ms.prod: ''
...
Эта статья описывает возможности, которые могут облегчить обработку общего журнала, а также обеспечить получение правильных данных и гарантировать, что внутреннее управление не скомпрометировано.  
...
Возможность быстрого удаления всех строк журнала включается в журнале в пункте **Функции** > **Удалить строки журнала**. Чтобы включить эту функцию, на странице **Управление функциями** выберите команду **Удалить оптимизации производительности журнала**. Эта функция влияет на расширения в таблице **LedgerJournalTrans** и ее метод **Delete**, так как набор строк удаляется без вызова метода **Delete** для каждой строки. 
