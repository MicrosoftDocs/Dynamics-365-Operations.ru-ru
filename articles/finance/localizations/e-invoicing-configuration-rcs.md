...
С помощью возможностей настройки электронного выставления накладных поможет вам обеспечить выполнение бизнес-требований и нормативных требований для электронных накладных без написания кода. И в сценариях, в которых электронные накладные должны быть электронно утверждены веб-службой, функции конфигурации также помогают выполнить требования для обмена сообщениями с веб-службой без написания какого-либо кода.
...
### <a name="generally-available-features"></a>Обычно доступные возможности

В следующей таблице показаны функции электронного выставления накладных, которые в обычно доступны.
...
| Бразилия         | Бразильский импорт NF-e из электронной почты (BR) | Модель 55 финансового документа |
...
| Европа         | Накладная поставщика PEPPOL                | Импорт накладных поставщиков PEPPOL |
| Саудовская Аравия   | Электронная накладная Саудовской Аравии (SA)| Накладные по продаже и по проекту |
...
#### <a name="saudi-arabian-sa-electronic-invoice-sales-and-project-invoices-for-saudi-arabia"></a>Электронная накладная для Саудовской Аравии (SA): накладные по продаже и накладные по проектам для Саудовской Аравии

- Электронная накладная по продаже (SA)
- Электронная накладная по проекту (SA)

В дополнение к конфигурациям формата электронной отчетности, доступным в готовом виде для работы со службой электронных накладных, можно также создать собственные конфигурации формата электронной отчетности. Однако конфигурации форматов, созданные для использования с функциями электронных накладных, не поддерживают прямую ссылку на таблицы Finance или Supply Chain Management, либо на любые из соответствующих метаданных. Поддерживаются только ссылки на сопоставление модели электронной отчетности.
...
| Подпись документа xml                             | Подписание XML-документов цифровой подписью.                                   | Общедоступно  |
| Подпишите документ JSON для налогового органа Египта | Подписание документов JSON цифровой подписью для налогового органа Египта.       | Общедоступно  |
| Интеграция с египетской службой ETA           | Связь с налоговыми органами Египта.                                     | Общедоступно  |
| Вызов бразильской службы SEFAZ                  | Интеграция с бразильской службой SEFAZ для отправки финансовых документов.       | Общедоступно  |
...
Правила применимости являются настраиваемыми предложениями, которые определены на уровне функций электронного выставления накладных. Правила настраиваются для создания контекста для выполнения функций электронного выставления накладных с помощью набора возможностей электронного выставления накладных.

Когда бизнес-документ из Финансы или Supply Chain Management отправляются для электронного выставления накладных, в бизнес-документе отсутствует явная ссылка, которая позволяет настроить для электронного выставления накладных возможность вызова определенной функции электронного выставления накладных для обработки отправки.

Тем не менее, при правильной настройке бизнес-документ содержит необходимые элементы, которые позволяют электронному выставлению накладных решить, какая функция электронного выставления накладных должна быть выбрана, а затем создаст электронную накладную.

Правила применимости позволяют набору возможностей электронного выставления накладных найти конкретные функции электронного выставления накладных, которые должны использоваться для обработки отправки. Это выполняется путем сопоставления содержимого из отправленного бизнес-документа с предложениями из правил применимости.

Например, две функции электронного выставления накладных с соответствующими правилами применимости разворачиваются в наборе возможностей электронного выставления накладных.

| Функция электронного выставления накладных | Правила применимости        |
|------------------------------|--------------------------- |
| A                            | <p>Страна = Бразилия</p><p>и</p><p>Юридическое лицо = BRMF</p>  |
| млрд                            | <p>Страна = Мексика</p><p>и</p><p>Юридическое лицо = MXMF</p>  |

Если бизнес-документ из Финансы или Supply Chain Management передается в набор возможностей электронного выставления накладных, в бизнес-документ будут включены следующие атрибуты, заполненные как:

- Страна = Бразилия
- Юридическое лицо = BRMF

Набор возможностей электронного выставления накладных выберет функцию электронного выставления накладных **A**, чтобы обработать отправку и создать электронную накладную.

Аналогичным образом, если бизнес-документ содержит:

- Страна = Мексика
- Юридическое лицо = MXMF

Для создания электронной накладной выбрана функция электронного выставления накладных **B**.

Настройка правил применимости не может быть неоднозначной. Это означает, что две или более функций электронного выставления накладных не могут иметь одинаковых предложений, в противном случае это приведет к отсутствию выбора. Если имеется дублирование функций электронного выставления накладных, для предотвращения неоднозначности используйте дополнительные предложения, позволяющие настроить возможность электронного выставления накладных для различения двух функций электронного выставления накладных.

Например, рассмотрим функцию электронного выставления накладных **C**. Эта функция является копией функции электронного выставления накладных **A**.

| Функция электронного выставления накладных | Правила применимости        |
|------------------------------|--------------------------- |
| A                            | <p>Страна = Бразилия</p><p>и</p><p>Юридическое лицо = BRMF</p>  |
| C                            | <p>Страна = Бразилия</p><p>и</p><p>Юридическое лицо = BRMF</p>  |

В этом примере функция **C** находится перед отправкой бизнес-документа, который содержит следующее:

- Страна = Бразилия
- Юридическое лицо = BRMF

Возможность электронного выставления накладных не может отличать, какую электронную накладную необходимо использовать для обработки отправки, поскольку отправки содержат в точности одинаковые условия.

Чтобы создать различие между двумя функциями через правила применимости, необходимо добавить в одну из функций новое предложение, позволяющее настроить функцию электронного выставления накладных для выбора нужной функции электронного выставления накладных.

| Функция электронного выставления накладных | Правила применимости        |
|------------------------------|--------------------------- |
| A                            | <p>Страна = Бразилия</p><p>и</p><p>Юридическое лицо = BRMF</p>  |
| C                            | <p>Страна = Бразилия</p><p>и</p><p>Юридическое лицо = BRMF</p><p>и</p><p>Модель=55</p>  |

Для поддержки создания более сложных предложений доступны следующие ресурсы:
...
Возможность группирования и разгруппирования предложений.
Пример выглядит следующим образом.

| Функция электронного выставления накладных | Правила применимости        |
|------------------------------|--------------------------- |
| C                            | <p>Страна = Бразилия</p><p>и</p><p>( Юридическое лицо = BRMF</p><p>или</p><p>Модель=55)</p>  |
