...
3. Выберите **Развернуть**, затем выберите один или оба из следующих вариантов, чтобы определить цель развертывания:
    ...
    - **Подключенное приложение** — это необязательно, но должно использоваться, если вы хотите, чтобы конфигурация, предоставляемая настройкой приложения, записывалась в экземпляр Microsoft Dynamics 365 Finance или Dynamics 365 Supply Chain Management, ранее связанный с ней. Пропуск этого типа развертывания требует ручной настройки параметров, определенных в настройке приложения в Finance или Supply Chain Management.
    - **Среда службы** — при этом версия функции электронного выставления накладных развертывается в среде службы. После этого модуль электронного выставления накладных готов к получению и обработке электронных документов, которые отправляются из Finance или Supply Chain Management.
