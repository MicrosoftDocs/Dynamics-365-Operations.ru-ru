...
- Необходим проект LCS, включающий версию 10.0.17 или более позднюю версию Microsoft Dynamics 365 Finance или Dynamics 365 Supply Chain Management. Кроме того, эти приложения должны быть развернуты в одном из следующих географических регионов Azure:
...
6. В поле **Код клиента AAD** введите код клиента вашей учетной записи подписки Azure. Указанный клиент Azure Active Directory (Azure AD) должен быть тем же клиентом, который используется для RCS.
7. Ознакомьтесь с условиями и установите флажок.
8. Выберите **Установить**. Установка может занять несколько минут.
...
2. В рабочей области **Функции глобализации** в разделе **Связанные параметры** выберите **Параметры электронной отчетности**.
3. На вкладке **Электронное выставление накладных** в поле **Универсальный код ресурса (URI) конечной точки службы** введите соответствующую конечную точку службы для своего географического расположения Azure, как показано в следующей таблице.
...
3. На странице **Настройки среды** на панели действий выберите **Среды службы**, затем выберите **Параметры Key Vault**.
...
1. На странице **Настройка среды** на панели действий выберите **Подключенные приложения**.
...
2. На вкладке **Электронное выставление накладных** в поле **URL-адрес конечной точки** введите соответствующую конечную точку службы для своего географического расположения Azure, как показано в следующей таблице.
...
### <a name="enable-flighting-keys-for-finance-or-supply-chain-management-version-10017"></a>Включение ключей фокус-тестирования для Finance или Supply Chain Management 10.0.17 версии
