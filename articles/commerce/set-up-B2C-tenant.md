...
После создания клиента B2C вы создадите приложение B2C в рамках клиента, чтобы взаимодействовать с действиями Commerce.

Для создания приложения B2C выполните следующие действия.

1. На портале Azure выберите **Приложения**, затем выберите **Добавить**.
1. В поле **Имя** введите имя нужного приложения AAD B2C.
1. В разделе **Веб-приложение/веб-интерфейс** для параметра **Включить веб-приложение/веб-интерфейс API** выберите **Да**.
1. Для параметра **Разрешить неявный поток** выберите значение **Да** (значение по умолчанию).
1. В разделе **URL-адрес ответа** введите выделенные URL-адреса ответа. Информацию об URL-адресах ответа и порядке форматирования их здесь см. в разделе [URL-адреса ответа](#reply-urls).
1. Для параметра **Включить собственный клиент** выберите значение **Нет** (значение по умолчанию).
1. Выберите **Создать**.

### <a name="reply-urls"></a>URL-адреса ответов

URL-адреса ответа важны, поскольку они позволяют создать белый список доменов возврата, когда ваш сайт вызывает Azure AD B2C для проверки подлинности пользователя. Это позволяет вернуть пользователя, прошедшего проверку подлинности, в домен, с которого он выполняет вход (домен вашего сайта). 
...
1. На вкладке **Рекомендовано** выберите **Регистрация и вход**.
...
1. На портале Azure выберите **Потоки пользователей (политики)** в левой области переходов.
1. На странице **Azure AD B2C – Потоки пользователей (политики)** выберите пункт **Создать поток пользователей**.
1. На вкладке **Рекомендовано** выберите **Редактирование профиля**.
...
1. В области **Поставщики удостоверений** выберите **Вход в локальную учетную запись**.
...
1. На портале Azure выберите **Потоки пользователей (политики)** в левой области переходов.
1. На странице **Azure AD B2C – Потоки пользователей (политики)** выберите пункт **Создать поток пользователей**.
1. На вкладке **Рекомендовано** выберите **Сброс пароля**.
...
- [Azure AD (один клиент)](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-setup-oidc-azure-active-directory)
- [Учетная запись Майкрософт](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-setup-msa-app)
...
1. В поле поиска Commerce выполните поиск **Номерные серии** (Администратор организации > Номерные серии).
1. В области действий выберите **Изменить** в разделе **Ведение**.
1. На экспресс-вкладке **Общие** выберите **Нет** для параметра **Вручную**.
1. На панели операций выберите **Сохранить**. 
...
1. В метаданных скопируйте URL-адрес издателя поставщика удостоверений (значение для **"издатель"**).
    - Пример: ``https://login.fabrikam.com/073405c3-0113-4f43-b5e2-df01266e24ae/v2.0/``.
...
1. Введите следующие необходимые пункты в отображаемой форме, используя значения из клиента B2C и приложения. Поля, которые не являются обязательными (без звездочки), могут быть оставлены пустыми.
    ...
    - **Имя клиента**: имя вашего клиента B2C, например "Fabrikam".
...
Дополнительную документацию Azure AD B2C по миграции клиентов см. в разделе [Миграция пользователей в Azure Active Directory B2C](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-user-migration).

### <a name="custom-policies"></a>Пользовательские политики

Дополнительные сведения о настройке взаимодействий Azure AD B2C и лежащих в основе потоках политик, предлагаемых стандартными политиками B2C, см. раздел [Пользовательские политики в Azure Active Directory B2C](https://docs.microsoft.com/azure/active-directory-b2c/active-directory-b2c-overview-custom). 
