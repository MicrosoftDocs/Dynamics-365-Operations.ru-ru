...
description: В этой теме описывается, как настроить метод платежа для счета клиента в Microsoft Dynamics 365 Commerce. Также описывается, как кредитные лимиты влияют на запись платежей в кредит на сайтах электронной коммерции бизнес-бизнес (B2B).
author: josaw1
ms.date: 02/16/2022
ms.topic: article
ms.prod: ''
...
В этой теме описывается, как настроить метод платежа для счета клиента в Microsoft Dynamics 365 Commerce. Также описывается, как кредитные лимиты влияют на запись платежей в кредит на сайтах электронной коммерции бизнес-бизнес (B2B).
...
В настоящее время сайты B2B, где свойство **Обязательный кредитный лимит** включено, имеют дополнительные функции. Если это свойство включено в записи клиента, когда клиент размещает заказ, сайт B2B не позволяет ему использовать метод платежа в кредит, чтобы заплатить больше оставшегося кредитного сальдо. Например, если оставшееся кредитное сальдо клиента равно $1 000, но заказ стоит $1 200, клиент может заплатить только $1 000 с помощью метода "в кредит". Для выплаты сальдо они должны использовать другой способ оплаты. Если для записи клиента свойство **Обязательный кредитный лимит** отключено, клиент может заплатить любую сумму, используя метод платежа в кредит. Однако даже в том случае, когда клиент может разместить заказы, система не разрешит выполнение заказов, если они превышают кредитный лимит. Если необходимо проверить кредитный лимит для всех клиентов, которые имеют право на платежи в кредит, рекомендуется настроить для свойства **Тип кредитного лимита** значение **Сальдо + отборочная накладная или лист продукции**, а для свойства **Обязательный кредитный лимит** задать значение **Нет**.
...
Свойство **Исключение из управления кредитом**, которое предотвращает блокировку заказов на продажу в Commerce, находится в заголовке заказа на продажу (**Розничная торговля и коммерция \> Клиенты \> Все заказы на продажу**). Если это свойство имеет значение **Да** (по умолчанию) для заказов на продажу в Commerce, заказы будут исключены из рабочего процесса блокировки в управлении кредитами. Обратите внимание, что, несмотря на то, что свойство имеет имя **Исключить из управления кредитом**, указанный кредитный лимит по-прежнему будет использоваться во время выполнения заказа. Заказы просто не блокируются.
