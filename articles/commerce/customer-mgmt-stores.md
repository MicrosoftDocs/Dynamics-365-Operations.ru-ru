...
Чтобы преобразовать асинхронных клиентов в синхронных, необходимо сначала выполнить **P-задание**, чтобы отправлять асинхронные клиенты в Commerce Headquarters. Затем запустите задание **Синхронизация клиентов и деловых партнеров из асинхронного режима** (ранее называемое **Синхронизация клиентов и деловых партнеров из асинхронного режима**) для создания кодов счетов клиентов. Наконец, выполните задание **1010**, чтобы синхронизировать коды счета нового клиента с каналами.
...
- Записи асинхронных клиентов нельзя изменить, пока клиент не будет создан в Commerce headquarters, а новый код счета клиента был синхронизирован с каналом обратно. Поэтому адрес невозможно сохранить для асинхронного клиента до тех пор, пока клиент не будет синхронизирован с Commerce headquarters, поскольку добавление адреса клиента внутренне реализовано как операция редактирования в профиле клиента. Однако если включена функция **Включить асинхронное создание адресов клиентов**, адреса клиентов могут быть также сохранены для асинхронных клиентов.
...
Хотя некоторые из упомянутых ограничений могут побудить вас выбрать параметр синхронного клиента для компании, рабочая группа Commerce работает над тем, чтобы возможности асинхронного клиента более точно соответствовали возможностям синхронного клиента. Например, в версии Commerce Version 10.0.22 новая функция **Включить асинхронное создание адресов клиентов**, которую можно включить в рабочей области **Управление функциями**, асинхронно сохраняет новые адреса клиентов для синхронных и асинхронных клиентов. Чтобы сохранить эти адреса в профиле клиента в Commerce headquarters, необходимо создать и запланировать повторяющееся пакетное задание для **P-задания**, задание **Синхронизация клиентов и деловых партнеров из асинхронного режима** и задание **1010**, чтобы преобразовать всех асинхронных клиентов в синхронных клиентов в Commerce Headquarters.

### <a name="customer-creation-in-pos-offline-mode"></a>Создание клиента в автономном режиме POS

Каждый раз, когда POS переходит в автономный режим, система автоматически создает клиентов асинхронно, даже если отключен режим создания асинхронного клиента. Поэтому, как упоминалось ранее, администраторы Commerce headquarters должны создать и запланировать повторяющееся пакетное задание для **P-задания**, задание **Синхронизация клиентов и деловых партнеров из асинхронного режима** и задание **1010**, чтобы преобразовать всех асинхронных клиентов в синхронных клиентов в Commerce Headquarters.
