...
Можно также добавить пользовательские домены. Для этого на странице управления Commerce среды для проекта под подзаголовком **e-Commerce** выберите **+ Добавить пользовательский домен**. Ползунок показывает существующие пользовательские домены и предоставляет возможность добавления нового пользовательского домена.

## <a name="update-which-commerce-scale-unit-is-used"></a>Обновление используемого Commerce Scale Unit

Commerce Scale Unit (CSU), используемый в Commerce, обычно выбирается при первоначальном создании среды. Commerce позволяет изменить экземпляр CSU, используемый в среде, позволяя улучшить обслуживание архитектуры с помощью функций самообслуживания и, тем самым, снизить необходимость обращения в службу поддержки. Чтобы обновить экземпляр CSU, перейдите на страницу управления Commerce среды для проекта и затем выберите **Обновить Scale Unit**. С помощью ползунка **Создать Commerce Scale Unit** выберите новый экземпляр CSU из списка модулей CSU, доступных для вашей среды.
...
- Настройте службу точки входа, например Azure Front Door, для обработки трафика точки входа и подключения к среде Commerce, которая обеспечивает больший контроль над доменом, управление сертификатами и более детализированные политики безопасности.

- Используйте предоставляемый Commerce экземпляр Azure Front Door, что требует координирования действий с рабочей группой Dynamics 365 Commerce для проверки доменов и получения сертификатов SSL для производственного домена.
...
- Необходимо подтвердить, что этот запрос службы предназначен для существующего домена (используемый для активного в данный момент сайта) или нового домена. 
