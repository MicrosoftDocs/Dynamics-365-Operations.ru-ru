...
- **Поведение расчета налога** - **Пересчитать** — это настройка по умолчанию и традиционный способ пересчета налогов при импорте заказа в бэк-офис. **Не пересчитывать** отключает пересчет налога до или до тех пор, пока заказ не будет отредактирован в бэк-офисе, когда запускается пересчет. 
