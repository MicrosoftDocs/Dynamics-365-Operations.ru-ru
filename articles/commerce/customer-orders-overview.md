...
## <a name="disable-option-to-pay-later"></a>Отключение параметра для оплаты позже

В Commerce версии 10.0.12 и более поздних продавцы могут удалить параметр для оплаты позже при создании заказа клиента на POS-терминале. Чтобы отключить этот параметр, откройте **Профиль функциональности** для канала, в котором не допускается оплата позже, затем выберите **Правка**. На вкладке **Общие** выберите раскрывающееся меню для параметра **Требуется платеж для выполнения**. Если на POS-терминале не должна быть разрешена оплата позже, выберите **Требуется карточка** и выберите **Сохранить**. Выполните график распределения **1070**, чтобы синхронизировать это изменение в канале. 
