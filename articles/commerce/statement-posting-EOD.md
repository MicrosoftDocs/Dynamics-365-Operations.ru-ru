...
Кроме того, на экспресс-вкладке **Пакетная обработка** на вкладке **Разноска** на странице **Параметры Commerce** были введены следующие параметры: 
...
- **Макс. число строк проводок, включенных в агрегирование** — это поле определяет количество строк проводки, которые будут включены в одну объединенную проводку перед созданием новой. Агрегированные проводки создаются на основе других критериев агрегирования, таких как клиент, рабочая дата или финансовая аналитика. Важно отметить, что строки из одной проводки не будут разбиты по разным агрегированным проводкам. Это означает, что число строк в агрегированной проводке может быть несколько больше или меньше, в зависимости от факторов, таких как число различных продуктов.
- **Максимальное число потоков для проверки проводок магазина** — в этом поле определяется число потоков, которые будут использоваться для проверки проводок. Проверка проводок является обязательным шагом, который должен быть выполнен до того, как проводки могут быть вставлены в журналы операций. Также необходимо определить **Продукт подарочного сертификата** на экспресс-вкладке **Подарочный сертификат** на вкладке **Разноска** на странице **Параметры Commerce**. Это должно быть определено, даже если подарочные сертификаты не используются организацией.

В следующей таблице перечислены рекомендуемые значения для перечисленных выше параметров. Эти значения следует проверять и адаптировать под конфигурацию развертывания и доступную инфраструктуру. Любое увеличение рекомендуемых значений может отрицательно повлиять на обработку других пакетов и должно быть проверено.
...
| Максимальное число строк при агрегировании проводок | Начало проверки значений с **1000**. В зависимости от конфигурации центрального офиса меньшие заказы могут быть более выгодными для производительности. | Этот параметр определяет число строк, которые будут включены в каждый заказ на продажу при разноске журнала операций. После достижения этого номера строки будут разделены в новый заказ. Хотя число строк продаж не будет точным, так как разбиение выполняется на уровне заказа на продажу, оно будет близко к заданному числу. Этот параметр используется для создания заказов на продажу для проводок по розничной торговле, не имеющих именованных клиентов. |
| Максимальное число потоков для проверки проводок магазина | Рекомендуется установить для этого параметра значение **4**, и оно увеличится только в том случае, если приемлемое быстродействие не достигнуто. Число потоков, используемых этим процессом, не может превышать число процессоров, доступных серверу обработки пакетных заданий. Если назначить здесь слишком много потоков, это может повлиять на другую пакетную обработку. | Этот параметр определяет количество проводок, которые могут быть проверены одновременно для данного магазина. |
