...
description: "В этом разделе объясняется, как открыть данные объекта в Microsoft Excel, и затем просматривать, обновлять и изменять эти данные с помощью надстройки Microsoft Dynamics Office для Excel. Чтобы открыть данные объекта, можно начать из Excel или из Microsoft Dynamics 365 for Operations."
author: ChrisGarty
manager: AnnBe
ms.date: 04/04/2017
ms.topic: article
...
В этом разделе объясняется, как открыть данные объекта в Microsoft Excel, и затем просматривать, обновлять и изменять эти данные с помощью надстройки Microsoft Dynamics Office для Excel. Чтобы открыть данные объекта, можно начать из Excel или из Microsoft Dynamics 365 for Operations.
...
## <a name="open-entity-data-in-excel-when-you-start-from-dynamics-365-for-operations"></a>Открытие данных объекта в Excel при запуске из Dynamics 365 for Operations
1.  На странице в Microsoft Dynamics 365 for Operations щелкните **Открыть в Microsoft Office**. Если корневой источнику данных (таблица) для страницы совпадает с корневым источником данных для любых объектов, для этой страницы создаются параметры по умолчанию **Открыть в Excel**. Параметры **Открыть в Excel** можно найти на часто используемые страницах, таких как **Все поставщики** и **Все клиенты**.
...
5.  Скопируйте URL-адрес браузера из целевого экземпляра Dynamics 365 for Operations, вставьте его в поле **URL-адрес сервера**, затем удалить все элементы после имени узла (например, удалите **/?cmp=usmf&mi=CustTableListPage**). Полученный URL-адрес должен быть просто именем узла (например, **https://xxx.dynamics.com**).
...
11. После добавления нужных полей в поле **Выбранные поля**, убедитесь в том, что курсор находится в нужное место на листе (например, в ячейке A1), затем нажмите **Готово**. Затем для закрытия конструктора щелкните **Готово**.
...
-   Щелкните в любом месте листа, затем нажмите кнопку **Создать** в надстройке Excel.
-   Щелкните в последней строке листа, затем нажимайте клавишу Tab, пока курсор не выйдет за последний столбец этой строки и не будет создана новая строка.
-   Щелкните в строке сразу под листом и начните вводить данные в ячейку. Когда пользователь перемещает фокус из этой ячейки, листа расширяется для включения новой строки.
...
4.  Настройте требуемым образом список в списке **Выбранные поля**:
    ...
    -   Чтобы изменить порядок полей, щелкните поле в списке **Выбранные поля**, затем щелкайте кнопку **Вверх** или **Вниз**.

5.  Примените изменения к источнику данных, щелкнув **Обновить**. Затем для закрытия конструктора щелкните **Готово**. Если было добавлено поле (столбец), нажмите кнопку **Обновить**, чтобы извлечь обновленный набор данных.

## <a name="httpspowerappsmicrosoftcomenustutorialsdataplatforminteractiveexceltroubleshootingtroubleshooting"></a>[](https://powerapps.microsoft.com/enus/tutorials/dataplatforminteractiveexcel/#troubleshooting)Устранение неполадок
