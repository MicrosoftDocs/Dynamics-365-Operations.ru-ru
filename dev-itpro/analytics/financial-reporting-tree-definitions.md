...
|---|---|
| Организация               | Название компании для единицы отчетности. **@ANY**, Которое обычно назначается только уровне сводки, позволяет аналитической структуры для всех компаний. Все дочерние ветви имеют компанию, заданную к ним.|
...
2.  Щелкните **файл**&gt;**новой**&gt;**определения аналитической структуры**.
3.  Щелкните **редактирование**&gt;**вставить элементы из измерений**.
...
11. Щелкните **файл**&gt;**Сохранить** для сохранения аналитической структуры. Введите уникальное имя и описание для дерева отчетности, и после этого щелкните **ОК**.
...
4.  Щелкните **файл**&gt;**Сохранить** для сохранения изменений.
...
На следующем рисунке показан пример диалогового окна **Вставить единицы отчетности из аналитик**. Например, в результате будет возвращена комбинация бизнес-единиц, центров затрат и подразделений. [![InsertReportingUnits](./media/insertreportingunits.png)](./media/insertreportingunits.png) результирующий определение структуры сортируется по подразделению, а затем по центру затрат, а затем по отделам. Измерение для пятой блок: **подразделение = \[001\], центр затрат =\[\], отдел = \[022\]**и определяет блок для счетов, которые относятся к 001 единицы бизнеса и отдел 022. [![ReportingTree](./media/reportingtree-1024x646.png)](./media/reportingtree.png)
