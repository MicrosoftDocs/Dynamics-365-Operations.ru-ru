...
-   Ярлык должен начаться с алфавитного знака (от a до z или от A до Z), и может быть любым сочетанием чисел и букв до 16 символов. **Примечание:** метка может содержать символ подчеркивания (\_), но никакие другие специальные символы не допускаются.
...
3.  Выберите код формата в списке В приведенной ниже таблице описаны коды форматов и их действия.
    ...
    | TOT                           | Всего                              | Определяет строку, которая использует математические операторы в столбце **Связанные формулы/строки/единицы**. Итоги содержат простые операторы, такие как **+**или**-**.                                                      |
    | CAL                           | Расчет                        | Определяет строку, которая использует математические операторы в столбце **Связанные формулы/строки/единицы**. Вычисления содержат сложные операторы, такие как **+**, **-**, **\***, **/**, и **IF/THEN и ELSE** инструкции. |
...
В следующем примере, Филлис хочет предотвратить печать заголовка и подчеркиваний в строке **Итого наличных** ее отчета, потому что не было никакой работа в любой из касс. Таким образом, в строке 220 (которой, как **---**указывает код формата является строка форматирования) в **связанные формулы/строки/блоки** ячеек, она вводит **250**, которое является код суммы строки, она хочет, чтобы при. [![RelatedRowsRowDefinition](./media/relatedrowsrowdefinition-1024x144.png)](./media/relatedrowsrowdefinition.png)
...
6.  В меню **Файл** выберите **Сохранить как**, чтобы сохранить определение столбца под новым именем. Добавить **CBR** к текущему имени файла (например, **по\_с начала года\_CBR**). Это определение столбца — ваше базовое определение столбца строки.
...
В следующем примере значения в столбце D отчета будут отсортированы в порядке возрастания 160 по 490 строк. Кроме того абсолютные значения в столбце G отчета будут отсортированы в убывающем порядке для строк с 610-й по 940.
...
5.  В разделе **Атрибуты** выполните следующие действия для того, чтобы добавить запись для каждого атрибута который должен быть включен с кодом строки:
    1.  Выполните двойной клик ячейки **Атрибут** и выберите имя атрибута. **Предупреждение:** замените знак решетки (\#) с числовым значением.
...
Когда вы выбираете определенный счет, конструктор отчетов обычно объединяет счет и финансовые годы, периоды и другую информацию, указанную в определении столбца. Вы можете использовать различную информацию, такую как различные финансовые периоды, для специфических строк. В следующей таблице показываются модификаторы счета, которые доступны. Замените знак решетки (\#) со значением, которое меньше или равна числу периодов в финансовом году.

| Модификатор счета | Что печатается                                                                           |
|------------------|-------------------------------------------------------------------------------------------|
| /BB              | Начальное сальдо счета.                                                     |
| /\#              | Сальдо для указанного периода.                                                     |
| /-\#             | Баланс для периода, который \#периоды до текущего периода.                  |
| /+\#             | Баланс для периода, который \#за текущий период.                   |
| /C               | Сальдо для текущего периода.                                                       |
| /C-\#            | Баланс для периода, который \#периоды до текущего периода.                  |
| /C+\#            | Баланс для периода, который \#за текущий период.                   |
| /Y               | Сальдо с начала года до текущего периода.                                      |
| /Y-\#            | Сальдо с начала года, до периода, который \#периоды до текущего периода. |
| /Y+\#            | Сальдо с начала года, до периода, который \#за текущий период.  |

### <a name="book-code-modifiers"></a>Модификаторы кода книги

Вы можете ограничивать строку существующим кодом книги. Определение столбца должно включить по крайней мере один столбец **FD**, который имеет код книги. **Примечание:** Ограничение кода книги для строки перекрывает ограничения кода книги в определении столбца для этой строки.
...
5.  В **из** введите начальную аналитику или щелкните ![Обзор](https://i-technet.sec.s-msft.com/dynimg/IC679490.gif "Обзор") для поиска доступных измерений. Для того, чтобы вписать диапазон аналитик, впишите конечную аналитику в поле **До**.
...
При вводе значения сегмента счета в **аналитики** диалогового окна, можно поместить подстановочный знак (?) или \*) в любом месте сегмента. Конструктор отчетов извлекает все значения для предопределенных позиций без учета подстановочные знаки. Например, определение строки содержит только естественные значения сегмента, и естественные сегменты имеют четыре знака. Путем ввода **6???** в строке попросите конструктор отчетов, чтобы включить все учетные записи, которые имеют значение сегмента счета, начинающегося с 6. Если ввести **6\***, возвращаются те же результаты, но в результаты также входят значения переменной ширины, такие как **60** и **600000**. Конструктор отчетов заменяет каждый подстановочный знак (?) полным диапазоном возможных значений, который включает буквы и специальные символы. Например, в диапазоне от **12?0** до **12?4**, подстановочный знак в **12?0** заменяется с самым низким значением в наборе символов, и подстановочный знак в **12?4** заменяется самым высоким значением в наборе символов. **Примечание:** Вы должны избежать использования подстановочных знаков для начальных и конечных счетов в диапазонах. Если вы используете подстановочный знаки в начальном или конечном счете, вы можете получить непредвиденные результаты.

### <a name="single-segment-or-single-dimension-ranges"></a>Диапазоны с одним сегментом или одной аналитикой

Вы можете определить диапазон значений сегментов или значений аналитик. Преимущество определения диапазона — то, что вы не должны обновлять определение строки каждый раз, когда новое значение сегмента или значение аналитики добавляется к финансовым данным. Например, диапазон **+ счет =\[6100: 6900\]** использует значения из счетов 6100 по 6900 сумму строки. Если диапазон включает подстановочный знак (?), конструктор отчетов не оценивает диапазон познаково. Вместо этого, определяются низкий и верхний концы диапазона, и после этого конечные значения и все значения между ними включены. **Примечание:** конструктор отчетов не может выбрать поля, счетов или аналитик из системы Microsoft Dynamics ERP, содержать следующие зарезервированные символы: &, \*, \[, \], {, или}. Вы можете добавить амперсанд (&), только когда вы автоматически строите определения строки путем использования диалогового окна **Вставить строки из аналитик**.

### <a name="multiple-segment-or-multiple-dimension-ranges"></a>Диапазоны с несколькими сегментами или несколькими аналитиками

При вводе диапазона с использованием комбинаций из нескольких значений измерений сравнение диапазонов выполняется на... Основы \financial-dimensions\dimension-BY-Dimension. Сравнение диапазона нельзя сделать посимвольно или на основе частичного сегмента. Например, диапазон **+ счет =\[5000: 6000\], отдел =\[1000:2000\], центр затрат =\[00\]** включает только счета, соответствующие каждому из сегментов. В этом случае первый измерение должно находиться в диапазоне от 5000 до 6000, второе измерение должно находиться в диапазоне от 1000 до 2000 и последнего измерения должен быть 00. Например **+ счет =\[5100\], отдел =\[1100\], центр затрат =\[01\]** не включены в отчет, так как последний сегмент находится вне указанного диапазона. Если значение сегмента содержит пробелы, заключите это значение в квадратные скобки (\[\]). Допустимы следующие значения для сегмента четырех символов: **\[234\], \[123 \], \[1 34\]**. Значения аналитик, которые должны быть заключены в квадратные скобки (\[\]) и отчет конструктор добавляет эти скобки для вас. Если диапазон с несколькими сегментами или несколькими измерениями содержит подстановочные знаки (?) или \*), определяются нижнюю и верхнюю границы диапазона всего несколькими сегментами или несколькими измерениями, а затем включаются конечное значения и все значения между ними. Если вы имеете большой диапазон, такой как весь диапазон счетов от 40000 до 99999, вы должны определить допустимый начальный счет и конечный счет, когда возможно. **Примечание:** конструктор отчетов не может выбрать поля, счетов или аналитик из системы Microsoft Dynamics ERP, содержать следующие зарезервированные символы: &, \*, \[, \], {, или}. Вы можете добавить амперсанд (&), только когда вы автоматически строите определения строки путем использования диалогового окна **Вставить строки из аналитик**.
...
|-----------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|
| Добавить два полностью определенных счета.                                                       | + Подразделение =\[000\], счета =\[1205\], отдел =\[00\]+ подразделение =\[100\], счета =\[1205\], отдел =\[00\] |
| Добавьте два значения сегмента.                                                                 | + Счет =\[1205\]+ счет =\[1210\]                                                                           |
| Добавьте Значения сегментов, которые содержат подстановочные знаки.                                    | + Счет =\[120? + счет =\[11??\]                                                                             |
| Суммировать диапазон полностью определенных счетов.                                                | + Подразделение =\[\], счет =\[1205\], отдел =\[00\]                                                   |
| Суммировать диапазон значений сегмента.                                                          | + Счет =\[1200: 1205\]                                                                                       |
| Суммировать диапазон значений сегментов, которые содержат подстановочные знаки.                         | + Счет =\[120?: 130?\]                                                                                       |
| Вычтите один полностью определенный счет из другой полностью определенный счет.              | + Подразделение =\[000\], учетной записи =\[1205\], отдел =\[00\]-подразделение =\[100\], счет =\[1205\], отдел =\[00\] |
| Вычтите одно значение сегмента из другого значения сегмента.                                  | + Счет =\[1205\]-счет =\[1210\]                                                                           |
| Вычтите значение сегмента которое включает подстановочный знак, из другого значения сегмента. | + Счет =\[1200\]-счет =\[11??\]                                                                           |
| Вычесть диапазон полностью определенных счетов.                                           | -Подразделение =\[\], счет =\[1200: 1205\], отдел =\[00:01\]                                           |
| Вычесть диапазон значений сегмента.                                                     | -Счет =\[1200: 1205\]                                                                                       |
| Вычесть диапазон значений сегментов, которые содержат подстановочные знаки.                    | -Счет =\[120?: 130?\]                                                                                       |

Хотя вы можете изменить счета напрямую, вы можете также использовать диалоговое окно **Аналитики**, чтобы применить правильное форматирование к вашим ссылкам на финансовые данные. Любое из значений может содержать подстановочные знаки (?) Or \*). Тем не менее, конструктор отчетов не может выбрать поля, счетов или аналитик из системы Microsoft Dynamics ERP, содержать следующие зарезервированные символы: &, \*, \[, \], {, или}. **Примечание:** Для того, чтобы вычесть значения, вы должны поместить скобки вокруг тех значений. Например, если ввести **450?-(4509)**, он отображается как **+ счет =\[4509\]-счет =\[450? \]**, и вы заметили, что в программе конструктора отчетов, следует вычесть сумму сегмента счета 4509 из суммы любого сегмента счета, начинающегося с 450.
...
3.  В первой строке диалогового окна **Аналитики** следовать этим шагам:
    ...
    2.  Дважды щелкните **оператор +/-** ячейку, а затем нажмите знак плюса (**+**) или минус (**-**) оператор, который применяется к одному или нескольким значениям аналитики или наборы в строке.
...
| Товар                | Описание                                                                                                                                                                                                                                                                                             |
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| По отдельности или диапазон | В **из** введите имя учетной записи или нажмите кнопку **Обзор** кнопку ![Обзор](https://i-technet.sec.s-msft.com/dynimg/IC679490.gif "Обзор"), чтобы выбрать для учетной записи. Для того, чтобы выбрать диапазон, введите или найдите значение в поле **До**.                                             |
| Набор значений аналитики | В поле **Имя** введите имя набора значений аналитик. Для того, чтобы создать, изменить, скопировать, или уничтожить комплект, щелкните **Управление наборами значений аналитик**. **Формула** поле заполняется с использованием формулы из **связь с финансовой аналитикой** ячейки для этого значения аналитики, заданным в определении строки. |
