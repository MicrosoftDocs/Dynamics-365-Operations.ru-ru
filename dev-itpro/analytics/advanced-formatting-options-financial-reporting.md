...
Когда вы конструируете отчет, вы должны создать все строки деталей сперва, для того, чтобы убедиться что значения будут получены как ожидалось. После этого добавьте перекрытия формата **NP** (не печатать), чтобы подавить деталь, которая включает окончательные значения. **Важно:** Когда вы используете код формата **CAL** в определении строки, вы не можете детализовать подробности проводки. Форсирование, формулы используется следующий формат: &lt;целевой столбец&gt;=&lt;рассчитанные столбца&gt;. &lt;код строки&gt; разделите все дополнительные подстановки для строки запятой и пробелом. Вот пример: D=C.190, E=C.100
...
Формула расчета в определение строки может включать **+**, **-**, **\***, и **/**операторы, а также **IF/THEN и ELSE** инструкции. Дополнительно, вычисление может включить индивидуальные ячейки и абсолютные суммы (фактические числа, которые включены в формулу). Формула может содержать до 1024 символов. Вычисления нельзя приложить к строкам, которые содержат ячейки типа **Ссылка на финансовые аналитики** (FD). Однако вы можете включить вычисления в последовательных строках, подавить печатание этих строк, и после этого подытожить строки вычисления.

### <a name="operators-in-a-calculation-formula"></a>Операторы в формуле вычисления

Формула вычисления использует более сложные операторы, чем формула итогов строки. Однако можно использовать **\***и **/**операторы вместе с дополнительные операторы для умножения (\*) и деления (/) суммы. Для использования диапазона или суммы в формуле вычисления, вы должны использовать знак at (@) перед любым кодом строки, если вы используете столбец в определении строки. Например, чтобы добавить сумму в строке 100 к сумме в строке 330, можно использовать формулу итогового значения строки **100 + 330** или формулу расчета в **@100+@330**. **Примечание:** Вы должны использовать знак at (@) перед каждым кодом строки, который вы используете в формуле вычисления. В противном случае номер прочитается как абсолютное количество. Например, формула **@100+330** USD 330 добавляет сумму из строки 100. При ссылке на столбец в формуле расчета знак "собаки" (@) не требуется.
...
В этом примере формула расчета **@100+@330** означает, что сумма в строке 100 добавляется к сумме в строке 330. Формула итога по строкам **340 + 370** добавляет сумму из строки 340 сумму из строки 370. (Сумма из строки 370 представляет собой сумму по формулу расчета).
...
Когда строка в определении строки имеет код формата **CAL**, и вы вписываете математическое вычисление в ячейке **Связанные формулы/строки/единицы**, вы должны также вписать букву связанного столбца и строки в отчет. Например, введите **A.120** для представления столбец A, строки 120. Кроме того, можно использовать знак (@), чтобы указать все столбцы. Например, введите **@120**для представления всех столбцов в строке 120. Любое математическое вычисление, не имеющей букву столбца или символа (@), рассматривается как действительное число. **Примечание:** при использовании кода строки в метке для ссылки на строку, необходимо использовать точку (.) в качестве разделителя между букву столбца и метки (например, **A.GROSS\_MARGIN/A.SALES**). При использовании символа (@), разделитель не требуется (например, **@GROSS\_MARGIN/@SALES**).
...
-   Чтобы ограничить формулы определенных столбцов, введите букву столбца, знак равенства (**=**), а затем формула.
...
Операторы **IF/THEN/ELSE** можно добавить к любому действительному вычислению и использовать с форматом **CAL**. Вы вводите формулы вычисления **IF/THEN/ELSE** в ячейке в столбец **Связанные формулы/строки/единицы**. **IF/THEN и ELSE** расчета формулы используется следующий формат: IF &lt;инструкция true/false&gt; ЗАТЕМ &lt;формула&gt; ELSE &lt;формула&gt;**ELSE &lt;формула&gt;** инструкции является необязательным.
...
-   **IF A.200&gt;0 и A.200&lt;10 000** (сложная инструкция)
-   **IF A.200&gt;10000 или ((A.340/B.1200)\*2 &lt;1200)** (сложная инструкция, которая содержит несколько выражений)
...
Формулы **THEN** и **ELSE** могут являться любым действительным расчетом, от очень простых назначений значений до сложных формул. Например, инструкция **IF A.200&gt;0, то A=B.200** означает, что «если значение в ячейке в столбце A строки 200 больше 0 (ноль), поместить значение из ячейки столбца B строки 200 в ячейку в столбце A текущей строки.» Предшествующее выражение **IF/THEN** помещает значение в один столбец текущей строки. Однако вы можете также использовать знак at (@) в оценках истина/ложь или формуле, чтобы представлять все столбцы. Вот некоторые другие примеры, которые описаны в следующих разделах:

-   **IF A.200 &gt;0, то B.200**: Если значение в ячейке A.200 положительно, значение из ячейки B.200 помещается в каждый столбец текущей строки.
-   **IF A.200 &gt;ЗАТЕМ 0 @200**: Если значение в ячейке A.200 положительно, значение из каждого столбца в строке 200 помещается в соответствующий столбец в текущей строке.
-   **Если @200&gt;ЗАТЕМ 0 @200**: Если значение в строке 200 текущего столбца положительно, значение из строки 200 помещается в тот же столбец текущей строки.
...
Чтобы ограничить вычисление один блок в аналитической структуре, таким образом, результирующее значение не передается блоку более высокого уровня, можно использовать **@Unit**код в **связанные формулы/строки/блоки** ячейки в определении строки. **@Unit**Код указан в столбце B аналитической структуры, **имя блока**. При использовании **@Unit**код, значения не свертывается, но вычисление выполняется на каждом уровне аналитической структуры. **Примечание:** Для использования этой функции, дерево отчетности необходимо связать с определением строки. Строка вычисления может сослаться к строку вычисления или на строку финансовых данных. Вычисление записано в ячейке **Связанные формулы/строки/единицы** определения строки и ограничения типа финансовых данных. Вычисление должно использовать условное вычисление, которое начинается с **IF @Unit**конструкции. Ниже приведен пример: IF @Unit(продажи) и ЗАТЕМ @100ELSE 0 это вычисление включает сумму из строки 100 в каждом столбце отчета, но только для блока "SALES". Если множественные единицы названы SALES, то сумма появляется в каждом из тех единиц. Дополнительно, строка 100 может быть строкой финансовых данных и может быть определена как не печатать. В этом случае, сумма предотвращается от появления во все блоках в дереве. Вы можете также ограничивать сумму одним столбцом отчета, например столбец H, путем использование ограничения столбца для того, чтобы напечатать значение только в этом столбце отчета. Вы можете включить комбинации **OR** в выражение **IF**. Ниже приведен пример: IF @Unit(продажи) и @Unit(SALESWEST), ЗАТЕМ 5 ELSE @100определить блок в ограничении типа вычисления можно одним из следующих способов:

-   Введите имя единицы, чтобы включить единицы, которые соответствуют. Например **Если @Unit(продажи)** позволяет выполнить расчет для любого блока с именем заказа на продажу, даже при наличии нескольких единиц продажи в дереве отчетов.
-   Впишите имя компании и единицы для того, чтобы ограничить вычисление специфическими единицами в специфической компании. Например, введите **IF @Unit(ACME: продажи**), чтобы ограничить вычисление единиц продажи в компании ACME.
-   Введите полный код иерархии из дерева отчетности, чтобы ограничить вычисление определенной единицей. Например, введите **IF @Unit(Сводка ^ ACME ^ ДАЛЬНЕВОСТОЧНЫЙ регион ^ SALES)**. **Примечание:** Для того, чтобы найти полный код иерархии, выполните правый клик в определении дерева отчетности, и после этого выберите **Копировать идентификатор единицы отчетности (H-код)**.

#### <a name="restrict-a-calculation-to-a-reporting-unit"></a>Ограничьте вычисление единицей отчетности

1.  В конструкторе отчетов щелкните **Определения строк**, и после этого раскройте определение строки, которое надо изменить.
2.  Выполните двойной клик ячейки **Код формата**, и после этого выберите **CAL**.
3.  Щелкните **связанные формулы/строки/блоки** ячейку, а затем введите условное вычисление, которое начинается с **IF @Unit**конструкции.

### <a name="ifthenelse-statements-in-a-column-definition"></a>Операторы IF/THEN/ELSE в определении столбца

Оператор **IF/THEN/ELSE** позволяет любому вычислению зависеть от результатов из любого другого столбца. Вы можете сослаться к другие столбцы, но вы не можете сослаться на ячейку отчета в выражении **IF**. Любое вычисление необходимо применить к всему столбцу. Например, инструкция **IF B&gt;100 THEN B ELSE C\*1,25** означает, «если суммы в столбце B более чем 100, следует поместить значение из столбца B в **CALC** столбца. Если количество в столбце B не больше чем 100, то умножить значение в столбце C на 1,25, и положить результат в столбец **CALC**.» Всегда ставьте за выражением **IF** логическое выражение, которое можно оценить как истина или ложь. Формулы, которые вы используете и для выражения **THEN**, и для **ELSE** могут содержать ссылки на любое число столбцов, и эти формулы могут быть так сложны, как вам надо. **Примечание:** Вы не можете положить результаты вычисления в любой другой столбец. Результаты должны находиться в столбце, который содержит формулу.
