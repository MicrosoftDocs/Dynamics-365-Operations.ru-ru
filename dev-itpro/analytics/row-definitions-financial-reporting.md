...
-   Ссылки на финансовые данные, которые могут включать значения измерения в Microsoft Dynamics 365 для операций **Примечание:** можно настроить определение строки для получения данных из системы финансовой аналитики при каждом создании отчета.
...
| Параметр                | Описание                                                                                                                                                                                                                                                                      |
|-----------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Аналитика             | Схема, которая определяет аналитику, которую добавить к определению строки. Этот шаблон содержит один амперсанд (&) или знак решетки (\#) для каждой позиции в измерениях. Обычно используйте все амперсанды аналитики "Счет ГК" и все знаки решетки для других аналитик. |
...
4.  Чтобы ограничить определение строки определенным диапазоном значений аналитик, введите начальное значение аналитики в ячейке **Начало диапазона аналитик** и конечное значение аналитики в ячейке **Конец диапазона аналитик**. Чтобы включить все значения для выбранной аналитики, оставьте эти ячейки пустыми. **Примечание:** подстановочные знаки (\* или?) в измерении диапазонов могут возвращать не все результаты необходимости, в зависимости от способа сортировки данных в базе данных ERP-системе.
...
6.  В поле **Нарастить каждую строку по** укажите зазор между последовательными кодами строк. Например если кодом первой строки равна 100, а значение приращения равно 30, первых строк будут коды 100, 130, 160, 190 и 220. Используйте значение приращения, которое предоставляет достаточно пространства для вставки новых строк формата и формул.
...
3.  Щелкните кнопку панели инструментов того формата, который применить. Например, чтобы перевести строки, выберите строку и нажмите кнопку **увеличить отступ**![увеличить отступ](https://i-technet.sec.s-msft.com/dynimg/IC679497.gif "увеличить отступ") на панели инструментов.
