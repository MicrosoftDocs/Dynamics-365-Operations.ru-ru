---
title: "Настройка процесса утверждения в workflow-процесс"
...
# <a name="configure-an-approval-process-in-a-workflow"></a>Настройка процесса утверждения в workflow-процесс

Используйте следующую процедуру для настройки свойств для процесса утверждения.

Чтобы настроить процесс утверждения, в редакторе workflow-процессов, щелкните правой кнопкой мыши элемент утверждения и нажмите кнопку **свойства** чтобы открыть **свойства** формы.
...
9.  Задать кому отправляются уведомления. Выберите один из параметров в следующей таблице, а затем выполните дополнительные шаги для параметра перед тем как перейдите к шагу 10.
    ...
    <li><strong>Доступных пользователей</strong>: список включает все Microsoft Dynamics 365 для пользователей операций. Выберите пользователей, которым требуется отправить уведомления, а затем переместите этих пользователей в список <strong>Выбранные пользователи</strong>:.</li>
    </ol></td>
    </tr>
    </tbody>
    </table>
...
3.  Для настройки стадии утверждения см. раздел [Настройка стадии утверждения](http://axhelp.dynamics.com/en/wiki/configure-an-approval-step/).
