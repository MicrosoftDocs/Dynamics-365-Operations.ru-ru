...
> [!Note] 
> Некоторые документы также поддерживают заголовка документа денежных сумм, таких как накладные расходы для заказов и счетов. 
...
-   **Распределить суммы** — просмотр и изменение распределений по бухгалтерским счетам для отдельной строки или заголовка документа и любых дочерних строк, например для налогов и накладных расходов.
    ...
    -   Любое дочернее распределение, например скидка, накладные расходы и налог, всегда подчиняется родительскому распределению. Можно просматривать отношения родитель потомок в **ссылки**&gt;**родительским сведения**.
    ...
    -   Распределения отклонений можно создать в сценариях сопоставления, например сопоставление накладной поставщика и заказа на покупку. Можно просматривать связи сопоставления между распределение затрат на **ссылки**&gt;**данные о документе**.
    -   Кнопка **Корректировать** отображается и включается для документов, которые поддерживают корректировки. **Правильные** создает новый распределений. Во-первых распределений создаются операции исходной распределений. Нельзя изменить эти распределений. Далее, новые правильный распределения по бухгалтерским счетам создаются. Эти распределения можно изменить, если можно было изменить исходные распределения.
    ...
    -   Можно просмотреть текущее состояние учета документа в **ссылки**. Статус для всего документа и показывает, обрабатывается или завершен документа.
-   ** Просмотр распределений **-Просмотр распределений по бухгалтерским счетам для всех строк и денежных сумм на документ. В этом представлении невозможно изменить распределения по бухгалтерским счетам.


Дополнительные сведения см. в разделе [распределениях по счетам и записей в журнале субкниги для накладных с произвольным текстом](accounting-distributions-subledger-journal-entries-vendor-invoices.md).
