...
2.  April, координатор по расчетам с поставщиками в Fabrikam, вводит и проверяет накладную, отправленную Contoso. Она проверяет следующие данные:
    ...
    -   Для номенклатур, требующих двустороннее или трехстороннее сопоставление цены в строке накладной находятся в допустимых отклонениях, определенных в Microsoft Dynamics 365 для Operations.This включает в себя следующие типы соответствия цены:
...
В Microsoft Dynamics 365 для операций строка накладной включает следующие сведения.
...
2.  April, координатор по расчетам с поставщиками в Fabrikam, вводит и проверяет накладную, отправленную Contoso. Она проверяет следующие данные:
    -   Для номенклатур, которым требовалась трехсторонняя проверка соответствия, что количество в строке накладной совпадает с полученным количеством. Полученное количество указывается на поступлении продукта, которое совпадает с накладной.
    -   Для номенклатур, требующих двустороннее или трехстороннее сопоставление цены в строке накладной находятся в допустимых отклонениях, определенных в Microsoft Dynamics 365 для операций. Это включает в себя следующие типы соответствия цены:
...
Дополнительные сведения см. в разделе [расчеты сопоставления накладных с поставщиками](accounts-payable-invoice-matching.md).
