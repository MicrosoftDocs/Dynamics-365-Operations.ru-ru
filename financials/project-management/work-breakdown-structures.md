...
Создание СДР — сложный процесс, который обычно выполняется в течение продолжительного периода времени и требует сотрудничества и информации от большого количества людей. Описывается, как использовать расширения СДР в Microsoft Dynamics 365 для операций в соответствии с требованиями для оценки и отслеживания.
...
Чтобы использовать все возможности планирования СДР функций, выполните следующие настройки:

1.  Настройте календарь по умолчанию и календарь проекта.
    1.  Щелкните **управление и учет по проектам**&gt;**установки**&gt;**планирование**. В поле **Календарь рабочего времени по умолчанию** укажите календарь по умолчанию. Это будет календарь рабочего времени по умолчанию для любого создаваемого нового проекта.
...
Чтобы настроить рабочие дни и рабочее время для календаря или создание нового календаря, нажмите **управления организацией**&gt;**обычные**&gt;**календари**.
...
-   Чтобы настроить себестоимости и цены продажи по категориям сборов, расходов и трудозатрат, щелкните **управление и учет по проектам**&gt;**установки**&gt;**цены**.
...
В следующих разделах обсуждаются как СДР возможностей может помочь с каждым из этих действий.

### <a name="work-decomposition"></a>Декомпозиция работы

Создание декомпозиции работы — обычно первый шаг в процессе создания СДР. Функции работы с СДР поддерживает следующие основные конструкции декомпозиции работ или декомпозиции. 
...
> [!NOTE] 
> При понижении уровня задачи в группе задач, который был конечного узла перед выполнением операции отступа, созданной суммарной задачи теряет свою собственную дат, силы и количество ресурсов. Теперь она использует сводку значений своих новых составных задач. 
...
Оценка графика обычно является вторым шагом создания СДР. Рекомендуется завершать оценку графика после создания задач. **Декомпозиции работ** страницы в Microsoft Dynamics 365, операция состоит из двух разделов. Верхняя область предназначена для оценки графика, а нижняя область включает вкладку **Оценочная стоимость и выручка**, которую можно использовать для оценки стоимости. 
...
**Задачи планирования в Microsoft Dynamics 365 для операций** влияют следующие факторы при планировании задачи оконечных узлов:
...
Планирование правила *** при включении поддержки автоматического планирования, применяются следующие правила для планирования задач для задач конечного узла:
...
Число людей × продолжительность × количество часов в стандартной рабочим днем в календаре проекта. 

В некоторых случаях бывает нужно отклониться из этих правил. Вы можете отключить автоматическое планирование для предотвращения Microsoft Dynamics 365 для операций из автоматически установку или исправления свойства задачи оконечных узлов дерева. При вводе информации для задачи, которая приводит к нарушению каких-либо правил планирование, отображается значок ошибки планирования для задачи. Если вы не хотите, чтоб отображались ошибки планирования, щелкните **Ошибки планирования отображаются**, чтобы отключить эту функцию. 

> [!NOTE] 
> Значения сводки или контейнер задачи по-прежнему быть рассчитан как сумма значений неотъемлемый задач, вне зависимости от того, было ли помощник автоматического планирования включить или отключить. 
...
**Устранение всех ошибок планирования** Microsoft Dynamics 365 для операции, чтобы исправить все ошибки планирования в СДР, на панели операций нажмите кнопку **устранить все планирование несоответствия**. 

> [!NOTE] 
> Эта функция может вызвать значительные изменения СДР. Ошибки исправляются в следующем порядке:
...
> [!NOTE] 
> Невозможно изменить сводку или контейнер задачи оценки затрат. Оценка стоимости для задачи сводки равна сумме оценки стоимости всех ее задач листового узла. Оцененная общая стоимость для каждой задачи рассчитывается как сумма оценочной стоимости следующих типов проводок:
...
Себестоимости и цены продажи для каждой трудозатрат или о расходах оценки строки основаны на настройки, определенный для каждой категории в таблицах ценообразования в **управление и учет по проектам**&gt;**установки**&gt;**цены**. Для номенклатур стоимость и цена продажи добавляются по умолчанию из номенклатуры или коммерческих соглашений на странице списка **Выпущенные продукты** в модуле "Управление сведениями о продукте".
...
Microsoft Dynamics 365 для операций имеет три представления для СДР проекта: Просмотр планирования, просмотр отслеживания усилий и просмотр отслеживания затрат.
...
-   Оставшиеся трудозатраты (также называется оценки на завершение \[и т. П\]) = запланировано силы — фактический решение,
...
Руководители проектов можно изменить объем работ по задачам двумя способами:
...
> [!NOTE] 
> При пересмотре усилия для задачи СДР в режиме отслеживания усилий задачи CTC, ПОПЗ, процент затрат потребления и расхождение прогнозируемых затрат пересчитываются в отслеживания Просмотр затрат. Однако пересмотр стоимости не повлияет на значения в представлении "Отслеживание усилий", поскольку стоимость по типу проводки (труд, материал или расход) или категория проекта не пересматриваются. 
...
Microsoft Dynamics 365 for Operations использует 0:100, доход правила, когда он отображает плановому значению. Согласно этому правилу стоимость задачи разносится в задачу по состоянию на дату окончания. Стоимость не разносится, пока задача не будет завершена на 100 %. 
...
Microsoft Dynamics 365 for Operations использует 0:100, доход правила при его графиков набранные значение. Согласно этому правилу стоимость задачи разносится в задачу по состоянию на дату окончания. Стоимость не разносится, пока задача не будет завершена на 100 %. 
...
## <a name="wbs-templates"></a>Шаблоны СДР
Функции работы с шаблонами СДР можно использовать для создания стандартных шаблонов для проектов. Если проекты, которые предлагает компания, включают большое количество повторяющихся работ, подумайте о том, чтобы создать шаблон СДР. 
