...
Следующие параметры влияют на способ обработки сопоставлений в Microsoft Dynamics 365 для операций. Сопоставление — это процесс сопоставления накладной с платежом или кредит-нотой. Эти параметры расположены в области **Сопоставление** на страницах **Параметры расчетов с клиентами** и **Параметры расчетов с поставщиками**.
...
-   **Администрирование скидок при оплате наличными** – Указание того, как [скидка при оплате наличными обрабатывается, когда накладная будет переплата](cash-discount-handling-overpayments.md). Для переплата может быть уменьшено скидки при оплате наличными, он может рассматриваться как разница или оставаться на счет для поставщика или клиента.
...
-   **Вычислять скидки по оплате для кредит-нот** — установите для этого параметра значение **Да**, чтобы автоматически рассчитывать скидку по оплате для кредит-нот. В модуле "Расчеты с клиентами" проводка кредит-ноты — это отрицательная проводка, которая имеет значение в поле **Накладная** на странице **Накладная с произвольным текстом**, или возврат на странице **Заказ на продажу**.
    -   Влияние этого параметра зависит от значения в поле **Использовать скидку по оплате** на странице **Сопоставление проводок**. Если этот параметр имеет значение **Да**, скидка используется *** использование скидки при оплате наличными *** установлено значение **обычное**. Когда *** использование скидки при оплате наличными *** установлено значение **всегда**, всегда учитывается скидки при оплате наличными, независимо от значения этого поля. Когда *** использование скидки при оплате наличными *** установлено значение **никогда**, скидка при оплате наличными не переводится, независимо от значения этого поля.
...
-   **Назначить приоритет сопоставления (только расчеты с клиентами)** — установите для этого параметра значение **Да**, чтобы включить кнопку **Пометить по приоритетам** на страницах **Ввод платежей клиента** и **Сопоставление проводок**. Эта кнопка позволяет назначить порядок предопределенных сопоставления для проводки.  После применения порядок сопоставления проводки заказа и распределения оплаты можно изменить перед разноской.
-   **Использовать приоритет для автоматических сопоставлений** — варианта **Да** использовать определенный приоритет при сопоставлении проводок, автоматически. Это поле доступно только тогда, когда **назначить приоритет сопоставления** и **автоматическое сопоставление**, настроенных на **Да**.
