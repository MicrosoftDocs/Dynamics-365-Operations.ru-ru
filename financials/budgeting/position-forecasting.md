---
title: "Прогнозируемая должность"
description: 
author: twheeloc
manager: AnnBe
ms.date: 04/04/2017
...
# <a name="position-forecasting"></a>Прогнозируемая должность
...
[![Графическое сверху](./media/graphic-top.png)](./media/graphic-top.png) 

Прогнозирование должностей использует три основных компонента для предоставления точных бюджетные суммы для позиции расходов. Эти сумы затем можно включить в бюджетный план для бюджетных вычислений. 
...
В типовой процесс для прогнозирования должностей необходимо сначала создать (элементы бюджета затрат и групп компенсации) компонентов программы установки. Затем на основе существующих позиций создаются прогнозируемые должности. Затем можно вносить корректировки. Например, можно добавлять и удалять должности, менять ставки зарплаты и затраты на выплаты льгот, а также добавлять увеличения заработной платы. Можно создать несколько версий прогнозируемых должностей, что сделает возможным сравнения между разными сценариями бюджетирования. Затем можно включить прогнозируемые должности в бюджетные платы и добавить затраты из позиций прогноза в качестве строк бюджетного плана.
...
Организации Jodi предоставляет скидку обучения 5 процентов от базовой зарплаты для сотрудника. Джоди хочет создать элемент затрат бюджета для этого расхода. Она создает новый элемент затрат бюджета и присваивает ему тип затрат бюджета **Льгота**.
...
Даты вступления в силу и даты истечения срока в таблицах тарифа компенсации позволяют менять ставки оплаты в любую дату. Эта функция полезна, если профсоюз договорился об увеличении зарплаты всем сотрудникам в середине бюджетного цикла. В этом случае измените дату окончания срока действия существующей таблицы, на дату, предшествующую дате изменения курса и добавьте таблицу соотношения, с которого начинается новая дата. При создании новой таблицы курс, при выборе **создать новую сетку компенсаций из существующей сетки**, можно выбрать существующую таблицу курс из отдела кадров. В созданной таблице ставок параметр **Массовое изменение** позволяет применить процент или фиксированное увеличение или уменьшение суммы ко всем ставкам в сетке. 
...
Для наиболее точного планирования бюджета, можно создать прогнозируемые должности, используя сведения из существующей должности в Microsoft Dynamics 365 для операций, независимо от того, является ли позиция в настоящее время заполненный или Незаполненный. 
...
Задание параметра **Назначить группу компенсации** превалирует над настройкой **Назначение элемента затрат бюджета**. В то же время, можно использовать два параметра. 

[![graphic4](./media/graphic4.png)](./media/graphic4.png) 

Другим вариантом является назначение дата годовщины. Выбранная дата (отрегулированная дата начала, дата начала работника, дата начала занятости или дата стажа) назначенного сотрудника затем задается в качестве даты годовщины прогнозируемой должности и используется для справки и при создании повышения оплаты труда.
...
Доступны два параметра в **бюджета затрат элемента** страница установки: **добавление позиций** и **редактирование должностей**. Параметр **Добавить в должности** добавляет элемент затрат бюджета в выбранные прогнозируемые должности. Если элемент уже назначен прогнозируемой должности, должность пропускается. Параметр **Обновить должности** применяет текущие значения (счет ГК, процент, годовая сумма и т. д.) к выбранным прогнозируемым должностям. 

Для каждого процесса существует одинаковая страница, где можно выбрать прогнозируемые должности. На странице **Добавить в должности** показаны все прогнозируемые должности, доступные для выбора, в то время как на странице **Обновить должности** показаны только прогнозируемые должности с уже назначенным элементом затрат бюджета. (Таким образом, **редактирование должностей** страница дает возможность узнать, какие прогнозные позиции уже прикреплен элемент затрат.) Выполняется переход прогнозируемые должности из верхней сетки нижней сетке для включения их в обновлении. 
...
Создание и изменение прогнозируемых должностей предназначено для добавления в бюджетные планы, бюджетные планы, включая наиболее точным бюджетных сумм. Существует два метода добавления прогнозируемых должностей в бюджетные планы. Можно воспользоваться процессом создания или выбора в бюджетном плане.
