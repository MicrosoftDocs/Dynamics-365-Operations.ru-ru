---
title: "Интеграция планирования бюджета с другими модулями"
description: 
author: twheeloc
manager: AnnBe
ms.date: 04/04/2017
...
# <a name="budget-planning-integration-with-other-modules"></a>Интеграция планирования бюджета с другими модулями
...
-   **Создать новый бюджетный план** создает новый план с атрибутами, которые указаны в ** целевой ** раздела. Эти атрибуты не обязательно должны быть уникальными. Следовательно, два плана могут иметь одинаковое название и другие значения.
...
Для всех процессов **источника** вкладке можно фильтровать данные с помощью **фильтр** кнопка. По умолчанию добавляются определенные поля фильтра для каждого процесса. Например, для процесса **Создать бюджетный план из Главной книги** доступны категории **Счет учета** и **Счет ГК**. Они отображаются на странице создания. Все добавляемые в фильтр поля также добавляются на страницу, вместе со всеми добавляемыми критериями.
...
Многие поля на вкладке **Цель** становятся доступными для редактирования или только чтения в зависимости от выбираемого действия. При переходе от создания нового бюджетного плана к обновлению существующего плана поле **Имя бюджетного плана** становится недоступным, а поля, связанные с выбором существующего плана, — доступными. На обоих уровнях **целевой** вкладке и ** источника ** вкладке **книги** будет всегда недоступно, поскольку значение зависит от выбранного процесса планирования бюджета. 
...
Поле **Минимум** позволяет задать пороговую сумму для создания строки бюджетного плана. Если исходная сумма меньше этого числа, бюджетный план не создается. Значение **0,00** позволяет все суммы, но не ограничить строки для положительные суммы. (Значение не ограничивает строки, чтобы положительные суммы. Отрицательные суммы всегда включаются и обычно представляют кредитных операций.)
...
Процесс **Создать бюджетный план из основных средств** не имеет механизмов для суммирования по периоду или дню. Имеется также возможность установить план как статистические. Можно использовать этот Периодический процесс для включения прогнозируемых проводок по основным средствам в планировании бюджета.
...
Строки бюджетного плана суммируются по счету ГК и позиции. Тем не менее можно исключить позицию, таким образом, чтобы строки суммируются счетом ГК. На вкладке **Цель** задайте для параметра **Включить позицию в бюджетный план** значение **Нет**.
...
Чтобы выбрать только затраты или доходы для строк бюджетного плана, воспользуйтесь фильтром, чтобы выбрать **Обновления бюджета: тип суммы = затраты**. Чтобы выбрать только один тип прогноза, воспользуйтесь фильтром, чтобы выбрать **Обновления бюджета: тип проводки = *xxx***. 
