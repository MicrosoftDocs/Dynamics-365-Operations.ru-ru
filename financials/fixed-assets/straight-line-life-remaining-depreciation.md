...
При выборе **календаря** в ***год амортизации*** поле, используется год с 1 января по 31 декабря предполагается, что, даже если финансовый календарь был определен по-разному. Параметр **Календарь** обновляет базу амортизации 1-го января каждого года. Обычно базой амортизации является остаточная стоимость минус ликвидационная стоимость. В примерах ниже база амортизации является числителем в первом выражении в столбце расчетов. Если для года амортизации выбран вариант **Календарь**, в поле **Частота периода** доступны следующие параметры.
