...
Параметр **Календарь** обновляет базу амортизации 1-го января каждого года. Как правило амортизация является остаточная стоимость минус ликвидационная стоимость. В примерах ниже база амортизации является числителем в первом выражении в столбце расчетов. 
...
> [!NOTE] 
> Как правило, когда сумму, которая вычисляется с использованием уменьшаемым метод амортизации сальдо в 200% становится меньше суммы, по которой будет рассчитана с помощью линейного метода выполняется преобразование для линейного метода для оставшегося срока службы.
